 ...
 * @return 界面对象的界面。
*/
QWidget* SLDCDcuGUI::ShowMe(QWidget* parent, const QString& strTitle)
{
    m_strTitle = strTitle;

    SLDCDcuGUIDlg* form;
    form = new SLDCDcuGUIDlg(parent);
 ...
 * @return szHelpID:长度。
*/
int SLDCDcuGUI::GetContextHelpID(char* szHelpID)
{
...
*/
int SLDCDcuGUI::GetModuleInfo(TLxTsspModuleInfo& moduleInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SLDCDcuGUI::GetObjectInfo(TLxTsspObjectInfo& ObjectInfo) const
{
...
//遍历xml文件，获取参数取值范围
int SLDCDcuGUI::QueryParaBlock(QString ConfigPath)
{
...
int SLDCDcuGUI::GetAllMapValueStringByPara(int MapSum,QDomNode pInDocNode_Para)
{
 ...
 */
int SLDCDcuGUI::GetErrorMsg(int iErrorNo, QString& strErrorMsg)
{
 ...
 */
void SLDCDcuGUI::SendMessage(TLxTsspMessage msg)
{
    return;
}
 ...
 * @return 1：成功；-1：失败。
 */
int SLDCDcuGUI::SetParameter(QMap<quint64, QByteArray> parameterList)
{
 ...
 */
int SLDCDcuGUI::SetParameter(quint64 ui64ParamID, QByteArray parameter)
{
 ...
 */
int SLDCDcuGUI::GetParameterID(QList<quint64>& pParameterIDList) const
{
 ...
 * @return 大于等于0：参数块个数。-1：失败。
 */
int SLDCDcuGUI::GetParameter(QMap<quint64, QByteArray>& parameterList) const
{
    SLDCDCUPara tmpComPara;          //参数

    SLDCDCUStatu *tmpComStatus = NULL;  //状态
    ...
    if(tmpStateMgr != NULL)
        tmpComStatus  =
                (SLDCDCUStatu*)tmpStateMgr->FindOneItem(m_StatusComSynID, stLen);
    ...
    QByteArray tmpCh;
    tmpCh.resize(sizeof(SLDCDCUPara));
    memcpy(tmpCh.data(), &tmpComPara, sizeof(SLDCDCUPara));
 ...
 * @return 1：成功；-1：失败。
 */
int SLDCDcuGUI::GetParameter(quint64 ui64ParamID, QByteArray& parameter) const
{
    if(m_ParamList.contains(ui64ParamID))
    {
 ...
 */
int SLDCDcuGUI::GetState(QMap<quint64, TLxTsspDeviceState>& stateList) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int SLDCDcuGUI::GetState(quint64 ui64StatusID, TLxTsspDeviceState& status) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int SLDCDcuGUI::GetStateDirect(quint64 ui64StatusID, TLxTsspDeviceState& status)
{
    if(m_StateList.contains(ui64StatusID))
    {
        //
 ...
 */
int SLDCDcuGUI::WriteData(quint64 ui64DataID, uchar* aData, uint uiDataLen)
{
    return 0;
}
 ...
 */
int SLDCDcuGUI::ReadData(quint64 ui64DataID, uchar* aData, uint& uiDataLen,
                             uint& uiReloadCount)
{
    return 0;
}

int SLDCDcuGUI::CreateDefaultMacroObj(ILxTsspParameterMacro* pMacroObj)
{
...
QWidget* SLDCDcuGUI::ShowMeInEdit(QWidget* parent,uchar ucType,ILxTsspParameterMacro* pMacroObj)
{
    g_form = new SLDCDcuGUIDlg(parent);
...
int SLDCDcuGUI::SaveToMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    if(g_form != NULL)
    {
