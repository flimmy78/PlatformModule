...
extern "C" int GetConfigFormat(QString& strConfigFormat)
{
    ...
    strConfigFormat =
            QString("<Module Name=\"libUPNet_Switch.so\" StationID=\"2311\" SubSysID=\"04\" DevID=\"3C00\" Note=%1> \n "
            "<Object ID=\"6000\" Num=\"1\" IndexStart=\"0\" Alias =\" \" Note=%1>...</Object> \n "
    ...
    for(int i=0; i<nodeList.length(); i++)
    {
        ...
        if(usObjID == 0x6000)
        {
            for(int k=iStart; k<iStop; k++)
            {
                ...
                strcpy(ObjectInfo.szConfigID, "D_Y17-92_127");
                strcpy(ObjectInfo.szAlias, objAlias.toUtf8().data());
                strcpy(ObjectInfo.szComment, objNote.toUtf8().data());
                g_ObjectList.insert(ui64ObjectID, new UPNet_SwitchGUI(ObjectInfo));
            }
        }
    }
