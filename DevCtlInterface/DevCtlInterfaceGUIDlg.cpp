...
#include "ui_DevCtlInterfaceGUIDlg.h"
...
extern quint64 GetID(ushort usStationID, uchar ucSubSysID, ushort usDeviceID, ushort usType,
              uchar ucSn);
TTCDevCtlInterfaceGUIDlg::TTCDevCtlInterfaceGUIDlg(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::TTCDevCtlInterfaceGUIDlg)
{
    ui->setupUi(this);
    m_pStateMgr = NULL;
...
//定时器
void TTCDevCtlInterfaceGUIDlg::timerEvent(QTimerEvent *)
{
    showDatagram();
}

void TTCDevCtlInterfaceGUIDlg::showDatagram()
{
...
void TTCDevCtlInterfaceGUIDlg::closeEvent(QCloseEvent *event)
{
    event->accept();
}

TTCDevCtlInterfaceGUIDlg::~TTCDevCtlInterfaceGUIDlg()
{
...
void TTCDevCtlInterfaceGUIDlg::setText(const QString& strTitle,ParaStruct xmlsetText[3][60],QString MapMean[50][30],int MapNum[50],uchar Type,ILxTsspParameterMacro* )
{
    m_strTitle = strTitle;
...
//初始化对话框
void TTCDevCtlInterfaceGUIDlg::InitDLG(int )
{
...
void TTCDevCtlInterfaceGUIDlg::SetObjInfo(TLxTsspObjectInfo ObjectInfo)
{
...
//保存参数宏
int TTCDevCtlInterfaceGUIDlg::SaveToMacro(ILxTsspParameterMacro* )
{
    //判断合法性
    return -1;
    //日志信息
...
void TTCDevCtlInterfaceGUIDlg::SendMessage(char* pData, unsigned short  usLen, unsigned short usCMD, unsigned short usTID)
{
    static uint seq = 0;
...
DWORDLONG TTCDevCtlInterfaceGUIDlg::StrToSynCode(const char * pStr)
{
    char c,C;
    const char*  p;
...
bool TTCDevCtlInterfaceGUIDlg::exchange(const QString& buf, int& v)
{
...
QString TTCDevCtlInterfaceGUIDlg::getStrFromDatagram(QByteArray &datagram)
{
...
void TTCDevCtlInterfaceGUIDlg::on_pushButton_clicked()
{
    char buf[10] = {0};
...
void TTCDevCtlInterfaceGUIDlg::on_pushButton_traceChannelSel_clicked()
{

}

void TTCDevCtlInterfaceGUIDlg::on_pushButton_realErrCodeFrmReport_clicked()
{
    char buf[4] = {0};
    int i = 0;
...
void TTCDevCtlInterfaceGUIDlg::on_pushButton_doppTestreport_clicked()
{
    char tmp;
...
void TTCDevCtlInterfaceGUIDlg::on_pushButton_show_clicked()
{
    if(!m_pTimer)
        return;
    m_pTimer->start(5000);
...
void TTCDevCtlInterfaceGUIDlg::on_timer_show_response()
{
...
void TTCDevCtlInterfaceGUIDlg::on_pushButton_clear_clicked()
{
    m_pTimer->stop();
    clear();
}

void TTCDevCtlInterfaceGUIDlg::clear()
{
...
QString TTCDevCtlInterfaceGUIDlg::getTaskQStr()
{
...
QString TTCDevCtlInterfaceGUIDlg::getDataTransFreqRgeTstQStr()
{
...
QString TTCDevCtlInterfaceGUIDlg::getRealErrCodFrmStatisticsQStr()
{
...
void TTCDevCtlInterfaceGUIDlg::on_pushButton_wordModeSet_clicked()
{
...
void TTCDevCtlInterfaceGUIDlg::on_pushButtonWordWay_clicked()
{
    char ch = (char)ui->comboBoxWorkWay->currentIndex()+1;
    SendMessage(&ch, 1, 0x200, 0x325);
}

void TTCDevCtlInterfaceGUIDlg::on_pushButton_pauseStatu_clicked()
{
...
void TTCDevCtlInterfaceGUIDlg::on_pushButton_statuCmd_clicked()
{
    if(g_pSearchThread)
        g_pSearchThread->continu();
    killTimer(m_timerStatu);
...
void TTCDevCtlInterfaceGUIDlg::on_pushButton_commCmd_clicked()
{
...
void TTCDevCtlInterfaceGUIDlg::on_pushButton_commClear_clicked()
{
...
void TTCDevCtlInterfaceGUIDlg::on_pushButton_statuClear_clicked()
{
...
void TTCDevCtlInterfaceGUIDlg::on_pushButton_flash_clicked()
{
