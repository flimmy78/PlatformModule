...
namespace
{
    USBBBE_SysParaSetGUIDlg* g_form = NULL;
}
extern quint64 GetID(ushort usStationID, uchar ucSubSysID, ushort usDeviceID, ushort usType,
              uchar ucSn);

USBBBE_SysParaSetGUI::USBBBE_SysParaSetGUI(TLxTsspObjectInfo ObjectInfo)
{
    //初始化
 ...
 * @return 界面对象的界面。
*/
QWidget* USBBBE_SysParaSetGUI::ShowMe(QWidget* parent, const QString& strTitle)
{
    m_strTitle = strTitle;

    USBBBE_SysParaSetGUIDlg* form;

    form = new USBBBE_SysParaSetGUIDlg(parent);
 ...
 * @return szHelpID:长度。
*/
int USBBBE_SysParaSetGUI::GetContextHelpID(char* szHelpID)
{
...
*/
int USBBBE_SysParaSetGUI::GetModuleInfo(TLxTsspModuleInfo& moduleInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int USBBBE_SysParaSetGUI::GetObjectInfo(TLxTsspObjectInfo& ObjectInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int USBBBE_SysParaSetGUI::PreInitialize()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int USBBBE_SysParaSetGUI::Initialize()
{
    //日志信息
    TLxTsspLog tmpLog;
    QString strLog;
    ...
    stCurPath = stCurPath + "/System/ModuleConfig/USBBBE_SysParaSetGUI.xml";
    //遍历xml文件，获取参数取值范围
    m_MapSumFlag = 0;
...
//遍历xml文件，获取参数取值范围
int USBBBE_SysParaSetGUI::QueryParaBlock(QString ConfigPath)
{
...
int USBBBE_SysParaSetGUI::GetAllMapValueStringByPara(int MapSum,QDomNode pInDocNode_Para)
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int USBBBE_SysParaSetGUI::Start()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
bool USBBBE_SysParaSetGUI::ClearUp()
{
    return 1;
}
 ...
 *           其它错误依情况增加；
 */
int USBBBE_SysParaSetGUI::GetErrorNumber()
{
    return m_iErrCode;
}
 ...
 */
int USBBBE_SysParaSetGUI::GetErrorMsg(int iErrorNo, QString& strErrorMsg)
{
 ...
 */
bool USBBBE_SysParaSetGUI::IsOnline()
{
    //对象初始化
    //查找对象管理器
    LxTsspObjectManager* pObjectManager = LxTsspObjectManager::Instance();
 ...
 */
void USBBBE_SysParaSetGUI::SendMessage(TLxTsspMessage msg)
{
    return;
}
 ...
 * @return 1：成功；-1：失败。
 */
int USBBBE_SysParaSetGUI::SetParameter(QMap<quint64, QByteArray> parameterList)
{
 ...
 */
int USBBBE_SysParaSetGUI::SetParameter(quint64 ui64ParamID, QByteArray parameter)
{
 ...
 */
int USBBBE_SysParaSetGUI::GetParameterID(QList<quint64>& pParameterIDList) const
{
 ...
 * @return 大于等于0：参数块个数。-1：失败。
 */
int USBBBE_SysParaSetGUI::GetParameter(QMap<quint64, QByteArray>& parameterList) const
{
    ...
    if(tmpStateMgr != NULL)
        tmpMCSSysStatus  =
                (MCSSysDataStatusStruct*)tmpStateMgr->FindOneItem(m_StatusComSynID, stLen);
 ...
 * @return 1：成功；-1：失败。
 */
int USBBBE_SysParaSetGUI::GetParameter(quint64 ui64ParamID, QByteArray& parameter) const
{
    if(m_ParamList.contains(ui64ParamID))
    {
 ...
 */
int USBBBE_SysParaSetGUI::GetState(QMap<quint64, TLxTsspDeviceState>& stateList) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int USBBBE_SysParaSetGUI::GetState(quint64 ui64StatusID, TLxTsspDeviceState& status) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int USBBBE_SysParaSetGUI::GetStateDirect(quint64 ui64StatusID, TLxTsspDeviceState& status)
{
    if(m_StateList.contains(ui64StatusID))
    {
        //
 ...
 */
int USBBBE_SysParaSetGUI::WriteData(quint64 ui64DataID, uchar* aData, uint uiDataLen)
{
    return 0;
}
 ...
 */
int USBBBE_SysParaSetGUI::ReadData(quint64 ui64DataID, uchar* aData, uint& uiDataLen,
                             uint& uiReloadCount)
{
    return 0;
}

TLxTsspParamMacroInformation* USBBBE_SysParaSetGUI::getMacroInfo()
{
    LxTsspObjectManager* pObjectManager = LxTsspObjectManager::Instance();
...
void USBBBE_SysParaSetGUI::updateParam(MCSSysDataStruct& comPara, ILxTsspParameterMacro* pMacroObj)
{
...
int USBBBE_SysParaSetGUI::CreateDefaultMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    MCSSysDataStruct tmpParam;
    //任务代号
    memcpy(tmpParam.ucTaskDescribe, &m_ParaSetStruct[0][0].CVal.cValue, 6);
    //任务标识
    tmpParam.uiM = m_ParaSetStruct[0][1].InitVal.lval;
    //任务标识
    tmpParam.dwlAnswerRevUpFreq = m_ParaSetStruct[0][2].InitVal.lval;
    //下行频率
    tmpParam.dwlAnswerRevDownFreq = m_ParaSetStruct[0][3].InitVal.lval;
    //基带送数选择
    tmpParam.ucSendMode = (UCHAR)m_ParaSetStruct[0][4].InitVal.lval + 1;
    //信息传输格式
    tmpParam.ucDataMode = (UCHAR)m_ParaSetStruct[0][5].InitVal.lval + 1;
    //测距采样率
    tmpParam.ucCJSendRate = (UCHAR)m_ParaSetStruct[0][6].InitVal.lval + 1;
    //测速采样率
    tmpParam.ucCSSendRate = (UCHAR)m_ParaSetStruct[0][7].InitVal.lval + 1;
    //测距数据标识码
    tmpParam.ucCjCode = (UCHAR)m_ParaSetStruct[0][10].InitVal.lval;
    //测速数据标识码
    tmpParam.ucCsCode = (UCHAR)m_ParaSetStruct[0][11].InitVal.lval;
    //遥测数据标识码
    tmpParam.ucTmCode = (UCHAR)m_ParaSetStruct[0][12].InitVal.lval;

    //取当前设置的参数，存入框架
    updateParam(tmpParam, pMacroObj);
    QByteArray byteArray;
    byteArray.resize(sizeof(tmpParam));
    memcpy(byteArray.data(), &tmpParam, sizeof(tmpParam));
    if(pMacroObj != NULL)
    {
...
QWidget* USBBBE_SysParaSetGUI::ShowMeInEdit(QWidget* parent,uchar ucType, ILxTsspParameterMacro* pMacroObj)
{
    g_form = new USBBBE_SysParaSetGUIDlg(parent);
...
int USBBBE_SysParaSetGUI::SaveToMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    if(g_form != NULL)
    {
