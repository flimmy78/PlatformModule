...
extern "C" int GetModuleInfo(TLxTsspModuleInfo& moduleInfo)
{
    ...
    /// 配置标识，如：D_Y17-61_008
    strcpy(moduleInfo.szConfigID, "D_Y17-92_108\0");
...
extern "C" int GetConfigFormat(QString& strConfigFormat)
{
    ...
    strConfigFormat = QString(
            "<Module Name=\"libUSBBBE_SysParaSet.so\" StationID=\"2311\" SubSysID=\"04\" DevID=\"0401\" Note=\"%1\"> \n "
            "<Object ID=\"5141\" Num=\"1\" IndexStart=\"0\" Alias =\" \" Note=\"参数设置\">...</Object> \n "
    ...
    for(int i=0; i<nodeList.length(); i++)
    {
        ...
        if(usObjID == 0x5141)
        {
            for(int k=iStart; k<iStop; k++)
            {
                ...
                g_ObjectList.insert(ui64ObjectID, new USBBBE_SysParaSetGUI(ObjectInfo));
            }
        }
    }
