...
namespace
{
    USBBBE_CJParaSetGUIDlg* g_form = NULL;
}
extern quint64 GetID(ushort usStationID, uchar ucSubSysID, ushort usDeviceID, ushort usType,
              uchar ucSn);
USBBBE_CJParaSetGUI::USBBBE_CJParaSetGUI(TLxTsspObjectInfo ObjectInfo)
{
    //初始化
 ...
 * @return 界面对象的界面。
*/
QWidget* USBBBE_CJParaSetGUI::ShowMe(QWidget* parent, const QString& strTitle)
{
    m_strTitle = strTitle;

    USBBBE_CJParaSetGUIDlg* form;

    form = new USBBBE_CJParaSetGUIDlg(parent);
 ...
 * @return szHelpID:长度。
*/
int USBBBE_CJParaSetGUI::GetContextHelpID(char* szHelpID)
{
...
*/
int USBBBE_CJParaSetGUI::GetModuleInfo(TLxTsspModuleInfo& moduleInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int USBBBE_CJParaSetGUI::GetObjectInfo(TLxTsspObjectInfo& ObjectInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int USBBBE_CJParaSetGUI::PreInitialize()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int USBBBE_CJParaSetGUI::Initialize()
{
    //日志信息
    TLxTsspLog tmpLog;
    QString strLog;
    ...
    stCurPath = stCurPath + "/System/ModuleConfig/USBBBE_CJParaSetGUI.xml";
    //遍历xml文件，获取参数取值范围
    m_MapSumFlag = 0;
    ...
    if (pItem->vSubItem.size() > 0)
    {
        ...
        /*************************框架**********************************/
        tmpCjParaID = GetID(sItem.usStationID, sItem.ucSubSysID, sItem.usDeviceID,
                                     sItem.usTID, sItem.ucSN);
        /*************************框架**********************************/

        /*************************状态**********************************/
        tmpCjStatusID = GetID(sItem.usStationID, sItem.ucSubSysID, sItem.usDeviceID,
...
//遍历xml文件，获取参数取值范围
int USBBBE_CJParaSetGUI::QueryParaBlock(QString ConfigPath)
{
...
int USBBBE_CJParaSetGUI::GetAllMapValueStringByPara(int MapSum,QDomNode pInDocNode_Para)
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
bool USBBBE_CJParaSetGUI::ClearUp()
{
    return 1;
}
 ...
 *           其它错误依情况增加；
 */
int USBBBE_CJParaSetGUI::GetErrorNumber()
{
    return m_iErrCode;
}
 ...
 */
int USBBBE_CJParaSetGUI::GetErrorMsg(int iErrorNo, QString& strErrorMsg)
{
 ...
 */
bool USBBBE_CJParaSetGUI::IsOnline()
{
    //对象初始化
    //查找对象管理器
    LxTsspObjectManager* pObjectManager = LxTsspObjectManager::Instance();
 ...
 */
void USBBBE_CJParaSetGUI::SendMessage(TLxTsspMessage msg)
{
    return;
}
 ...
 * @return 1：成功；-1：失败。
 */
int USBBBE_CJParaSetGUI::SetParameter(QMap<quint64, QByteArray> parameterList)
{
 ...
 */
int USBBBE_CJParaSetGUI::SetParameter(quint64 ui64ParamID, QByteArray parameter)
{
 ...
 */
int USBBBE_CJParaSetGUI::GetParameterID(QList<quint64>& pParameterIDList) const
{
 ...
 * @return 大于等于0：参数块个数。-1：失败。
 */
int USBBBE_CJParaSetGUI::GetParameter(QMap<quint64, QByteArray>& parameterList) const
{
    MCSUSBCjParamStruct tmpCjPara;       //参数

    MCSUSBCjStatusStruct* tmpCjStatus = NULL;      //状态
    ...
    if(tmpStateMgr != NULL)
        tmpCjStatus  =
                (MCSUSBCjStatusStruct*)tmpStateMgr->FindOneItem(tmpCjStatusID, stLen);

    if(tmpCjStatus != NULL)
    {
        tmpCjPara = tmpCjStatus->tParams;
    }

    QByteArray  tmpCh;
    tmpCh.resize(sizeof(MCSUSBCjParamStruct));
    memcpy(tmpCh.data(), &tmpCjPara, sizeof(MCSUSBCjParamStruct));
    parameterList.insert(tmpCjParaID, tmpCh);
 ...
 * @return 1：成功；-1：失败。
 */
int USBBBE_CJParaSetGUI::GetParameter(quint64 ui64ParamID, QByteArray& parameter) const
{
    if(m_ParamList.contains(ui64ParamID))
    {
 ...
 */
int USBBBE_CJParaSetGUI::GetState(QMap<quint64, TLxTsspDeviceState>& stateList) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int USBBBE_CJParaSetGUI::GetState(quint64 ui64StatusID, TLxTsspDeviceState& status) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int USBBBE_CJParaSetGUI::GetStateDirect(quint64 ui64StatusID, TLxTsspDeviceState& status)
{
    if(m_StateList.contains(ui64StatusID))
    {
        //
 ...
 */
int USBBBE_CJParaSetGUI::WriteData(quint64 ui64DataID, uchar* aData, uint uiDataLen)
{
    return 0;
}
 ...
 */
int USBBBE_CJParaSetGUI::ReadData(quint64 ui64DataID, uchar* aData, uint& uiDataLen,
                             uint& uiReloadCount)
{
    return 0;
}

int USBBBE_CJParaSetGUI::CreateDefaultMacroObj(ILxTsspParameterMacro* pMacroObj)
{

    MCSUSBCjParamStruct aMCSUSBCjParam;
    //最低侧音选择
    aMCSUSBCjParam.ucLSToneCheck = m_ParaSetStruct[0][0].InitVal.lval + 1;
    //侧音等待延时
    aMCSUSBCjParam.uiSToneWait = m_ParaSetStruct[0][2].InitVal.lval;
    //距离预报方式
    aMCSUSBCjParam.ucPreRangDataMd = m_ParaSetStruct[0][3].InitVal.lval + 1;
    //距离解模糊
    aMCSUSBCjParam.ucRMHCheck = m_ParaSetStruct[0][4].InitVal.lval + 1;
    //距离预报值
    aMCSUSBCjParam.ulPridictR = m_ParaSetStruct[0][5].InitVal.lval;
    //应答机距离零值
    aMCSUSBCjParam.uiRSPZero = (UINT)(m_ParaSetStruct[0][6].InitVal.lval * 100.0);

    //取当前设置的参数，存入框架
    QByteArray byteArray;
    byteArray.resize(sizeof(aMCSUSBCjParam));
    memcpy(byteArray.data(),&aMCSUSBCjParam,sizeof(aMCSUSBCjParam));
    if(pMacroObj!=NULL)
    {
        pMacroObj->UpdateParameterBlock(tmpCjParaID,byteArray);
    }
    return 1;
}

QWidget* USBBBE_CJParaSetGUI::ShowMeInEdit(QWidget* parent,uchar ucType,ILxTsspParameterMacro* pMacroObj)
{
    g_form = new USBBBE_CJParaSetGUIDlg(parent);
...
int USBBBE_CJParaSetGUI::SaveToMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    if(g_form!=NULL)
    {
