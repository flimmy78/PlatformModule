#include "SS1BBE_CjCsParamSetGUI.h"
...
namespace
{
    SS1BBE_CjCsParamSetGUIDlg* g_form = NULL;
}
extern quint64 GetID(ushort usStationID, uchar ucSubSysID, ushort usDeviceID, ushort usType,
              uchar ucSn);
SS1BBE_CjCsParamSetGUI::SS1BBE_CjCsParamSetGUI(TLxTsspObjectInfo ObjectInfo)
{
    //初始化
 ...
 * @return 界面对象的界面。
*/
QWidget* SS1BBE_CjCsParamSetGUI::ShowMe(QWidget* parent, const QString& strTitle)
{
    m_strTitle = strTitle;

    SS1BBE_CjCsParamSetGUIDlg* form;

    form = new SS1BBE_CjCsParamSetGUIDlg(parent);
 ...
 * @return szHelpID:长度。
*/
int SS1BBE_CjCsParamSetGUI::GetContextHelpID(char* szHelpID)
{
...
*/
int SS1BBE_CjCsParamSetGUI::GetModuleInfo(TLxTsspModuleInfo& moduleInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS1BBE_CjCsParamSetGUI::GetObjectInfo(TLxTsspObjectInfo& ObjectInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS1BBE_CjCsParamSetGUI::PreInitialize()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS1BBE_CjCsParamSetGUI::Initialize()
{
    //日志信息
    TLxTsspLog tmpLog;
    QString strLog;
    ...
    stCurPath = stCurPath + "/System/ModuleConfig/SS1BBE_CjCsParamSetGUI.xml";
    //遍历xml文件，获取参数取值范围
    m_MapSumFlag = 0;
...
//遍历xml文件，获取参数取值范围
int SS1BBE_CjCsParamSetGUI::QueryParaBlock(QString ConfigPath)
{
...
int SS1BBE_CjCsParamSetGUI::GetAllMapValueStringByPara(int MapSum,QDomNode pInDocNode_Para)
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS1BBE_CjCsParamSetGUI::Start()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS1BBE_CjCsParamSetGUI::Stop()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
bool SS1BBE_CjCsParamSetGUI::ClearUp()
{
    return 1;
}
 ...
 *           其它错误依情况增加；
 */
int SS1BBE_CjCsParamSetGUI::GetErrorNumber()
{
    return m_iErrCode;
}
 ...
 */
int SS1BBE_CjCsParamSetGUI::GetErrorMsg(int iErrorNo, QString& strErrorMsg)
{
 ...
 */
bool SS1BBE_CjCsParamSetGUI::IsOnline()
{
    //对象初始化
    //查找对象管理器
    LxTsspObjectManager* pObjectManager = LxTsspObjectManager::Instance();
 ...
 */
void SS1BBE_CjCsParamSetGUI::SendMessage(TLxTsspMessage msg)
{
    return;
}
 ...
 * @return 1：成功；-1：失败。
 */
int SS1BBE_CjCsParamSetGUI::SetParameter(QMap<quint64, QByteArray> parameterList)
{
 ...
 */
int SS1BBE_CjCsParamSetGUI::SetParameter(quint64 ui64ParamID, QByteArray parameter)
{
 ...
 */
int SS1BBE_CjCsParamSetGUI::GetParameterID(QList<quint64>& pParameterIDList) const
{
 ...
 * @return 大于等于0：参数块个数。-1：失败。
 */
int SS1BBE_CjCsParamSetGUI::GetParameter(QMap<quint64, QByteArray>& parameterList) const
{
    SMCU::MCSSSCjParamStruct4 tmpComPara;          //参数

    SMCU::MCSSSCjStatusStruct4 *tmpComStatus = NULL;  //状态参数
    ...
    if(tmpStateMgr != NULL)
        tmpComStatus  =
                (SMCU::MCSSSCjStatusStruct4*)tmpStateMgr->FindOneItem(m_StatusComSynID, stLen);

    if(tmpComStatus != NULL)
    {
        for (int i = 0; i< MAX_TARGET_COUNT; i++)
            tmpComPara.SSCjParam[i] = tmpComStatus->SSCjStatu[i].tParams;
    }

    QByteArray  tmpCh;
    tmpCh.resize(sizeof(SMCU::MCSSSCjParamStruct4));
    memcpy(tmpCh.data(), &tmpComPara, sizeof(SMCU::MCSSSCjParamStruct4));
 ...
 * @return 1：成功；-1：失败。
 */
int SS1BBE_CjCsParamSetGUI::GetParameter(quint64 ui64ParamID, QByteArray& parameter) const
{
    if(m_ParamList.contains(ui64ParamID))
    {
 ...
 */
int SS1BBE_CjCsParamSetGUI::GetState(QMap<quint64, TLxTsspDeviceState>& stateList) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int SS1BBE_CjCsParamSetGUI::GetState(quint64 ui64StatusID, TLxTsspDeviceState& status) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int SS1BBE_CjCsParamSetGUI::GetStateDirect(quint64 ui64StatusID, TLxTsspDeviceState& status)
{
    if(m_StateList.contains(ui64StatusID))
    {
        //
 ...
 */
int SS1BBE_CjCsParamSetGUI::WriteData(quint64 ui64DataID, uchar* aData, uint uiDataLen)
{
    return 0;
}
 ...
 */
int SS1BBE_CjCsParamSetGUI::ReadData(quint64 ui64DataID, uchar* aData, uint& uiDataLen,
                             uint& uiReloadCount)
{
    return 0;
}

int SS1BBE_CjCsParamSetGUI::CreateDefaultMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    SMCU::MCSSSCjParamStruct4 tmpComPara;          //参数
    for(int nIndex = 0; nIndex < MAX_TARGET_COUNT; nIndex++)
    {
        //距离解模糊
        tmpComPara.SSCjParam[nIndex].ucRMHCheck = m_ParaSetStruct[nIndex][0].InitVal.lval + 1;
        //距离预报值
        tmpComPara.SSCjParam[nIndex].ulPridictR = m_ParaSetStruct[nIndex][1].InitVal.lval * 1000;
        //距离预报方式
        tmpComPara.SSCjParam[nIndex].ucPreRangDataMd = m_ParaSetStruct[nIndex][2].InitVal.lval + 1;
        //应答机零值距离
        tmpComPara.SSCjParam[nIndex].uiRSPDisZero = m_ParaSetStruct[nIndex][3].InitVal.lval * 2000;
    }
...
QWidget* SS1BBE_CjCsParamSetGUI::ShowMeInEdit(QWidget* parent, uchar ucType, ILxTsspParameterMacro* pMacroObj)
{
    g_form = new SS1BBE_CjCsParamSetGUIDlg(parent);
...
int SS1BBE_CjCsParamSetGUI::SaveToMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    if(g_form != NULL)
    {
