#ifndef SS1BBE_CJCSPARAMSETGUIDLG_H
#define SS1BBE_CJCSPARAMSETGUIDLG_H
...
namespace Ui
{
    class SS1BBE_CjCsParamSetGUIDlg;
}

class SS1BBE_CjCsParamSetGUIDlg : public QDialog
{
    Q_OBJECT
public:
    explicit SS1BBE_CjCsParamSetGUIDlg(QWidget *parent = 0);
    QSize sizeHint() const;
    ~SS1BBE_CjCsParamSetGUIDlg();
...
private:
    Ui::SS1BBE_CjCsParamSetGUIDlg *ui;

    QString m_strTitle;
    TLxTsspObjectInfo m_ObjectInfo;
    ILxTsspLogManager* m_pILxTsspLogManager;

    SMCU::MCSSSCjParamStruct4 m_MCSSSCjParam;       //参数结构体
    SMCU::MCSSSCjStatusStruct4* m_MCSSSCjStatus;       //状态参数结构体
    ...
    quint64 m_ParaMCSSSCjID;          //参数框架ID

    quint64 m_StatusParaMCSSSCjID;    //参数状态ID
    ...
    int m_totalTargetNum;
    int m_totalTargetNum_inParam;
public:
    //设置标题 传倒信息
...
private slots:
    ...
    void on_comboBox_DisAmbi_currentIndexChanged(int index);

    void on_comboBox_DisForWay_currentIndexChanged(int index);
    void on_lineEdit_DisFor_textChanged(const QString &arg1);
    void on_lineEdit_ResponsorZeroPos_textChanged(const QString &arg1);
};

#endif // SS1BBE_CJCSPARAMSETGUIDLG_H
