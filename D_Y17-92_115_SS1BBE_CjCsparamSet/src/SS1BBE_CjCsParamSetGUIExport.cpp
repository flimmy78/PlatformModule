...
extern "C" int GetConfigFormat(QString& strConfigFormat)
{
    ...
    strConfigFormat = QString(
            "<Module Name=\"libSS1BBE_CjparamSet.so\" StationID=\"2311\" SubSysID=\"04\" DevID=\"0401\" Note=\"%1\"> \n "
    ...
    for(int i=0; i<nodeList.length(); i++)
    {
        ...
        if(usObjID == 0x5152)
        {
            for(int k=iStart; k<iStop; k++)
            {
                ...
                g_ObjectList.insert(ui64ObjectID, new SS1BBE_CjCsParamSetGUI(ObjectInfo));
            }
        }
    }
