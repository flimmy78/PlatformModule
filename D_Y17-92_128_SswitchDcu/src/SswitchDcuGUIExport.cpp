...
extern "C" int GetModuleInfo(TLxTsspModuleInfo& moduleInfo)
{
    ...
    /// 配置标识，如：D_Y17-61_008
    strcpy(moduleInfo.szConfigID, "D_Y17-92_128\0");
...
extern "C" int GetConfigFormat(QString& strConfigFormat)
{
    ...
    strConfigFormat =
            QString("<Module Name=\"libSswitchDcu.so\" StationID=\"2311\" SubSysID=\"04\" DevID=\"0400\" Note=%1> \n "
            "<Object ID=\"6003\" Num=\"1\" IndexStart=\"0\" Alias =\" \" Note=%1>...</Object> \n "
    ...
    for(int i=0; i<nodeList.length(); i++)
    {
        ...
        if(usObjID == 0x6003)
        {
            for(int k=iStart; k<iStop; k++)
            {
                ...
                strcpy(ObjectInfo.szConfigID, "D_Y17-92_128");
                strcpy(ObjectInfo.szAlias, objAlias.toUtf8().data());
                strcpy(ObjectInfo.szComment, objNote.toUtf8().data());
                g_ObjectList.insert(ui64ObjectID, new SswitchDcuGUI(ObjectInfo));
            }
        }
    }
