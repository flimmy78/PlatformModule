...
bool CDDT::init(USHORT deviceID, PlatformResource* pRes)
{
    ...
    //DATA_SOURCE_ALLOC(SCBBETestOutStatusStruct, m_pSCBBETestOutStatu, "数传检测输出参数设置", deviceID, 0)
    DATA_SOURCE_ALLOC_PRO_CTRL(PCRDATATRANS_TTC, m_ddtCtlRsltRpt, "过程控制命令", deviceID, 0);
    return true;
}

void CDDT::set(SDDT &ddt)
{
    memcpy(m_pMCSSysStatus, &ddt.m_MCSSysStatus,sizeof(ddt.m_MCSSysStatus));
    memcpy(m_pMCSDDTRevStatus, &ddt.m_MCSDDTRevStatus,sizeof(ddt.m_MCSDDTRevStatus));
    memcpy(m_pMCSDDTTMQStatusI, &ddt.m_MCSDDTTMQStatusI,sizeof(ddt.m_MCSDDTTMQStatusI));
    memcpy(m_pMCSDDTTMQStatusQ, &ddt.m_MCSDDTTMQStatusQ,sizeof(ddt.m_MCSDDTTMQStatusQ));
    memcpy(m_pMCSDDTTMSimulStatus, &ddt.m_MCSDDTTMSimulStatus,sizeof(ddt.m_MCSDDTTMSimulStatus));
    memcpy(m_pMCSDivComStatu, &ddt.m_MCSDivComStatu,sizeof(ddt.m_MCSDivComStatu));
    //memcpy(m_pSCBBETestOutStatu, &ddt.m_SCBBETestOutStatu,sizeof(ddt.m_SCBBETestOutStatu));
    memcpy(m_ddtCtlRsltRpt, &ddt.m_ctlRsltRpt, sizeof(ddt.m_ctlRsltRpt));
}
...
void CDDT:: get(SDDT &ddt){
    memcpy(&ddt.m_MCSSysStatus, m_pMCSSysStatus,sizeof(ddt.m_MCSSysStatus));
    memcpy(&ddt.m_MCSDDTRevStatus,m_pMCSDDTRevStatus ,sizeof(ddt.m_MCSDDTRevStatus));
    memcpy(&ddt.m_MCSDDTTMQStatusI,m_pMCSDDTTMQStatusI ,sizeof(ddt.m_MCSDDTTMQStatusI));
    memcpy(&ddt.m_MCSDDTTMQStatusQ,m_pMCSDDTTMQStatusQ ,sizeof(ddt.m_MCSDDTTMQStatusQ));
    memcpy(&ddt.m_MCSDDTTMSimulStatus,m_pMCSDDTTMSimulStatus ,sizeof(ddt.m_MCSDDTTMSimulStatus));
    memcpy(&ddt.m_MCSDivComStatu, m_pMCSDivComStatu,sizeof(ddt.m_MCSDivComStatu));
    //memcpy(&ddt.m_SCBBETestOutStatu,m_pSCBBETestOutStatu ,sizeof(ddt.m_SCBBETestOutStatu));
    memcpy( &ddt.m_ctlRsltRpt,m_ddtCtlRsltRpt, sizeof(ddt.m_ctlRsltRpt));
}
