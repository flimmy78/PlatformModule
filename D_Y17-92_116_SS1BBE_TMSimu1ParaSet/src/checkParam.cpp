...
int SSBBE_TMSimu1ParaSetGUIDlg::msgBoxFunction(QMessageBox &msgBox, QString msg, BOOL ret)
{
    msgBox.setText(msg);
    msgBox.exec();
    return ret;
}
BOOL SSBBE_TMSimu1ParaSetGUIDlg::checkStringLen(const int lenInByte, QString& str)
{
    return str.size() <=  2*lenInByte && !str.isEmpty();
}
BOOL SSBBE_TMSimu1ParaSetGUIDlg::CheckParam()
{
    QMessageBox msgBox;
    QString strTmp;
...
//    int JiSuanFrmLen =0;
    ...
    {
        return msgBoxFunction(msgBox,"固定数设置输入为空，请重新输入!", FALSE);
    }
    //字长
    m_WordLen = ui->lineEdit_WordLen->text().toInt();
    if(ui->lineEdit_WordLen->text().isEmpty())
    {
        strTmp = QString("字长输入为空，请重新输入！");
        return msgBoxFunction(msgBox, strTmp, FALSE);
    }
    if(m_WordLen < WORDLEN_MIN || m_WordLen > WORDLEN_MAX)
    {
        strTmp = QString("字长范围是[%1～%2]，请重新输入！").arg(WORDLEN_MIN).arg(WORDLEN_MAX);
        return msgBoxFunction(msgBox, strTmp, FALSE);
    }
    //字长与固定数的关系
    ...
    if(m_DataType == 0)  //固定数
    {
        ...
        QString numStr = QString::number(num, 16).toUpper();
    ...
    int m_CodeRate = sCodeRate.toDouble();
    ...
    {
        return msgBoxFunction(msgBox, "帧同步码组长必须是字长的整数倍，请重新输入！", FALSE);
    }
    ...
    if(m_ByfrmType != 0)
    {
        ...
        {
            return msgBoxFunction(msgBox,QString("副帧长度输入范围应为[%1~%2]帧。")
                                  .arg(SUBFRMLEN_MIN).arg(SUBFRMLEN_MAX), FALSE);
        }
    }

    USHORT nIDPos1, nIDPos2;
    if ((m_EncodeStyle == 0)||(m_EncodeStyle == 1)) //没有RS编码时
    {
    ...
    else if(m_ByfrmType==2)
    {
        if ((m_EncodeStyle == 0)||(m_EncodeStyle == 1))
        {
            ...
            {
                return msgBoxFunction(msgBox,QString("副帧码组长度输入范围应为[%1~%2]bit.")
                                      .arg(SUBFPATTERNLEN_MIN).arg(SUBFPATTERNLEN_MAX), FALSE);
            }
