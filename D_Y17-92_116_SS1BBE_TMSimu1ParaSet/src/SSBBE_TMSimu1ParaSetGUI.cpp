...
#include "SSBBE_TMSimu1ParaSetGUIDlg.h"
...
namespace
{
SSBBE_TMSimu1ParaSetGUIDlg* g_form = NULL;
}
extern quint64 GetID(ushort usStationID, uchar ucSubSysID, ushort usDeviceID, ushort usType,
                     uchar ucSn);
SSBBE_TMSimu1ParaSetGUI::SSBBE_TMSimu1ParaSetGUI(TLxTsspObjectInfo ObjectInfo)
{
    //初始化
 ...
 * @return 界面对象的界面。
*/
QWidget* SSBBE_TMSimu1ParaSetGUI::ShowMe(QWidget* parent, const QString& strTitle)
{
    m_strTitle = strTitle;

    SSBBE_TMSimu1ParaSetGUIDlg* form;

    form = new SSBBE_TMSimu1ParaSetGUIDlg(parent);
 ...
 * @return szHelpID:长度。
*/
int SSBBE_TMSimu1ParaSetGUI::GetContextHelpID(char* szHelpID)
{
...
*/
int SSBBE_TMSimu1ParaSetGUI::GetModuleInfo(TLxTsspModuleInfo& moduleInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SSBBE_TMSimu1ParaSetGUI::GetObjectInfo(TLxTsspObjectInfo& ObjectInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SSBBE_TMSimu1ParaSetGUI::PreInitialize()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SSBBE_TMSimu1ParaSetGUI::Initialize()
{
    //日志信息
    TLxTsspLog tmpLog;
    QString strLog;
...
//遍历xml文件，获取参数取值范围
int SSBBE_TMSimu1ParaSetGUI::QueryParaBlock(QString ConfigPath)
{
...
int SSBBE_TMSimu1ParaSetGUI::GetAllMapValueStringByPara(int MapSum,QDomNode pInDocNode_Para)
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SSBBE_TMSimu1ParaSetGUI::Start()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SSBBE_TMSimu1ParaSetGUI::Stop()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
bool SSBBE_TMSimu1ParaSetGUI::ClearUp()
{
    return 1;
}
 ...
 *           其它错误依情况增加；
 */
int SSBBE_TMSimu1ParaSetGUI::GetErrorNumber()
{
    return m_iErrCode;
}
 ...
 */
int SSBBE_TMSimu1ParaSetGUI::GetErrorMsg(int iErrorNo, QString& strErrorMsg)
{
 ...
 */
bool SSBBE_TMSimu1ParaSetGUI::IsOnline()
{
    //对象初始化
    //查找对象管理器
    LxTsspObjectManager* pObjectManager = LxTsspObjectManager::Instance();
 ...
 */
void SSBBE_TMSimu1ParaSetGUI::SendMessage(TLxTsspMessage msg)
{
    return;
}
 ...
 * @return 1：成功；-1：失败。
 */
int SSBBE_TMSimu1ParaSetGUI::SetParameter(QMap<quint64, QByteArray> parameterList)
{
 ...
 */
int SSBBE_TMSimu1ParaSetGUI::SetParameter(quint64 ui64ParamID, QByteArray parameter)
{
 ...
 */
int SSBBE_TMSimu1ParaSetGUI::GetParameterID(QList<quint64>& pParameterIDList) const
{
 ...
 * @return 大于等于0：参数块个数。-1：失败。
 */
int SSBBE_TMSimu1ParaSetGUI::GetParameter(QMap<quint64, QByteArray>& parameterList) const
{
    SMCU::MCSSSTMSimulParamStruct4 tmpTMSiPara;       //码同步参数

    SMCU::MCSSSTMSimulStatusStruct4* tmpTMSiStatus = NULL;      //码同步参数
    ...
    DWORD stLen = sizeof(SMCU::MCSSSTMSimulParamStruct4);
    if(tmpStateMgr != NULL)
        tmpTMSiStatus  =
                (SMCU::MCSSSTMSimulStatusStruct4*)tmpStateMgr->FindOneItem(m_StatusTMSiID, stLen);

    if(tmpTMSiStatus != NULL)
    {
        for (int i = 0; i<MAX_TARGET_COUNT; i++)
            tmpTMSiPara.SSTMSimulParam[i] = tmpTMSiStatus->SSTMSimulStatu[i].tParams;
    }

    QByteArray  tmpCh;
    tmpCh.resize(sizeof(SMCU::MCSSSTMSimulParamStruct4));
    memcpy(tmpCh.data(), &tmpTMSiPara, sizeof(SMCU::MCSSSTMSimulParamStruct4));
 ...
 * @return 1：成功；-1：失败。
 */
int SSBBE_TMSimu1ParaSetGUI::GetParameter(quint64 ui64ParamID, QByteArray& parameter) const
{
    if(m_ParamList.contains(ui64ParamID))
    {
 ...
 */
int SSBBE_TMSimu1ParaSetGUI::GetState(QMap<quint64, TLxTsspDeviceState>& stateList) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int SSBBE_TMSimu1ParaSetGUI::GetState(quint64 ui64StatusID, TLxTsspDeviceState& status) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int SSBBE_TMSimu1ParaSetGUI::GetStateDirect(quint64 ui64StatusID, TLxTsspDeviceState& status)
{
    if(m_StateList.contains(ui64StatusID))
    {
        //
 ...
 */
int SSBBE_TMSimu1ParaSetGUI::WriteData(quint64 ui64DataID, uchar* aData, uint uiDataLen)
{
    return 0;
}
 ...
 */
int SSBBE_TMSimu1ParaSetGUI::ReadData(quint64 ui64DataID, uchar* aData, uint& uiDataLen,
                                      uint& uiReloadCount)
{
    return 0;
}

int SSBBE_TMSimu1ParaSetGUI::CreateDefaultMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    /*码同步参数*/
    SMCU::MCSSSTMSimulParamStruct4 aTMSimulParam;
    for(int nIndex = 0; nIndex < MAX_TARGET_COUNT; nIndex++)
    {
        ...
        //交织深度
        aTMSimulParam.SSTMSimulParam[nIndex].ucInterleav = m_ParaSetStruct[nIndex][9].InitVal.lval + 1;
        //卷积输出相位
...
QWidget* SSBBE_TMSimu1ParaSetGUI::ShowMeInEdit(QWidget* parent,uchar ucType,ILxTsspParameterMacro* pMacroObj)
{
    g_form = new SSBBE_TMSimu1ParaSetGUIDlg(parent);
...
int SSBBE_TMSimu1ParaSetGUI::SaveToMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    if(g_form!=NULL)
    {
