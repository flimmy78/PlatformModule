...
#include "ui_SSBBE_TMSimu1ParaSetGUIDlg.h"
#include <QCloseEvent>
#include <QMessageBox>
#include <QDateTime>
...
extern quint64 GetID(ushort usStationID, uchar ucSubSysID, ushort usDeviceID, ushort usType,
                     uchar ucSn);
SSBBE_TMSimu1ParaSetGUIDlg::SSBBE_TMSimu1ParaSetGUIDlg(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SSBBE_TMSimu1ParaSetGUIDlg)
{
    ui->setupUi(this);
    m_pStateMgr = NULL;
    ...
    connect(m_timer, SIGNAL(timeout()), this, SLOT(updateTarget()));
    setAttribute(Qt::WA_DeleteOnClose);
}

SSBBE_TMSimu1ParaSetGUIDlg::~SSBBE_TMSimu1ParaSetGUIDlg()
{
    killTimer(m_TimerID);
    m_timer->stop();
    delete m_timer;
...
void SSBBE_TMSimu1ParaSetGUIDlg::setText(const QString& strTitle, ParaStruct xmlsetText[4][60], QString MapMean[100][30], int MapNum[100], uchar Type, ILxTsspParameterMacro* pMacroObj)
{
    m_strTitle = strTitle;
...
//初始化对话框
void SSBBE_TMSimu1ParaSetGUIDlg::InitDLG(int Type, ILxTsspParameterMacro* pMacroObj)
{
    iParaSetFlag = Type;
    //设置输入格式
    InitInput();
    ...
    if(Type == 0)//取当前参数
    {
        ...
        if(m_pStateMgr != NULL)
        {
            DWORD stLen = sizeof(SMCU::MCSSSTMSimulStatusStruct4);
            m_MCSSSTMSimulStatus = (SMCU::MCSSSTMSimulStatusStruct4*)m_pStateMgr->FindOneItem(m_StatusParaMCSSSTMSimulID, stLen);
            if(m_MCSSSTMSimulStatus == NULL || stLen != sizeof(SMCU::MCSSSTMSimulStatusStruct4))
                return;
        }
        //启动定时器
    ...
    else if(Type == 1)//取宏里参数
    {
        ...
        if(pMacroObj!=NULL)
        {
            ...
            memcpy(&m_MCSSSTMSimulParam, byteArray.data(), sizeof(SMCU::MCSSSTMSimulParamStruct4));
        }
...
//设置输入格式
void SSBBE_TMSimu1ParaSetGUIDlg::InitInput()
{
    //正整数
...
void SSBBE_TMSimu1ParaSetGUIDlg::ShowControls()
{
    showTargetParam(0);
    //默认选择目标1
...
//显示参数
void SSBBE_TMSimu1ParaSetGUIDlg::ShowPara(int type)
{
    if(type == 0)       //参数设置
    {
        //码同步
        if(m_MCSSSTMSimulStatus != NULL)
        {
            ...
            {
                for (int i = 0; i < MAX_TARGET_COUNT; i++)
                    m_MCSSSTMSimulParam.SSTMSimulParam[i] = m_MCSSSTMSimulStatus->SSTMSimulStatu[i].tParams;
            }
        }
        else
        {
        ...
        {
            DWORD stLenTM = sizeof(SMCU::MCSSSTMStatusStruct4);
            if(m_pStateMgr != NULL)
            {
                m_MCSSSTMStatus = (SMCU::MCSSSTMStatusStruct4*)m_pStateMgr->FindOneItem(m_StatusParaMCSSSTMID, stLenTM);
            }
        }
    }
    else
...
//设置帧长范围
void SSBBE_TMSimu1ParaSetGUIDlg::SetFrameLenRange(int iWordLen)
{
    /* if(iWordLen ==8)
    {
...
void SSBBE_TMSimu1ParaSetGUIDlg::SetObjInfo(TLxTsspObjectInfo ObjectInfo)
{
    ...
    {
        SUBITEM& sItem = *(pItemTM->getSubItemByDeviceID_SN(m_ObjectInfo.usLocalDID, m_ObjectInfo.ucSN));
        /*************************框架**********************************/
...
//使能或不使能控件
void SSBBE_TMSimu1ParaSetGUIDlg::EnabledControls(bool bFlag)
{
    /*************************Group1*************************/
    //数据输出方式
...
//保存参数宏
int SSBBE_TMSimu1ParaSetGUIDlg::SaveToMacro(ILxTsspParameterMacro* pMacroObj)
{
    //判断合法性
    int iRet = CheckPara();
    if(iRet == -1)
...
//从界面取参数
void SSBBE_TMSimu1ParaSetGUIDlg::GetPara()
{
    getParamTarget();
}

//验证参数范围
int SSBBE_TMSimu1ParaSetGUIDlg::CheckPara()
{
    return CheckParam() ? 1 : -1;
}

//定时器
void SSBBE_TMSimu1ParaSetGUIDlg::timerEvent(QTimerEvent *event)
{
    ShowPara(0);
}

//更改参数按钮休息处理函数
void SSBBE_TMSimu1ParaSetGUIDlg::on_pushButton_Modify_clicked()
{
    if(ui->pushButton_Modify->text() == "更改")
    {
...
void SSBBE_TMSimu1ParaSetGUIDlg::on_pushButton_Set_clicked()
{
    //验证参数范围
    if(CheckPara() == 1)
    {
        ...
        if(m_pHAManager != NULL)
        {
            struct{
                ...
                struct{
                    USHORT tmp;
                    SMCU::MCSSSTMSimulParamStruct SSTMSimulParam;
...
void SSBBE_TMSimu1ParaSetGUIDlg::closeEvent(QCloseEvent *event)
{
    event->accept();
}

QSize SSBBE_TMSimu1ParaSetGUIDlg::sizeHint() const
{
    return QSize(this->width(), this->height());
}


void SSBBE_TMSimu1ParaSetGUIDlg::SubFrmEn(bool bFlag)
{
    //副帧长度
...
//有无ID副帧
void SSBBE_TMSimu1ParaSetGUIDlg::IDSubFrmEn(bool bFlag)
{
...
//有无循环副帧
void SSBBE_TMSimu1ParaSetGUIDlg::CycSubFrmEn(bool bFlag)
{
    //循环副帧
...
//有无反码副帧
void SSBBE_TMSimu1ParaSetGUIDlg::ReserveSubFrmEn(bool bFlag)
{

}

void SSBBE_TMSimu1ParaSetGUIDlg::SendMessage(char* pData, unsigned short  usLen, unsigned short usC, unsigned short usTID)
{
    static uint seq = 0;
...
DWORDLONG SSBBE_TMSimu1ParaSetGUIDlg::StrToSynCode(const char * pStr)
{
    char c,C;
    const char*  p;
...
void SSBBE_TMSimu1ParaSetGUIDlg::showTargetParam(int nIndex, int nType)
{
    if (nType == 0)
    {
        //向下拉框控件中写入从xml中读取的内容
        ...
        //加扰多项式长度SSBBE_TMSimu1ParaSetGUIDlg
...
void SSBBE_TMSimu1ParaSetGUIDlg::getParamTarget()
{
    bool ok;
    //帧同步码组位置
    ...
    //码速率
    m_MCSSSTMSimulParam.SSTMSimulParam[m_nTargetNum].uiTM_CodeRate = ui->lineEdit_CodeRate->text().toUInt();
    //固定数设置
    ...
    m_MCSSSTMSimulParam.SSTMSimulParam[m_nTargetNum].uiFrameSynG = StrToSynCode(pTem1);
    //加扰多项式
    ...
    m_MCSSSTMSimulParam.SSTMSimulParam[m_nTargetNum].uiCycFrmSynCode = StrToSynCode(pTem2);
    //ID计数位置
    m_MCSSSTMSimulParam.SSTMSimulParam[m_nTargetNum].usIDSubFrmPos = ui->lineEdit_IDCountPos->text().toInt();
}

void SSBBE_TMSimu1ParaSetGUIDlg::on_pushButton_autoMap_clicked()
{
    m_autoMap = true;
...
int SSBBE_TMSimu1ParaSetGUIDlg::getSharedMemoryTargetNum()
{
...
void SSBBE_TMSimu1ParaSetGUIDlg::showRadioButton()
{
...
void SSBBE_TMSimu1ParaSetGUIDlg::updateRadioButton(int num)
{
    if (m_targetNum_usedInMacro == num)
        return;
    m_targetNum_usedInMacro = num;
    m_nTargetNum = 0;
    set_radioButton_Target();
    ui->radioButton_Target1->hide();
    ui->radioButton_Target2->hide();
...
void SSBBE_TMSimu1ParaSetGUIDlg::set_radioButton_Target()
{
    switch(m_nTargetNum)
    {
    case 0:
...
void SSBBE_TMSimu1ParaSetGUIDlg::on_radioButton_Target1_clicked()
{
    if(1 != CheckPara())
    {
        set_radioButton_Target();
...
void SSBBE_TMSimu1ParaSetGUIDlg::on_radioButton_Target2_clicked()
{
    if(1 != CheckPara())
    {
        set_radioButton_Target();
...
void SSBBE_TMSimu1ParaSetGUIDlg::on_radioButton_Target3_clicked()
{
    if(1 != CheckPara())
    {
        set_radioButton_Target();
...
void SSBBE_TMSimu1ParaSetGUIDlg::on_radioButton_Target4_clicked()
{
    if(1 != CheckPara())
    {
        set_radioButton_Target();
...
//数据源
void SSBBE_TMSimu1ParaSetGUIDlg::on_comboBox_DataSource_currentIndexChanged(int index)
{
    if (ui->pushButton_Modify->text() == "更改")
        return;
    if(index==0){
...
void SSBBE_TMSimu1ParaSetGUIDlg::on_comboBox_DataSourceType_currentIndexChanged(int index)
{
    if (ui->pushButton_Modify->text() == "更改")
        return;
...
//编码方式
void SSBBE_TMSimu1ParaSetGUIDlg::on_comboBox_DecodeType_currentIndexChanged(int index)
{
    if (ui->pushButton_Modify->text() == "更改")
        return;
    if (index== 0){//关
...
//副帧同步方式
void SSBBE_TMSimu1ParaSetGUIDlg::on_comboBox_SubFrmSyncType_currentIndexChanged(int index)
{
    if (ui->pushButton_Modify->text() == "更改")
        return;
...
//卷及码类型
void SSBBE_TMSimu1ParaSetGUIDlg::on_comboBox_RecurrentCodeType_currentIndexChanged(int index)
{
    if (ui->pushButton_Modify->text() == "更改")
        return;
    // 1/2
...
//解扰控制
void SSBBE_TMSimu1ParaSetGUIDlg::on_comboBox_RandEn_currentIndexChanged(int index)
{
    if (ui->pushButton_Modify->text() == "更改")
        return;
...
void SSBBE_TMSimu1ParaSetGUIDlg::updateTarget()
{
    if(!m_pCommFunc)return;
    updateRadioButton(m_pCommFunc->getTargetNum() - 1);
}

void SSBBE_TMSimu1ParaSetGUIDlg::on_lineEdit_CodeRate_textChanged(const QString &arg1)
{
    CCommFunc::lineEditPrecision(arg1, ui->lineEdit_CodeRate, 3);
}

void SSBBE_TMSimu1ParaSetGUIDlg::on_lineEdit_WordLen_textChanged(const QString &arg1)
{
    int cNum;
