...
namespace
{
DDTBBE_RecSetGUIDlg* g_form = NULL;
}
extern quint64 GetID(ushort usStationID, uchar ucSubSysID, ushort usDeviceID, ushort usType,
                     uchar ucSn);
DDTBBE_RecSetGUI::DDTBBE_RecSetGUI(TLxTsspObjectInfo ObjectInfo)
{
    //初始化
 ...
 * @return 界面对象的界面。
*/
QWidget* DDTBBE_RecSetGUI::ShowMe(QWidget* parent, const QString& strTitle)
{
    m_strTitle = strTitle;

    DDTBBE_RecSetGUIDlg* form;

    form = new DDTBBE_RecSetGUIDlg(parent);
 ...
 * @return szHelpID:长度。
*/
int DDTBBE_RecSetGUI::GetContextHelpID(char* szHelpID)
{
...
*/
int DDTBBE_RecSetGUI::GetModuleInfo(TLxTsspModuleInfo& moduleInfo) const
{
    ...
    /// 配置标识，如：D_Y17-61_008
    strcpy(moduleInfo.szConfigID, "D_Y17-92_202\0");
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int DDTBBE_RecSetGUI::GetObjectInfo(TLxTsspObjectInfo& ObjectInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int DDTBBE_RecSetGUI::PreInitialize()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int DDTBBE_RecSetGUI::Initialize()
{
    //日志信息
    TLxTsspLog tmpLog;
    QString strLog;
    ...
    stCurPath = stCurPath + "/System/ModuleConfig/DDTBBE_RecSetGUI.xml";
    //遍历xlm文件，获取参数取值范围
    m_MapSumFlag = 0;
...
//遍历xml文件，获取参数取值范围
int DDTBBE_RecSetGUI::QueryParaBlock(QString ConfigPath)
{
...
int DDTBBE_RecSetGUI::GetAllMapValueStringByPara(int MapSum,QDomNode pInDocNode_Para)
{
 ...
 *           其它错误依情况增加；
 */
int DDTBBE_RecSetGUI::GetErrorNumber()
{
    return m_iErrCode;
}
 ...
 */
int DDTBBE_RecSetGUI::GetErrorMsg(int iErrorNo, QString& strErrorMsg)
{
 ...
 */
bool DDTBBE_RecSetGUI::IsOnline()
{
    //对象初始化
    //查找对象管理器
    LxTsspObjectManager* pObjectManager = LxTsspObjectManager::Instance();
 ...
 */
void DDTBBE_RecSetGUI::SendMessage(TLxTsspMessage msg)
{
    return;
}
 ...
 * @return 1：成功；-1：失败。
 */
int DDTBBE_RecSetGUI::SetParameter(QMap<quint64, QByteArray> parameterList)
{
 ...
 */
int DDTBBE_RecSetGUI::SetParameter(quint64 ui64ParamID, QByteArray parameter)
{
 ...
 */
int DDTBBE_RecSetGUI::GetParameterID(QList<quint64>& pParameterIDList) const
{
 ...
 * @return 大于等于0：参数块个数。-1：失败。
 */
int DDTBBE_RecSetGUI::GetParameter(QMap<quint64, QByteArray>& parameterList) const
{
    MCSDDTRevParamStruct tmpCodeSynPara;       //码同步参数

    MCSDDTRevStatusStruct* tmpCodeSynStatus = NULL;      //码同步参数
    ...
    if(tmpStateMgr != NULL)
        tmpCodeSynStatus  =
                (MCSDDTRevStatusStruct*)tmpStateMgr->FindOneItem(tmpStatusID, stLen);
 ...
 * @return 1：成功；-1：失败。
 */
int DDTBBE_RecSetGUI::GetParameter(quint64 ui64ParamID, QByteArray& parameter) const
{
    if(m_ParamList.contains(ui64ParamID))
    {
 ...
 */
int DDTBBE_RecSetGUI::GetState(QMap<quint64, TLxTsspDeviceState>& stateList) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int DDTBBE_RecSetGUI::GetState(quint64 ui64StatusID, TLxTsspDeviceState& status) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int DDTBBE_RecSetGUI::GetStateDirect(quint64 ui64StatusID, TLxTsspDeviceState& status)
{
    if(m_StateList.contains(ui64StatusID))
    {
        //
 ...
 */
int DDTBBE_RecSetGUI::WriteData(quint64 ui64DataID, uchar* aData, uint uiDataLen)
{
    return 0;
}
 ...
 */
int DDTBBE_RecSetGUI::ReadData(quint64 ui64DataID, uchar* aData, uint& uiDataLen,
                               uint& uiReloadCount)
{
    return 0;
}

int DDTBBE_RecSetGUI::CreateDefaultMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    /*码同步参数*/
    MCSDDTRevParamStruct aMCSDDTRevParam;
    //载波环路带宽
    aMCSDDTRevParam.ucPllBw = m_ParaSetStruct[0][2].InitVal.lval + 1;
    //AGC时间常数
    aMCSDDTRevParam.ucAgcTime = m_ParaSetStruct[0][1].InitVal.lval + 2;
    //调制体制
    aMCSDDTRevParam.ucModesel = m_ParaSetStruct[0][3].InitVal.lval + 1;
    //I路功率因子
    aMCSDDTRevParam.ucIPowRate = m_ParaSetStruct[0][6].InitVal.lval;
    //Q路功率因子
    aMCSDDTRevParam.ucQPowRate = m_ParaSetStruct[0][7].InitVal.lval;
    //功率比有效标志
    aMCSDDTRevParam.ucPowerType = m_ParaSetStruct[0][9].InitVal.lval + 1;
    //IO功率比
    aMCSDDTRevParam.cRatiodB = m_ParaSetStruct[0][8].InitVal.lval;
    //解调数据输出选择
    aMCSDDTRevParam.ucSignalSel = m_ParaSetStruct[0][5].InitVal.lval + 1;
    //GRAY编码选择
    aMCSDDTRevParam.ucGrayType = m_ParaSetStruct[0][10].InitVal.lval + 1;
    //取当前设置的参数，存入框架
    QByteArray byteArray;
    byteArray.resize(sizeof(aMCSDDTRevParam));
    memcpy(byteArray.data(),&aMCSDDTRevParam,sizeof(aMCSDDTRevParam));
    if(pMacroObj!=NULL)
    {
...
QWidget* DDTBBE_RecSetGUI::ShowMeInEdit(QWidget* parent,uchar ucType,ILxTsspParameterMacro* pMacroObj)
{
    g_form = new DDTBBE_RecSetGUIDlg(parent);
...
int DDTBBE_RecSetGUI::SaveToMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    if(g_form!=NULL)
    {
