 ...
 * @return 界面对象的界面。
*/
QWidget* RCLOCKDcuGUI::ShowMe(QWidget* parent, const QString& strTitle)
{
    m_strTitle = strTitle;

    RCLOCKDcuGUIDlg* form;
    form = new RCLOCKDcuGUIDlg(parent);
 ...
 * @return szHelpID:长度。
*/
int RCLOCKDcuGUI::GetContextHelpID(char* szHelpID)
{
...
*/
int RCLOCKDcuGUI::GetModuleInfo(TLxTsspModuleInfo& moduleInfo) const
{
    ...
    /// 配置标识，如：D_Y17-61_008
    strcpy(moduleInfo.szConfigID, "D_Y17-92_133\0");
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int RCLOCKDcuGUI::GetObjectInfo(TLxTsspObjectInfo& ObjectInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int RCLOCKDcuGUI::PreInitialize()
{
    return 1;
}
...
int RCLOCKDcuGUI::Initialize()
{
    ...
    stCurPath = stCurPath + "/System/ModuleConfig/RCLOCKDcuGUI.xml";
    //遍历xlm文件，获取参数取值范围
    m_MapSumFlag = 0;
...
//遍历xlm文件，获取参数取值范围
int RCLOCKDcuGUI::QueryParaBlock(QString ConfigPath)
{
...
int RCLOCKDcuGUI::GetAllMapValueStringByPara(int MapSum,QDomNode pInDocNode_Para)
{
 ...
 *           其它错误依情况增加；
 */
int RCLOCKDcuGUI::GetErrorNumber()
{
    return m_iErrCode;
}
 ...
 */
int RCLOCKDcuGUI::GetErrorMsg(int iErrorNo, QString& strErrorMsg)
{
 ...
 */
void RCLOCKDcuGUI::SendMessage(TLxTsspMessage msg)
{
    return;
}
 ...
 * @return 1：成功；-1：失败。
 */
int RCLOCKDcuGUI::SetParameter(QMap<quint64, QByteArray> parameterList)
{
 ...
 */
int RCLOCKDcuGUI::SetParameter(quint64 ui64ParamID, QByteArray parameter)
{
 ...
 */
int RCLOCKDcuGUI::GetParameterID(QList<quint64>& pParameterIDList) const
{
 ...
 * @return 大于等于0：参数块个数。-1：失败。
 */
int RCLOCKDcuGUI::GetParameter(QMap<quint64, QByteArray>& parameterList) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int RCLOCKDcuGUI::GetParameter(quint64 ui64ParamID, QByteArray& parameter) const
{
    if(m_ParamList.contains(ui64ParamID))
    {
 ...
 */
int RCLOCKDcuGUI::GetState(QMap<quint64, TLxTsspDeviceState>& stateList) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int RCLOCKDcuGUI::GetState(quint64 ui64StatusID, TLxTsspDeviceState& status) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int RCLOCKDcuGUI::GetStateDirect(quint64 ui64StatusID, TLxTsspDeviceState& status)
{
    if(m_StateList.contains(ui64StatusID))
    {
        //
 ...
 */
int RCLOCKDcuGUI::WriteData(quint64 ui64DataID, uchar* aData, uint uiDataLen)
{
    return 0;
}
 ...
 */
int RCLOCKDcuGUI::ReadData(quint64 ui64DataID, uchar* aData, uint& uiDataLen,
                             uint& uiReloadCount)
{
    return 0;
}

int RCLOCKDcuGUI::CreateDefaultMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    RCLOCKDCUStatu tmpComPara;          //参数
    //时间区域
    tmpComPara.ucTimeZone = m_ParaSetStruct[0][0].InitVal.ival;
    //时间信息
    /*QString strDateTime = QString("%1-%2-%3 %4:%5:%6").arg(tmpComPara.ucYear)
            .arg(tmpComPara.ucMonth).arg(tmpComPara.ucDay).arg(tmpComPara.ucHour)
            .arg(tmpComPara.ucMin).arg(tmpComPara.ucSec);*/
    //参考有效状态
...
QWidget* RCLOCKDcuGUI::ShowMeInEdit(QWidget* parent,uchar ucType,ILxTsspParameterMacro* pMacroObj)
{
    g_form = new RCLOCKDcuGUIDlg(parent);
...
int RCLOCKDcuGUI::SaveToMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    if(g_form != NULL)
    {
