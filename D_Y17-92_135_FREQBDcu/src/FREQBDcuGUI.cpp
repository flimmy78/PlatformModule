 ...
 * @return 界面对象的界面。
*/
QWidget* FREQBDcuGUI::ShowMe(QWidget* parent, const QString& strTitle)
{
    m_strTitle = strTitle;

    FREQBDcuGUIDlg* form;

    form = new FREQBDcuGUIDlg(parent);
 ...
 * @return szHelpID:长度。
*/
int FREQBDcuGUI::GetContextHelpID(char* szHelpID)
{
...
*/
int FREQBDcuGUI::GetModuleInfo(TLxTsspModuleInfo& moduleInfo) const
{
    ...
    /// 配置标识，如：D_Y17-61_008
    strcpy(moduleInfo.szConfigID, "D_Y17-92_135\0");
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int FREQBDcuGUI::GetObjectInfo(TLxTsspObjectInfo& ObjectInfo) const
{
...
//遍历xml文件，获取参数取值范围
int FREQBDcuGUI::QueryParaBlock(QString ConfigPath)
{
...
int FREQBDcuGUI::GetAllMapValueStringByPara(int MapSum,QDomNode pInDocNode_Para)
{
 ...
 *           其它错误依情况增加；
 */
int FREQBDcuGUI::GetErrorNumber()
{
    return m_iErrCode;
}
 ...
 */
int FREQBDcuGUI::GetErrorMsg(int iErrorNo, QString& strErrorMsg)
{
 ...
 */
void FREQBDcuGUI::SendMessage(TLxTsspMessage msg)
{
    return;
}
 ...
 * @return 1：成功；-1：失败。
 */
int FREQBDcuGUI::SetParameter(QMap<quint64, QByteArray> parameterList)
{
 ...
 */
int FREQBDcuGUI::SetParameter(quint64 ui64ParamID, QByteArray parameter)
{
 ...
 */
int FREQBDcuGUI::GetParameterID(QList<quint64>& pParameterIDList) const
{
 ...
 * @return 大于等于0：参数块个数。-1：失败。
 */
int FREQBDcuGUI::GetParameter(QMap<quint64, QByteArray>& parameterList) const
{
    FREQBDCUPara tmpComPara;          //参数

    FREQBDCUStatu *tmpComStatus = NULL;  //状态
    ...
    if (tmpStateMgr != NULL)
        tmpComStatus  = (FREQBDCUStatu*)tmpStateMgr->FindOneItem(m_StatusComSynID, stLen);
    ...
    QByteArray tmpCh;
    tmpCh.resize(sizeof(FREQBDCUStatu));
    memcpy(tmpCh.data(), &tmpComPara, sizeof(FREQBDCUStatu));
 ...
 * @return 1：成功；-1：失败。
 */
int FREQBDcuGUI::GetParameter(quint64 ui64ParamID, QByteArray& parameter) const
{
    if(m_ParamList.contains(ui64ParamID))
    {
 ...
 */
int FREQBDcuGUI::GetState(QMap<quint64, TLxTsspDeviceState>& stateList) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int FREQBDcuGUI::GetState(quint64 ui64StatusID, TLxTsspDeviceState& status) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int FREQBDcuGUI::GetStateDirect(quint64 ui64StatusID, TLxTsspDeviceState& status)
{
    if(m_StateList.contains(ui64StatusID))
    {
        //
 ...
 */
int FREQBDcuGUI::WriteData(quint64 ui64DataID, uchar* aData, uint uiDataLen)
{
    return 0;
}
 ...
 */
int FREQBDcuGUI::ReadData(quint64 ui64DataID, uchar* aData, uint& uiDataLen,
                             uint& uiReloadCount)
{
    return 0;
}

int FREQBDcuGUI::CreateDefaultMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    FREQBDCUPara comPara;

    //取当前设置的参数，存入框架
    QByteArray byteArray;
    byteArray.resize(sizeof(FREQBDCUPara));
    memcpy(byteArray.data(), &comPara, sizeof(FREQBDCUPara));
    if(pMacroObj != NULL)
    {
...
QWidget* FREQBDcuGUI::ShowMeInEdit(QWidget* parent,uchar ucType,ILxTsspParameterMacro* pMacroObj)
{
    g_form = new FREQBDcuGUIDlg(parent);
...
int FREQBDcuGUI::SaveToMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    if(g_form != NULL)
    {
