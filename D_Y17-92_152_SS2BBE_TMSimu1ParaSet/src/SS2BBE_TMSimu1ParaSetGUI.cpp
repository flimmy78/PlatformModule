...
namespace
{
SS2BBE_TMSimu1ParaSetGUIDlg* g_form = NULL;
}
extern quint64 GetID(ushort usStationID, uchar ucSubSysID, ushort usDeviceID, ushort usType,
                     uchar ucSn);
SS2BBE_TMSimu1ParaSetGUI::SS2BBE_TMSimu1ParaSetGUI(TLxTsspObjectInfo ObjectInfo)
{
    //初始化
 ...
 * @return 界面对象的界面。
*/
QWidget* SS2BBE_TMSimu1ParaSetGUI::ShowMe(QWidget* parent, const QString& strTitle)
{
    m_strTitle = strTitle;

    SS2BBE_TMSimu1ParaSetGUIDlg* form;

    form = new SS2BBE_TMSimu1ParaSetGUIDlg(parent);
 ...
 * @return szHelpID:长度。
*/
int SS2BBE_TMSimu1ParaSetGUI::GetContextHelpID(char* szHelpID)
{
...
*/
int SS2BBE_TMSimu1ParaSetGUI::GetModuleInfo(TLxTsspModuleInfo& moduleInfo) const
{
    ...
    /// 配置标识，如：D_Y17-61_008
    strcpy(moduleInfo.szConfigID, "D_Y17-92_116\0");
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS2BBE_TMSimu1ParaSetGUI::GetObjectInfo(TLxTsspObjectInfo& ObjectInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS2BBE_TMSimu1ParaSetGUI::PreInitialize()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS2BBE_TMSimu1ParaSetGUI::Initialize()
{
    //日志信息
    TLxTsspLog tmpLog;
    QString strLog;
    ...
    stCurPath = stCurPath + "/System/ModuleConfig/SSBBE_TMSimu1ParaSetGUI.xml";
    //遍历xml文件，获取参数取值范围
    m_MapSumFlag = 0;
    ...
    if (pItem->vSubItem.size() > 0)
    {
        ...
        /*************************框架**********************************/
        m_ParaTMSiID = GetID(sItem.usStationID, sItem.ucSubSysID, sItem.usDeviceID,
                             sItem.usTID, sItem.ucSN);
        /*************************框架**********************************/

        /*************************状态**********************************/
        m_StatusTMSiID = GetID(sItem.usStationID, sItem.ucSubSysID, sItem.usDeviceID,
    ...
    strLog = QString("遥测解调参数设置: 读取配置文件初始化调制参数成功！").arg(MODULENAME);
...
//遍历xml文件，获取参数取值范围
int SS2BBE_TMSimu1ParaSetGUI::QueryParaBlock(QString ConfigPath)
{
...
int SS2BBE_TMSimu1ParaSetGUI::GetAllMapValueStringByPara(int MapSum,QDomNode pInDocNode_Para)
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS2BBE_TMSimu1ParaSetGUI::Start()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS2BBE_TMSimu1ParaSetGUI::Stop()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
bool SS2BBE_TMSimu1ParaSetGUI::ClearUp()
{
    return 1;
}
 ...
 *           其它错误依情况增加；
 */
int SS2BBE_TMSimu1ParaSetGUI::GetErrorNumber()
{
    return m_iErrCode;
}
 ...
 */
int SS2BBE_TMSimu1ParaSetGUI::GetErrorMsg(int iErrorNo, QString& strErrorMsg)
{
 ...
 */
bool SS2BBE_TMSimu1ParaSetGUI::IsOnline()
{
    //对象初始化
    //查找对象管理器
    LxTsspObjectManager* pObjectManager = LxTsspObjectManager::Instance();
 ...
 */
void SS2BBE_TMSimu1ParaSetGUI::SendMessage(TLxTsspMessage msg)
{
    return;
}
 ...
 * @return 1：成功；-1：失败。
 */
int SS2BBE_TMSimu1ParaSetGUI::SetParameter(QMap<quint64, QByteArray> parameterList)
{
 ...
 */
int SS2BBE_TMSimu1ParaSetGUI::SetParameter(quint64 ui64ParamID, QByteArray parameter)
{
 ...
 */
int SS2BBE_TMSimu1ParaSetGUI::GetParameterID(QList<quint64>& pParameterIDList) const
{
 ...
 * @return 大于等于0：参数块个数。-1：失败。
 */
int SS2BBE_TMSimu1ParaSetGUI::GetParameter(QMap<quint64, QByteArray>& parameterList) const
{
    SMCU2::MCSSSB2TMSimulParamStruct4 tmpTMSiPara;       //码同步参数

    SMCU2::MCSSSB2TMSimulStatusStruct4* tmpTMSiStatus = NULL;      //码同步参数
    ...
    DWORD stLen = sizeof(SMCU2::MCSSSB2TMSimulParamStruct4);
    if(tmpStateMgr != NULL)
        tmpTMSiStatus  =
                (SMCU2::MCSSSB2TMSimulStatusStruct4*)tmpStateMgr->FindOneItem(m_StatusTMSiID, stLen);

    if(tmpTMSiStatus != NULL)
    {
        for (int i = 0; i<MAX_TARGET_COUNT; i++)
            tmpTMSiPara.SSTMSimulParam[i] = tmpTMSiStatus->MCSSSB2TMSimulStatu[i].tParams;
    }

    QByteArray  tmpCh;
    tmpCh.resize(sizeof(SMCU2::MCSSSB2TMSimulParamStruct4));
    memcpy(tmpCh.data(), &tmpTMSiPara, sizeof(SMCU2::MCSSSB2TMSimulParamStruct4));
    parameterList.insert(m_ParaTMSiID, tmpCh);
 ...
 * @return 1：成功；-1：失败。
 */
int SS2BBE_TMSimu1ParaSetGUI::GetParameter(quint64 ui64ParamID, QByteArray& parameter) const
{
    if(m_ParamList.contains(ui64ParamID))
    {
 ...
 */
int SS2BBE_TMSimu1ParaSetGUI::GetState(QMap<quint64, TLxTsspDeviceState>& stateList) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int SS2BBE_TMSimu1ParaSetGUI::GetState(quint64 ui64StatusID, TLxTsspDeviceState& status) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int SS2BBE_TMSimu1ParaSetGUI::GetStateDirect(quint64 ui64StatusID, TLxTsspDeviceState& status)
{
    if(m_StateList.contains(ui64StatusID))
    {
        //
 ...
 */
int SS2BBE_TMSimu1ParaSetGUI::WriteData(quint64 ui64DataID, uchar* aData, uint uiDataLen)
{
    return 0;
}
 ...
 */
int SS2BBE_TMSimu1ParaSetGUI::ReadData(quint64 ui64DataID, uchar* aData, uint& uiDataLen,
                                      uint& uiReloadCount)
{
    return 0;
}

int SS2BBE_TMSimu1ParaSetGUI::CreateDefaultMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    /*码同步参数*/
    SMCU2::MCSSSB2TMSimulParamStruct4 aTMSimulParam;
    for(int nIndex = 0; nIndex < MAX_TARGET_COUNT; nIndex++)
    {
        //帧同步码组位置
        aTMSimulParam.SSTMSimulParam[nIndex].ucFspPlace = m_ParaSetStruct[nIndex][0].InitVal.lval + 1;
        //码型
        aTMSimulParam.SSTMSimulParam[nIndex].uiCodeStyle = m_ParaSetStruct[nIndex][2].InitVal.lval + 1;
        //编码方式
        aTMSimulParam.SSTMSimulParam[nIndex].ucCodingMd = m_ParaSetStruct[nIndex][4].InitVal.lval + 1;
        //RS码类型
        aTMSimulParam.SSTMSimulParam[nIndex].ucRSMd = m_ParaSetStruct[nIndex][5].InitVal.lval + 1;
        //RS编码坐标基
        aTMSimulParam.SSTMSimulParam[nIndex].ucBasiSel = m_ParaSetStruct[nIndex][6].InitVal.lval + 1;
        //数据源选择
        aTMSimulParam.SSTMSimulParam[nIndex].ucDataCheck = m_ParaSetStruct[nIndex][7].InitVal.lval + 1;
        //交织深度
        aTMSimulParam.SSTMSimulParam[nIndex].ucInterleav = m_ParaSetStruct[nIndex][9].InitVal.lval ;
        //卷积输出相位
        aTMSimulParam.SSTMSimulParam[nIndex].ucG2Inv = m_ParaSetStruct[nIndex][10].InitVal.lval + 1;
        //卷积码类型
        aTMSimulParam.SSTMSimulParam[nIndex].ucViterbiMd = m_ParaSetStruct[nIndex][11].InitVal.lval + 1;
        //数据源类型
        aTMSimulParam.SSTMSimulParam[nIndex].ucdatasel = m_ParaSetStruct[nIndex][12].InitVal.lval + 1;
        //帧同步码组长
        aTMSimulParam.SSTMSimulParam[nIndex].ucFrameSynGLen = m_ParaSetStruct[nIndex][16].InitVal.lval + 1;
        //加扰控制
        aTMSimulParam.SSTMSimulParam[nIndex].ucturbCtrl = m_ParaSetStruct[nIndex][18].InitVal.lval + 1;
        //加扰多项式长度
        aTMSimulParam.SSTMSimulParam[nIndex].ucRandLen = m_ParaSetStruct[nIndex][20].InitVal.lval + 1;
        //副帧码组长度
        aTMSimulParam.SSTMSimulParam[nIndex].ucCycFrmSynCodeLen = m_ParaSetStruct[nIndex][22].InitVal.lval + 1;
        //副帧同步方式
        aTMSimulParam.SSTMSimulParam[nIndex].ucSFrameSynMd = m_ParaSetStruct[nIndex][23].InitVal.lval + 1;
        //ID字基值
        aTMSimulParam.SSTMSimulParam[nIndex].ucIDBase = m_ParaSetStruct[nIndex][26].InitVal.lval + 1;
        //ID计数方向
        aTMSimulParam.SSTMSimulParam[nIndex].ucIDDirect = m_ParaSetStruct[nIndex][27].InitVal.lval + 1;

        //码速率
        aTMSimulParam.SSTMSimulParam[nIndex].uiTM_CodeRate = m_ParaSetStruct[nIndex][1].InitVal.lval;
        //固定数设置
        aTMSimulParam.SSTMSimulParam[nIndex].uidata = m_ParaSetStruct[nIndex][3].InitVal.lval;
        //帧长
        aTMSimulParam.SSTMSimulParam[nIndex].usFrameLen = m_ParaSetStruct[nIndex][13].InitVal.lval;
        //字长
        aTMSimulParam.SSTMSimulParam[nIndex].ucWordLen = m_ParaSetStruct[nIndex][14].InitVal.lval;
        //格式长
        aTMSimulParam.SSTMSimulParam[nIndex].ucSframeLen = m_ParaSetStruct[nIndex][15].InitVal.lval;
        //帧同步码组
        aTMSimulParam.SSTMSimulParam[nIndex].uiFrameSynG = m_ParaSetStruct[nIndex][17].InitVal.lval;
        //加扰多项式
        aTMSimulParam.SSTMSimulParam[nIndex].uiRandPoly = m_ParaSetStruct[nIndex][19].InitVal.lval;
        //加扰初相
        aTMSimulParam.SSTMSimulParam[nIndex].uiPhase = m_ParaSetStruct[nIndex][21].InitVal.lval;
        //副帧码组位置
        aTMSimulParam.SSTMSimulParam[nIndex].usCycFrmSynCodePos = m_ParaSetStruct[nIndex][24].InitVal.lval;
        //副帧同步码组
        aTMSimulParam.SSTMSimulParam[nIndex].uiCycFrmSynCode = m_ParaSetStruct[nIndex][25].InitVal.lval;
        //ID计数位置
        aTMSimulParam.SSTMSimulParam[nIndex].usIDSubFrmPos = m_ParaSetStruct[nIndex][28].InitVal.lval;
    }

    //取当前设置的参数，存入框架
    QByteArray byteArray;
    byteArray.resize(sizeof(aTMSimulParam));
    memcpy(byteArray.data(),&aTMSimulParam,sizeof(aTMSimulParam));
    if(pMacroObj!=NULL)
    {
        pMacroObj->UpdateParameterBlock(m_ParaTMSiID,byteArray);
    }

    return 1;
}

QWidget* SS2BBE_TMSimu1ParaSetGUI::ShowMeInEdit(QWidget* parent,uchar ucType,ILxTsspParameterMacro* pMacroObj)
{
    g_form = new SS2BBE_TMSimu1ParaSetGUIDlg(parent);
...
int SS2BBE_TMSimu1ParaSetGUI::SaveToMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    if(g_form!=NULL)
    {
