...
extern "C" int GetConfigFormat(QString& strConfigFormat)
{
    ...
    strConfigFormat =
            "<Module Name=\"libSK_BBEDivCombaraSetGUI.so\" StationID=\"2311\" SubSysID=\"04\" DevID=\"0401\" Note=\"分集合成参数设置\"> \n \
            <Object ID=\"5132\" Num=\"1\" IndexStart=\"0\" Alias =\" \" Note=\"参数设置\">...</Object> \n \
            ... \n \
            </Module>";
    ...
    for(int i=0; i<nodeList.length(); i++)
    {
        ...
        if(usObjID == 0x5132)
        {
            for(int k=iStart; k<iStop; k++)
            {
                ...
                strcpy(ObjectInfo.szConfigID, "D_Y17-92_204");
                strcpy(ObjectInfo.szAlias, objAlias.toUtf8().data());
                strcpy(ObjectInfo.szComment, objNote.toUtf8().data());
                g_ObjectList.insert(ui64ObjectID, new SK_BBEDivCombaraSetGUI(ObjectInfo));
            }
        }
    }
