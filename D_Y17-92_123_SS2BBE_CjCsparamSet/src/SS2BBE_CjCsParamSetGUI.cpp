#include "SS2BBE_CjCsParamSetGUI.h"
...
namespace
{
    SS2BBE_CjCsParamSetGUIDlg* g_form = NULL;
}
extern quint64 GetID(ushort usStationID, uchar ucSubSysID, ushort usDeviceID, ushort usType,
              uchar ucSn);
SS2BBE_CjCsParamSetGUI::SS2BBE_CjCsParamSetGUI(TLxTsspObjectInfo ObjectInfo)
{
    //初始化
 ...
 * @return 界面对象的界面。
*/
QWidget* SS2BBE_CjCsParamSetGUI::ShowMe(QWidget* parent, const QString& strTitle)
{
    m_strTitle = strTitle;

    SS2BBE_CjCsParamSetGUIDlg* form;

    form = new SS2BBE_CjCsParamSetGUIDlg(parent);
 ...
 * @return szHelpID:长度。
*/
int SS2BBE_CjCsParamSetGUI::GetContextHelpID(char* szHelpID)
{
...
*/
int SS2BBE_CjCsParamSetGUI::GetModuleInfo(TLxTsspModuleInfo& moduleInfo) const
{
    ...
    /// 配置标识，如：D_Y17-61_008
    strcpy(moduleInfo.szConfigID, "D_Y17-92_115\0");
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS2BBE_CjCsParamSetGUI::GetObjectInfo(TLxTsspObjectInfo& ObjectInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS2BBE_CjCsParamSetGUI::PreInitialize()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS2BBE_CjCsParamSetGUI::Initialize()
{
    //日志信息
    TLxTsspLog tmpLog;
    QString strLog;
    ...
    stCurPath = stCurPath + "/System/ModuleConfig/SS2BBE_CjCsParamSetGUI.xml";
    //遍历xml文件，获取参数取值范围
    m_MapSumFlag = 0;
...
//遍历xml文件，获取参数取值范围
int SS2BBE_CjCsParamSetGUI::QueryParaBlock(QString ConfigPath)
{
...
int SS2BBE_CjCsParamSetGUI::GetAllMapValueStringByPara(int MapSum,QDomNode pInDocNode_Para)
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS2BBE_CjCsParamSetGUI::Start()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS2BBE_CjCsParamSetGUI::Stop()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
bool SS2BBE_CjCsParamSetGUI::ClearUp()
{
    return 1;
}
 ...
 *           其它错误依情况增加；
 */
int SS2BBE_CjCsParamSetGUI::GetErrorNumber()
{
    return m_iErrCode;
}
 ...
 */
int SS2BBE_CjCsParamSetGUI::GetErrorMsg(int iErrorNo, QString& strErrorMsg)
{
 ...
 */
bool SS2BBE_CjCsParamSetGUI::IsOnline()
{
    //对象初始化
    //查找对象管理器
    LxTsspObjectManager* pObjectManager = LxTsspObjectManager::Instance();
 ...
 */
void SS2BBE_CjCsParamSetGUI::SendMessage(TLxTsspMessage msg)
{
    return;
}
 ...
 * @return 1：成功；-1：失败。
 */
int SS2BBE_CjCsParamSetGUI::SetParameter(QMap<quint64, QByteArray> parameterList)
{
 ...
 */
int SS2BBE_CjCsParamSetGUI::SetParameter(quint64 ui64ParamID, QByteArray parameter)
{
 ...
 */
int SS2BBE_CjCsParamSetGUI::GetParameterID(QList<quint64>& pParameterIDList) const
{
 ...
 * @return 大于等于0：参数块个数。-1：失败。
 */
int SS2BBE_CjCsParamSetGUI::GetParameter(QMap<quint64, QByteArray>& parameterList) const
{
    SMCU2::MCSSSB2CjParamStruct4 tmpComPara;          //参数

    SMCU2::MCSSSB2CjStatusStruct4 *tmpComStatus = NULL;  //状态参数
    ...
    if(tmpStateMgr != NULL)
        tmpComStatus  =
                (SMCU2::MCSSSB2CjStatusStruct4*)tmpStateMgr->FindOneItem(m_StatusComSynID, stLen);

    if(tmpComStatus != NULL)
    {
        for (int i = 0; i< MAX_TARGET_COUNT; i++)
            tmpComPara.cjParam[i] = tmpComStatus->cjStatus[i].tParams;
    }

    QByteArray  tmpCh;
    tmpCh.resize(sizeof(SMCU2::MCSSSB2CjParamStruct4));
    memcpy(tmpCh.data(), &tmpComPara, sizeof(SMCU2::MCSSSB2CjParamStruct4));
 ...
 * @return 1：成功；-1：失败。
 */
int SS2BBE_CjCsParamSetGUI::GetParameter(quint64 ui64ParamID, QByteArray& parameter) const
{
    if(m_ParamList.contains(ui64ParamID))
    {
 ...
 */
int SS2BBE_CjCsParamSetGUI::GetState(QMap<quint64, TLxTsspDeviceState>& stateList) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int SS2BBE_CjCsParamSetGUI::GetState(quint64 ui64StatusID, TLxTsspDeviceState& status) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int SS2BBE_CjCsParamSetGUI::GetStateDirect(quint64 ui64StatusID, TLxTsspDeviceState& status)
{
    if(m_StateList.contains(ui64StatusID))
    {
        //
 ...
 */
int SS2BBE_CjCsParamSetGUI::WriteData(quint64 ui64DataID, uchar* aData, uint uiDataLen)
{
    return 0;
}
 ...
 */
int SS2BBE_CjCsParamSetGUI::ReadData(quint64 ui64DataID, uchar* aData, uint& uiDataLen,
                             uint& uiReloadCount)
{
    return 0;
}

int SS2BBE_CjCsParamSetGUI::CreateDefaultMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    SMCU2::MCSSSB2CjParamStruct4 tmpComPara;          //参数
    for(int nIndex = 0; nIndex < MAX_TARGET_COUNT; nIndex++)
    {
        //目标类型
        tmpComPara.cjParam[nIndex].ucTargetType = m_ParaSetStruct[nIndex][0].InitVal.lval + 1;
        //目标代号
        tmpComPara.cjParam[nIndex].ucTargetCode = m_ParaSetStruct[nIndex][1].InitVal.lval;
        //地面测站代号
        tmpComPara.cjParam[nIndex].ucStationID = m_ParaSetStruct[nIndex][2].InitVal.lval;
        //是否使用测站代号
        tmpComPara.cjParam[nIndex].ucStationIDIsUsed = m_ParaSetStruct[nIndex][3].InitVal.lval + 1;
        //工作站数目
        tmpComPara.cjParam[nIndex].ucStationCount = m_ParaSetStruct[nIndex][4].InitVal.lval + 1;
        //上行测量帧CRC校验
        tmpComPara.cjParam[nIndex].ucCRC = m_ParaSetStruct[nIndex][5].InitVal.lval + 1;
        //下行测量帧RP
        tmpComPara.cjParam[nIndex].ucRP = m_ParaSetStruct[nIndex][6].InitVal.lval + 1;
        //下行测量帧CRC校验
        tmpComPara.cjParam[nIndex].ucDownCRC = m_ParaSetStruct[nIndex][7].InitVal.lval + 1;
        //距离预报方式
        tmpComPara.cjParam[nIndex].ucPreRangDataMd = m_ParaSetStruct[nIndex][8].InitVal.lval + 1;
        //星上通道号
        tmpComPara.cjParam[nIndex].ucSatChnelNo = m_ParaSetStruct[nIndex][9].InitVal.lval + 1;
        //距离解模糊
        tmpComPara.cjParam[nIndex].ucRMHCheck = m_ParaSetStruct[nIndex][10].InitVal.lval + 1;
        //距离预报值
        tmpComPara.cjParam[nIndex].ulPridictR = m_ParaSetStruct[nIndex][11].InitVal.lval;
        //应答机零值(单程)
        tmpComPara.cjParam[nIndex].uiRSPDisZero = m_ParaSetStruct[nIndex][12].InitVal.lval;
        //应答机相对时延
        tmpComPara.cjParam[nIndex].uiRSPDifTime = m_ParaSetStruct[nIndex][13].InitVal.lval;

    }

    //取当前设置的参数，存入框架
    QByteArray byteArray;
    byteArray.resize(sizeof(SMCU2::MCSSSB2CjParamStruct4));
...
QWidget* SS2BBE_CjCsParamSetGUI::ShowMeInEdit(QWidget* parent, uchar ucType, ILxTsspParameterMacro* pMacroObj)
{
    g_form = new SS2BBE_CjCsParamSetGUIDlg(parent);
...
int SS2BBE_CjCsParamSetGUI::SaveToMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    if(g_form != NULL)
    {
