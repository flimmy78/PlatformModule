#ifndef SS2BBE_CJCSPARAMSETGUIDLG_H
#define SS2BBE_CJCSPARAMSETGUIDLG_H
...
namespace Ui
{
    class SS2BBE_CjCsParamSetGUIDlg;
}

class SS2BBE_CjCsParamSetGUIDlg : public QDialog
{
    Q_OBJECT
public:
    explicit SS2BBE_CjCsParamSetGUIDlg(QWidget *parent = 0);
    QSize sizeHint() const;
    ~SS2BBE_CjCsParamSetGUIDlg();
...
private:
    Ui::SS2BBE_CjCsParamSetGUIDlg *ui;

    QString m_strTitle;
    TLxTsspObjectInfo m_ObjectInfo;
    ILxTsspLogManager* m_pILxTsspLogManager;

    SMCU2::MCSSSB2CjParamStruct4 m_MCSSSCjParam;       //参数结构体
    SMCU2::MCSSSB2CjStatusStruct4* m_MCSSSCjStatus;       //状态参数结构体
...
private slots:
    ...
    void on_comboBox_DisAmbigous_currentIndexChanged(int index);
    void on_comboBox_DisForType_currentIndexChanged(int index);
    void on_comboBox_StationNum_currentIndexChanged(int index);
    void on_comboBox_TargetType_currentIndexChanged(int index);
};

#endif // SS2BBE_CJCSPARAMSETGUIDLG_H
