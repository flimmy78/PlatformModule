...
namespace
{
    USBBBE_CodeTMParaSetGUIDlg* g_form = NULL;
}
extern quint64 GetID(ushort usStationID, uchar ucSubSysID, ushort usDeviceID, ushort usType,
              uchar ucSn);
USBBBE_CodeTMParaSetGUI::USBBBE_CodeTMParaSetGUI(TLxTsspObjectInfo ObjectInfo)
{
    //初始化
 ...
 * @return 界面对象的界面。
*/
QWidget* USBBBE_CodeTMParaSetGUI::ShowMe(QWidget* parent, const QString& strTitle)
{
    m_strTitle = strTitle;

    USBBBE_CodeTMParaSetGUIDlg* form;

    form = new USBBBE_CodeTMParaSetGUIDlg(parent);
 ...
 * @return szHelpID:长度。
*/
int USBBBE_CodeTMParaSetGUI::GetContextHelpID(char* szHelpID)
{
...
*/
int USBBBE_CodeTMParaSetGUI::GetModuleInfo(TLxTsspModuleInfo& moduleInfo) const
{
    ...
    /// 配置标识，如：D_Y17-61_008
    strcpy(moduleInfo.szConfigID, "D_Y17-92_103\0");
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int USBBBE_CodeTMParaSetGUI::GetObjectInfo(TLxTsspObjectInfo& ObjectInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int USBBBE_CodeTMParaSetGUI::PreInitialize()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int USBBBE_CodeTMParaSetGUI::Initialize()
{
    //日志信息
    TLxTsspLog tmpLog;
    QString strLog;
    ...
    stCurPath = stCurPath + "/System/ModuleConfig/USBBBE_CodeTMParaSetGUI.xml";
    //遍历xml文件，获取参数取值范围
    m_MapSumFlag = 0;
...
//遍历xml文件，获取参数取值范围
int USBBBE_CodeTMParaSetGUI::QueryParaBlock(QString ConfigPath)
{
...
int USBBBE_CodeTMParaSetGUI::GetAllMapValueStringByPara(int MapSum,QDomNode pInDocNode_Para)
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int USBBBE_CodeTMParaSetGUI::Start()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int USBBBE_CodeTMParaSetGUI::Stop()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
bool USBBBE_CodeTMParaSetGUI::ClearUp()
{
    return 1;
}
 ...
 *           其它错误依情况增加；
 */
int USBBBE_CodeTMParaSetGUI::GetErrorNumber()
{
    return m_iErrCode;
}
 ...
 */
int USBBBE_CodeTMParaSetGUI::GetErrorMsg(int iErrorNo, QString& strErrorMsg)
{
 ...
 */
bool USBBBE_CodeTMParaSetGUI::IsOnline()
{
    //对象初始化
    //查找对象管理器
    LxTsspObjectManager* pObjectManager = LxTsspObjectManager::Instance();
 ...
 */
void USBBBE_CodeTMParaSetGUI::SendMessage(TLxTsspMessage msg)
{
    return;
}
 ...
 * @return 1：成功；-1：失败。
 */
int USBBBE_CodeTMParaSetGUI::SetParameter(QMap<quint64, QByteArray> parameterList)
{
 ...
 */
int USBBBE_CodeTMParaSetGUI::SetParameter(quint64 ui64ParamID, QByteArray parameter)
{
 ...
 */
int USBBBE_CodeTMParaSetGUI::GetParameterID(QList<quint64>& pParameterIDList) const
{
 ...
 * @return 大于等于0：参数块个数。-1：失败。
 */
int USBBBE_CodeTMParaSetGUI::GetParameter(QMap<quint64, QByteArray>& parameterList) const
{
    MCSUSBTMParamStruct tmpComParam;       //参数
    MCSUSBTMStatusStruct* tmpComStatus = NULL;       //状态
    ...
    if(tmpStateMgr != NULL)
        tmpComStatus  =
                (MCSUSBTMStatusStruct*)tmpStateMgr->FindOneItem(m_StatusComSynID, stLen);
    ...
    QByteArray  tmpCh;
    tmpCh.resize(sizeof(MCSUSBTMParamStruct));
    memcpy(tmpCh.data(), &tmpComParam, sizeof(MCSUSBTMParamStruct));
 ...
 * @return 1：成功；-1：失败。
 */
int USBBBE_CodeTMParaSetGUI::GetParameter(quint64 ui64ParamID, QByteArray& parameter) const
{
    if(m_ParamList.contains(ui64ParamID))
    {
 ...
 */
int USBBBE_CodeTMParaSetGUI::GetState(QMap<quint64, TLxTsspDeviceState>& stateList) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int USBBBE_CodeTMParaSetGUI::GetState(quint64 ui64StatusID, TLxTsspDeviceState& status) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int USBBBE_CodeTMParaSetGUI::GetStateDirect(quint64 ui64StatusID, TLxTsspDeviceState& status)
{
    if(m_StateList.contains(ui64StatusID))
    {
        //
 ...
 */
int USBBBE_CodeTMParaSetGUI::WriteData(quint64 ui64DataID, uchar* aData, uint uiDataLen)
{
    return 0;
}
 ...
 */
int USBBBE_CodeTMParaSetGUI::ReadData(quint64 ui64DataID, uchar* aData, uint& uiDataLen,
                             uint& uiReloadCount)
{
    return 0;
}

int USBBBE_CodeTMParaSetGUI::CreateDefaultMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    MCSUSBTMParamStruct tmpComPara;          //
    //帧同步码位置
    tmpComPara.ucFspPlace = m_ParaSetStruct[0][0].InitVal.lval + 1;
    //遥测信息标识码
    tmpComPara.ucDataCode = m_ParaSetStruct[0][1].InitVal.lval;
    //副载波频率
    tmpComPara.uiSWFreq = m_ParaSetStruct[0][2].InitVal.lval;
    //译码方式
    tmpComPara.ucCodingMd = m_ParaSetStruct[0][3].InitVal.lval + 1;
    //交织深度
    tmpComPara.ucInterleav = m_ParaSetStruct[0][4].InitVal.lval;
    //码型
    tmpComPara.uiCodeStyle = m_ParaSetStruct[0][5].InitVal.lval + 1;
    //卷积码类型
    tmpComPara.ucViterbiMd = m_ParaSetStruct[0][6].InitVal.lval + 1;
    //Viterbi译码输出相位
    tmpComPara.ucG2Inv = m_ParaSetStruct[0][7].InitVal.lval + 1;
    //码速率
    tmpComPara.uiTM_CodeRate = m_ParaSetStruct[0][8].InitVal.lval;
    //R-S码类型
    tmpComPara.ucRSMd = m_ParaSetStruct[0][9].InitVal.lval + 1;
    //RS译码坐标
    tmpComPara.ucBasiSel = m_ParaSetStruct[0][10].InitVal.lval + 1;
    //字长
    tmpComPara.ucWordLen = m_ParaSetStruct[0][11].InitVal.lval;
    //帧同步码组长
    tmpComPara.ucFrameSynGLen = m_ParaSetStruct[0][12].InitVal.lval + 1;
    //解扰多项式长度
    tmpComPara.ucRandLen = m_ParaSetStruct[0][13].InitVal.lval + 1;
    //帧长
    tmpComPara.usFrameLen = m_ParaSetStruct[0][14].InitVal.lval;
    //帧同步码组
    tmpComPara.uiFrameSynG = m_ParaSetStruct[0][15].InitVal.lval;
    //解扰多项式
    tmpComPara.uiRandPoly = m_ParaSetStruct[0][16].InitVal.lval;
    //格式长
    tmpComPara.ucSframeLen = m_ParaSetStruct[0][17].InitVal.lval;
    //解扰控制
    tmpComPara.ucUnturbCtrl = m_ParaSetStruct[0][18].InitVal.lval + 1;
    //解扰寄存器初相
    tmpComPara.uiPhase = m_ParaSetStruct[0][19].InitVal.lval;
    //副帧同步方式
    tmpComPara.ucSFrameSynMd = m_ParaSetStruct[0][20].InitVal.lval + 1;
    //副帧码组长度
    tmpComPara.ucCycFrmSynCodeLen = m_ParaSetStruct[0][21].InitVal.lval + 1;
    //副帧码组位置
    tmpComPara.usCycFrmSynCodePos = m_ParaSetStruct[0][22].InitVal.lval;
    //副帧同步码组
    tmpComPara.uiCycFrmSynCode = m_ParaSetStruct[0][23].InitVal.lval;
    //ID字位置
    tmpComPara.usIDSubFrmPos = m_ParaSetStruct[0][24].InitVal.lval;
    //ID字基值
    tmpComPara.ucIDBase = m_ParaSetStruct[0][25].InitVal.lval + 1;
    //ID计数方向
    tmpComPara.ucIDDirect = m_ParaSetStruct[0][26].InitVal.lval + 1;
    //帧同步容错搜索容错位数
    tmpComPara.ucFrSynSerLiErr = m_ParaSetStruct[1][0].InitVal.lval;
    //帧同步容错校核容错位数
    tmpComPara.ucFrSynLoLiErr = m_ParaSetStruct[1][1].InitVal.lval;
    //帧同步锁定容错位数
    tmpComPara.ucFrSynVerLiErr = m_ParaSetStruct[1][2].InitVal.lval;
    //帧同步容错校核检测帧数
    tmpComPara.ucFrSynVerCheck = m_ParaSetStruct[1][3].InitVal.lval;
    //帧同步容错锁定帧数
    tmpComPara.ucFrSynLoCheck = m_ParaSetStruct[1][4].InitVal.lval;
    //副帧同步容错校核容错位数
    tmpComPara.ucCFSearchErrBits = m_ParaSetStruct[1][5].InitVal.lval;
    //副帧同步容错校核容错位数
    tmpComPara.ucCFVerifyErrBits = m_ParaSetStruct[1][6].InitVal.lval;
    //副帧同步容错锁定容错位数
    tmpComPara.ucCFLockErrBits = m_ParaSetStruct[1][7].InitVal.lval;
    //副帧校核帧数
    tmpComPara.ucSubFVerifyFrame = m_ParaSetStruct[1][8].InitVal.lval;
    //副帧锁定帧数
    tmpComPara.ucSubFLockFrame = m_ParaSetStruct[1][9].InitVal.lval;
...
QWidget* USBBBE_CodeTMParaSetGUI::ShowMeInEdit(QWidget* parent, uchar ucType, ILxTsspParameterMacro* pMacroObj)
{
    g_form = new USBBBE_CodeTMParaSetGUIDlg(parent);
...
int USBBBE_CodeTMParaSetGUI::SaveToMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    if(g_form != NULL)
    {
