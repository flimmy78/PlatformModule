...
extern "C" int GetConfigFormat(QString& strConfigFormat)
{
    ...
    strConfigFormat = QString(
            "<Module Name=\"libUSBBBE_CodeTMParaSetGUI.so\" StationID=\"2311\" SubSysID=\"04\" DevID=\"0401\" Note=\"%1\"> \n "
            "<Object ID=\"5139\" Num=\"1\" IndexStart=\"0\" Alias =\" \" Note=\"参数设置\">...</Object> \n "
    ...
    for(int i=0; i<nodeList.length(); i++)
    {
        ...
        if(usObjID == 0x5139)
        {
            for(int k=iStart; k<iStop; k++)
            {
                ...
                strcpy(ObjectInfo.szConfigID, "D_Y17-92_103");
                strcpy(ObjectInfo.szAlias, objAlias.toUtf8().data());
                strcpy(ObjectInfo.szComment, objNote.toUtf8().data());
                g_ObjectList.insert(ui64ObjectID, new USBBBE_CodeTMParaSetGUI(ObjectInfo));
            }
        }
    }
