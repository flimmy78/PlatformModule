...
//定义检前记录的基础值
ushort g_usRecordMonitorDeviceID;
...
extern "C" int GetConfigFormat(QString& strConfigFormat)
{
    ...
    strConfigFormat = QString("<Module Name=\"libRecordMonitor.so\" StationID=\"2311\" SubSysID=\"00\" DevID=\"1200\" Note=\"%1\"> \n "
            "<Object ID=\"5120\" Num=\"1\" IndexStart=\"0\" Alias =\" \" Note=\"%1\">...</Object> \n "
                             ...
                             QMap<quint64, void*>& ObjList)
{
    ...
    g_usRecordMonitorDeviceID = usDeviceID;
    ...
    for(int i=0; i<nodeList.length(); i++)
    {
        ...
        if(usObjID == 0x5120)
        {
            for(int k=iStart; k<iStop; k++)
            {
                ...
                strcpy(ObjectInfo.szConfigID, "D_Y17-92_115");
                strcpy(ObjectInfo.szAlias, objAlias.toUtf8().data());
                strcpy(ObjectInfo.szComment, objNote.toUtf8().data());
                g_ObjectList.insert(ui64ObjectID, new RecordMonitorGUI(ObjectInfo));
            }
        }
    }
