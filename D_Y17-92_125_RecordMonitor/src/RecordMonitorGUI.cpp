...
namespace
{
    RecordMonitorGUIDlg* g_form = NULL;
}

extern quint64 GetID(ushort usStationID, uchar ucSubSysID, ushort usDeviceID, ushort usType,
              uchar ucSn);

RecordMonitorGUI::RecordMonitorGUI(TLxTsspObjectInfo ObjectInfo)
{
    //初始化
 ...
 * @return 界面对象的界面。
*/
QWidget* RecordMonitorGUI::ShowMe(QWidget* parent, const QString& strTitle)
{
    m_strTitle = strTitle;

    RecordMonitorGUIDlg* form;

    form = new RecordMonitorGUIDlg(parent);
 ...
 * @return szHelpID:长度。
*/
int RecordMonitorGUI::GetContextHelpID(char* szHelpID)
{
...
*/
int RecordMonitorGUI::GetModuleInfo(TLxTsspModuleInfo& moduleInfo) const
{
    ...
    /// 配置标识，如：D_Y17-61_008
    strcpy(moduleInfo.szConfigID, "D_Y17-92_125\0");
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int RecordMonitorGUI::GetObjectInfo(TLxTsspObjectInfo& ObjectInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int RecordMonitorGUI::PreInitialize()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int RecordMonitorGUI::Initialize()
{
    //日志信息
    TLxTsspLog tmpLog;
    QString strLog;
...
//    stCurPath = stCurPath + "/System/ModuleConfig/RecorderMonitorGUI.xml";
//    //遍历xlm文件，获取参数取值范围
//    m_MapSumFlag = 0;
...
//遍历xlm文件，获取参数取值范围
int RecordMonitorGUI::QueryParaBlock(QString ConfigPath)
{
...
int RecordMonitorGUI::GetAllMapValueStringByPara(int MapSum,QDomNode pInDocNode_Para)
{
 ...
 *           其它错误依情况增加；
 */
int RecordMonitorGUI::GetErrorNumber()
{
    return m_iErrCode;
}
 ...
 */
int RecordMonitorGUI::GetErrorMsg(int iErrorNo, QString& strErrorMsg)
{
 ...
 */
bool RecordMonitorGUI::IsOnline()
{
    //对象初始化
    //查找对象管理器
    LxTsspObjectManager* pObjectManager = LxTsspObjectManager::Instance();
 ...
 */
void RecordMonitorGUI::SendMessage(TLxTsspMessage msg)
{
    return;
}
 ...
 * @return 1：成功；-1：失败。
 */
int RecordMonitorGUI::SetParameter(QMap<quint64, QByteArray> parameterList)
{
 ...
 */
int RecordMonitorGUI::SetParameter(quint64 ui64ParamID, QByteArray parameter)
{
 ...
 */
int RecordMonitorGUI::GetParameterID(QList<quint64>& pParameterIDList) const
{
 ...
 * @return 大于等于0：参数块个数。-1：失败。
 */
int RecordMonitorGUI::GetParameter(QMap<quint64, QByteArray>& parameterList) const
{
    ...
    if(tmpStateMgr != NULL)
        tmpCodeSynStatus  =
                (DDTBBECodeSynStatus*)tmpStateMgr->FindOneItem(tmpStatusID, stLen);
 ...
 * @return 1：成功；-1：失败。
 */
int RecordMonitorGUI::GetParameter(quint64 ui64ParamID, QByteArray& parameter) const
{
    if(m_ParamList.contains(ui64ParamID))
    {
 ...
 */
int RecordMonitorGUI::GetState(QMap<quint64, TLxTsspDeviceState>& stateList) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int RecordMonitorGUI::GetState(quint64 ui64StatusID, TLxTsspDeviceState& status) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int RecordMonitorGUI::GetStateDirect(quint64 ui64StatusID, TLxTsspDeviceState& status)
{
    if(m_StateList.contains(ui64StatusID))
    {
        //
 ...
 */
int RecordMonitorGUI::WriteData(quint64 ui64DataID, uchar* aData, uint uiDataLen)
{
    return 0;
}
 ...
 */
int RecordMonitorGUI::ReadData(quint64 ui64DataID, uchar* aData, uint& uiDataLen,
                             uint& uiReloadCount)
{
    return 0;
}

int RecordMonitorGUI::CreateDefaultMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    return 1;
}

QWidget* RecordMonitorGUI::ShowMeInEdit(QWidget* parent, uchar ucType, ILxTsspParameterMacro* pMacroObj)
{
    g_form = new RecordMonitorGUIDlg(parent);
...
int RecordMonitorGUI::SaveToMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    if(g_form != NULL)
    {
