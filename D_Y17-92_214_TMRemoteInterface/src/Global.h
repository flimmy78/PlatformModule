...
//注意：只用与过程控制命令 index只能是 0 2 4 6 8 10
#define DATA_SOURCE_ALLOC_PRO_CTRL(_type, dataSource, moduleName, devID, index) \
    do{\
    ITEM* pItem = m_pPlatformRes->m_devMap.getItem(moduleName);\
    if (NULL == pItem || pItem->vSubItem.size() <= index)return false;\
    int tmp = index;\
    if(devID%2 != 0)++tmp;\
    SUBITEM* sItem = &pItem->vSubItem[tmp];\
    if(NULL == sItem)return false;\
    int temdwLen = sizeof(_type);\
    dataSource = (_type*)m_pPlatformRes->FindOneItem(sItem->usStationID, sItem->ucSubSysID, sItem->usDeviceID, sItem->usStateType, sItem->ucSN, temdwLen);\
    if(dataSource)break;\
    dataSource = (_type*)m_pPlatformRes->ApplyOneItem(sItem->usStationID, sItem->ucSubSysID, sItem->usDeviceID, sItem->usStateType, sItem->ucSN, temdwLen);\
    if(!dataSource)return false;\
    memset(dataSource, 0, temdwLen);\
    }while(0);
#define DATA_SOURCE_ALLOC(_type, dataSource, moduleName, devID, index) \
    do{\
    ITEM* pItem = m_pPlatformRes->m_devMap.getItem(moduleName);\
    if (NULL == pItem /*|| pItem->vSubItem.size() <= index*/)\
{\
    qDebug()<<"NULL == pItem";\
    return false;\
    }\
    SUBITEM* sItem = pItem->getSubItemByDeviceID_SN(devID, index);\
    if(NULL == sItem)\
{\
    qDebug()<<"NULL == sItem"<<"devID"<<devID;\
    return false;\
    }\
    int temdwLen = sizeof(_type);\
    dataSource = (_type*)m_pPlatformRes->ApplyOneItem(sItem->usStationID, \
    sItem->ucSubSysID, sItem->usDeviceID, sItem->usStateType, sItem->ucSN, temdwLen);\
    if(!dataSource)\
{\
    qDebug()<<"!dataSource"<<"usStationID"<<sItem->usStationID\
<<"ucSubSysID"<<sItem->ucSubSysID<<"usDeviceID"<<sItem->usDeviceID<<"usStateType"\
<<sItem->usStateType<<"ucSN"<<sItem->ucSN<<"temdwLen"<<temdwLen;\
    return false;\
    }\
    memset(dataSource, 0, temdwLen);\
    }while(0);
