...
bool CRM::init(USHORT deviceID, PlatformResource* pRes)
{
    ...
    m_resID = m_pPlatformRes->m_nIDCurrent;
    DATA_SOURCE_ALLOC(RECORDFILELIST, m_pFileListRes, "检前记录设备监控", deviceID, 1)
    m_filesListID = m_pPlatformRes->m_nIDCurrent;
    DATA_SOURCE_ALLOC(RECORDFILEINFOCOMM, m_pFileInfoRes, "检前记录设备监控", deviceID, 2)
    m_fileInfoID = m_pPlatformRes->m_nIDCurrent;
    return true;
}

void CRM::set(SRM& srm)
{
    memcpy(m_pStatusWorkRes, &srm.m_StatusWorkRes, sizeof(srm.m_StatusWorkRes));
    memcpy(m_pFileListRes, &srm.m_FileListRes, sizeof(srm.m_FileListRes));
    memcpy(m_pFileInfoRes, &srm.m_FileInfoRes, sizeof(srm.m_FileInfoRes));
}

void CRM::get(SRM& srm)
{
    memcpy(&srm.m_StatusWorkRes, m_pStatusWorkRes, sizeof(srm.m_StatusWorkRes));
    memcpy(&srm.m_FileListRes, m_pFileListRes, sizeof(srm.m_FileListRes));
    memcpy(&srm.m_FileInfoRes, m_pFileInfoRes, sizeof(srm.m_FileInfoRes));
}
