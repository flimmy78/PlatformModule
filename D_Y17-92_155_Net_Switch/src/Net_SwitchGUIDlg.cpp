...
Net_SwitchGUIDlg::Net_SwitchGUIDlg(QWidget *parent) : QDialog(parent),
    ui(new Ui::Net_SwitchGUIDlg)
{
    ui->setupUi(this);
    m_pStateMgr = NULL;
...
Net_SwitchGUIDlg::~Net_SwitchGUIDlg()
{
    killTimer(m_TimerID);
    delete ui;
}

void Net_SwitchGUIDlg::setText(const QString& strTitle,ParaStruct xmlsetText[3][60],QString MapMean[50][30],int MapNum[50],uchar Type,ILxTsspParameterMacro* pMacroObj)
{
    m_strTitle = strTitle;
...
//初始化对话框
void Net_SwitchGUIDlg::InitDLG(int Type, ILxTsspParameterMacro* pMacroObj)
{
    if(Type == 0)//取当前参数
    {
        //取当前设置的参数
...
void Net_SwitchGUIDlg::SetObjInfo(TLxTsspObjectInfo ObjectInfo)
{
...
//保存参数宏
int Net_SwitchGUIDlg::SaveToMacro(ILxTsspParameterMacro* pMacroObj)
{
    //获取设置的通道
...
//定时器
void Net_SwitchGUIDlg::timerEvent(QTimerEvent *event)
{
...
void Net_SwitchGUIDlg::OnSaveBtn()
{
    //保存配置信息到文件中
...
void Net_SwitchGUIDlg::OnBtnDel()
{
    QString strName = m_pComType->currentText();
    if (strName.isEmpty())
    {
...
void Net_SwitchGUIDlg::OnBtnModify()
{
    if (!m_bOnLine)
    {
...
void Net_SwitchGUIDlg::OnBtnSetting()
{
    //获取设置的通道
    QList<QString> channelList = getShowInfo();
    QByteArray ba;
...
#ifdef SHOW_STATUS
    //不需要在界面上显示状态信息
    m_StatusTimerID = startTimer(1000);
#endif // SHOW_STATUS
...
void Net_SwitchGUIDlg::onChooseAccount(const QString &strText)
{
    qDebug() << strText;
    //获取当前选择的列表
    QStringList strList;
...
void Net_SwitchGUIDlg::setShowInfo(const QStringList &infoList)
{
...
bool Net_SwitchGUIDlg::initCmdInfo(QString strModuleName)
{
    bool bRet = m_DeviceMap.loadXML();
    if (!bRet)
    {
...
QList<QString> Net_SwitchGUIDlg::getShowInfo()
{
...
void Net_SwitchGUIDlg::closeEvent(QCloseEvent *event)
{
    event->accept();
}

void Net_SwitchGUIDlg::init()
{
    QDir dir;
    QString strPath = dir.currentPath();
    QString strConfigPath = strPath + "/TM/System/ModuleConfig/DOWNNet_SwitchConfig.xml";
    ...
    //加载保存的配置信息
    m_strFilePath = strPath + "/TM/System/ModuleConfig/DOWNNet_SwitchSave.data";
    loadSaveInfo(m_strFilePath);
...
QWidget *Net_SwitchGUIDlg::getSignalBoard(int boardHeight, int nNum,
...
void Net_SwitchGUIDlg::showLight()
{
    ...
    /********************设备是否在线************************/
    m_bOnLine = CCommFunc::getInstance()->getDownNet_switchStatus();
    if (!m_bOnLine)
    {
...
void Net_SwitchGUIDlg::setOfflineStatusValue()
{

}

void Net_SwitchGUIDlg::initMatrixAndOther()
{
    m_nMatrixNum = m_reader.m_IMFConfig.matrixNum;
    int m_nNum = m_reader.m_IMFConfig.matrixNum;
    static const int s_DIALOG_HEIGHT = m_nNum * 1.0 / MAX_NUM * 800;//400最佳，TODO:因为Matrix不适应32个情况
...
void Net_SwitchGUIDlg::initCommboAndBtn()
{
...
void Net_SwitchGUIDlg::loadSaveInfo(const QString &strPath)
{
    m_IFMList.clear();
    //解析文件中的数据
    QFile file(strPath);
...
void Net_SwitchGUIDlg::saveData2File(const QString &strPath, const QString &name)
{
    //解析文件中的数据
    QFile file(strPath);
...
void Net_SwitchGUIDlg::closeLight()
{
    for (int nRow = 1; nRow <= m_nMatrixNum; nRow++)
    {
        for(int col = 1;  col <= m_nMatrixNum; col++)
        setLight(nRow, col, 0);
    }
}
void Net_SwitchGUIDlg::updateCombo()
{
    //
    m_pComType->clear();
    //将配置信息显示在列表框中
...
QSize Net_SwitchGUIDlg::sizeHint() const
{
    return QSize(this->width(), this->height());
}

void Net_SwitchGUIDlg::SendMessage(char* pData, unsigned short  usLen, unsigned short usC, unsigned short usTID)
{
    static uint seq = 0;
...
void Net_SwitchGUIDlg::setLight(int nRow, int nCol, bool bFlag /* = true */)
{
    if (m_pMatrix != NULL)
    {
...
void Net_SwitchGUIDlg::setType(bool bMain, bool bNormal)
{
...
void Net_SwitchGUIDlg::setStatus()
{
#ifdef SHOW_STATUS
