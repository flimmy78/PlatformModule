...
extern "C" int GetModuleInfo(TLxTsspModuleInfo& moduleInfo)
{
    ...
    /// 配置标识，如：D_Y17-61_008
    strcpy(moduleInfo.szConfigID, "D_Y17-92_238\0");
...
extern "C" int GetConfigFormat(QString& strConfigFormat)
{
    ...
    strConfigFormat =
            "<Module Name=\"TCBBE_TCParamSetGUI.so\" StationID=\"2311\" SubSysID=\"04\" DevID=\"0401\" Note=\"BPSK遥测解调参数设置\"> \n \
    ...
    for(int i=0; i<nodeList.length(); i++)
    {
        ...
        if(usObjID == 0x5154)
        {
            for(int k=iStart; k<iStop; k++)
            {
                ...
                g_ObjectList.insert(ui64ObjectID, new TCBBE_TCParamSetGUI(ObjectInfo));
            }
        }
    }
