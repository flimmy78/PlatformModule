...
namespace
{
    USBBBE_PMModuSetGUIDlg* g_form = NULL;
}
extern quint64 GetID(ushort usStationID, uchar ucSubSysID, ushort usDeviceID, ushort usType,
              uchar ucSn);
USBBBE_PMModuSetGUI::USBBBE_PMModuSetGUI(TLxTsspObjectInfo ObjectInfo)
{
    //初始化
 ...
 * @return 界面对象的界面。
*/
QWidget* USBBBE_PMModuSetGUI::ShowMe(QWidget* parent, const QString& strTitle)
{
    m_strTitle = strTitle;

    USBBBE_PMModuSetGUIDlg* form;

    form = new USBBBE_PMModuSetGUIDlg(parent);
 ...
 * @return szHelpID:长度。
*/
int USBBBE_PMModuSetGUI::GetContextHelpID(char* szHelpID)
{
...
*/
int USBBBE_PMModuSetGUI::GetModuleInfo(TLxTsspModuleInfo& moduleInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int USBBBE_PMModuSetGUI::GetObjectInfo(TLxTsspObjectInfo& ObjectInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int USBBBE_PMModuSetGUI::PreInitialize()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int USBBBE_PMModuSetGUI::Initialize()
{
    //日志信息
    TLxTsspLog tmpLog;
    QString strLog;
    ...
    stCurPath = stCurPath + "/System/ModuleConfig/USBBBE_PMModuSetGUI.xml";
    //遍历xml文件，获取参数取值范围
    m_MapSumFlag = 0;
...
//遍历xml文件，获取参数取值范围
int USBBBE_PMModuSetGUI::QueryParaBlock(QString ConfigPath)
{
...
int USBBBE_PMModuSetGUI::GetAllMapValueStringByPara(int MapSum,QDomNode pInDocNode_Para)
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
bool USBBBE_PMModuSetGUI::ClearUp()
{
    return 1;
}
 ...
 *           其它错误依情况增加；
 */
int USBBBE_PMModuSetGUI::GetErrorNumber()
{
    return m_iErrCode;
}
 ...
 */
int USBBBE_PMModuSetGUI::GetErrorMsg(int iErrorNo, QString& strErrorMsg)
{
 ...
 */
bool USBBBE_PMModuSetGUI::IsOnline()
{
    //对象初始化
    //查找对象管理器
    LxTsspObjectManager* pObjectManager = LxTsspObjectManager::Instance();
 ...
 */
void USBBBE_PMModuSetGUI::SendMessage(TLxTsspMessage msg)
{
    return;
}
 ...
 * @return 1：成功；-1：失败。
 */
int USBBBE_PMModuSetGUI::SetParameter(QMap<quint64, QByteArray> parameterList)
{
 ...
 */
int USBBBE_PMModuSetGUI::SetParameter(quint64 ui64ParamID, QByteArray parameter)
{
 ...
 */
int USBBBE_PMModuSetGUI::GetParameterID(QList<quint64>& pParameterIDList) const
{
 ...
 * @return 大于等于0：参数块个数。-1：失败。
 */
int USBBBE_PMModuSetGUI::GetParameter(QMap<quint64, QByteArray>& parameterList) const
{
    MCSUSBUpModulParamStruct tmpUpModulParam;       //码同步参数

    MCSUSBUpModulStatusStruct* tmpUpModulStatus = NULL;      //码同步参数
    ...
    if(tmpStateMgr != NULL)
        tmpUpModulStatus  =
                (MCSUSBUpModulStatusStruct*)tmpStateMgr->FindOneItem(m_StatusModuID, stLen);

    if(tmpUpModulStatus != NULL)
    {
        tmpUpModulParam = tmpUpModulStatus->tParams;
    }

    QByteArray  tmpCh;
    tmpCh.resize(sizeof(MCSUSBUpModulParamStruct));
    memcpy(tmpCh.data(), &tmpUpModulParam, sizeof(MCSUSBUpModulParamStruct));
 ...
 * @return 1：成功；-1：失败。
 */
int USBBBE_PMModuSetGUI::GetParameter(quint64 ui64ParamID, QByteArray& parameter) const
{
    if(m_ParamList.contains(ui64ParamID))
    {
 ...
 */
int USBBBE_PMModuSetGUI::GetState(QMap<quint64, TLxTsspDeviceState>& stateList) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int USBBBE_PMModuSetGUI::GetState(quint64 ui64StatusID, TLxTsspDeviceState& status) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int USBBBE_PMModuSetGUI::GetStateDirect(quint64 ui64StatusID, TLxTsspDeviceState& status)
{
    if(m_StateList.contains(ui64StatusID))
    {
        //
 ...
 */
int USBBBE_PMModuSetGUI::WriteData(quint64 ui64DataID, uchar* aData, uint uiDataLen)
{
    return 0;
}
 ...
 */
int USBBBE_PMModuSetGUI::ReadData(quint64 ui64DataID, uchar* aData, uint& uiDataLen,
                             uint& uiReloadCount)
{
    return 0;
}

int USBBBE_PMModuSetGUI::CreateDefaultMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    MCSUSBUpModulParamStruct aMCSUSBUpModulParam;
    //载波输出
    aMCSUSBUpModulParam.ucWaveOutCtrl = m_ParaSetStruct[0][0].InitVal.lval + 1;
    //多普勒预置频率
    aMCSUSBUpModulParam.ucPreDplValue = m_ParaSetStruct[0][1].InitVal.lval;
    //载波输出电平
    aMCSUSBUpModulParam.cWaveOutLevel = m_ParaSetStruct[0][2].InitVal.lval;
    //测距加调
    aMCSUSBUpModulParam.ucToneSw = m_ParaSetStruct[0][3].InitVal.lval + 1;
    //遥控加调
    aMCSUSBUpModulParam.ucTcSw = m_ParaSetStruct[0][4].InitVal.lval + 1;
    //话音加调
    aMCSUSBUpModulParam.ucAudioSw = m_ParaSetStruct[0][5].InitVal.lval + 1;
    //盲发开关
    aMCSUSBUpModulParam.ucTcCntr = m_ParaSetStruct[0][6].InitVal.lval + 1;
    //测距主音调制度
    aMCSUSBUpModulParam.ucT0Index = m_ParaSetStruct[0][7].InitVal.lval;
    //测距次音调制度
    aMCSUSBUpModulParam.ucTnIndex = m_ParaSetStruct[0][8].InitVal.lval;
    //遥测模拟源调制度
    aMCSUSBUpModulParam.ucTmsimIndex = m_ParaSetStruct[0][9].InitVal.lval;
    //遥控调制度
    aMCSUSBUpModulParam.ucTcIndex = m_ParaSetStruct[0][10].InitVal.lval;
    //话音调制度
    aMCSUSBUpModulParam.ucAudioIndex = m_ParaSetStruct[0][11].InitVal.lval;
    //扫描速率
    aMCSUSBUpModulParam.uiScanRate = m_ParaSetStruct[0][12].InitVal.lval;
    //扫描范围
    aMCSUSBUpModulParam.uiScanRange = m_ParaSetStruct[0][13].InitVal.lval;
    //噪声源输出
    aMCSUSBUpModulParam.ucNoiseSw = m_ParaSetStruct[0][14].InitVal.lval + 1;
    //输出载噪比
    aMCSUSBUpModulParam.ucCN0 = m_ParaSetStruct[0][15].InitVal.lval;
    //遥测模拟源加调
    aMCSUSBUpModulParam.ucTmsimSw = m_ParaSetStruct[1][1].InitVal.lval + 1;
    //模拟开关
//    aMCSUSBUpModulParam.usHRSel = m_ParaSetStruct[0][8].InitVal.lval;
    //模拟多普勒范围
//    aMCSUSBUpModulParam.usHRSel = m_ParaSetStruct[0][8].InitVal.lval;
    //模拟多普勒变化率
//    aMCSUSBUpModulParam.usHRSel = m_ParaSetStruct[0][8].InitVal.lval;
    ...
    byteArray.resize(sizeof(aMCSUSBUpModulParam));
    memcpy(byteArray.data(),&aMCSUSBUpModulParam,sizeof(aMCSUSBUpModulParam));
    if(pMacroObj!=NULL)
    {
...
QWidget* USBBBE_PMModuSetGUI::ShowMeInEdit(QWidget* parent,uchar ucType,ILxTsspParameterMacro* pMacroObj)
{
    g_form = new USBBBE_PMModuSetGUIDlg(parent);
...
int USBBBE_PMModuSetGUI::SaveToMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    if(g_form!=NULL)
    {
