...
extern "C" int GetConfigFormat(QString& strConfigFormat)
{
    ...
    strConfigFormat =
            QString("<Module Note=\"%1\" DeviceID=\"3D00\" SubSysID=\"00\" Name=\"libTESTNet_Switch.so\" StationID=\"2311\"> \n"
                    "<Object Note=\"%1\" Num=\"1\" Alias=\" \" ID=\"6002\" IndexStart=\"0\"/> \n"
    ...
    for(int i=0; i<nodeList.length(); i++)
    {
        ...
        if(usObjID == 0x6002)
        {
            for(int k=iStart; k<iStop; k++)
            {
                ...
                strcpy(ObjectInfo.szConfigID, "D_Y17-92_126");
                strcpy(ObjectInfo.szAlias, objAlias.toUtf8().data());
                strcpy(ObjectInfo.szComment, objNote.toUtf8().data());
                g_ObjectList.insert(ui64ObjectID, new TestNet_SwitchGUI(ObjectInfo));
            }
        }
    }
