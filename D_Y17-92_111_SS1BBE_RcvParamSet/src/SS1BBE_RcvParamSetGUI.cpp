#include "SS1BBE_RcvParamSetGUI.h"
...
namespace
{
    SS1BBE_RcvParamSetGUIDlg* g_form = NULL;
}
extern quint64 GetID(ushort usStationID, uchar ucSubSysID, ushort usDeviceID, ushort usType,
              uchar ucSn);
SS1BBE_RcvParamSetGUI::SS1BBE_RcvParamSetGUI(TLxTsspObjectInfo ObjectInfo)
{
    //初始化
 ...
 * @return 界面对象的界面。
*/
QWidget* SS1BBE_RcvParamSetGUI::ShowMe(QWidget* parent, const QString& strTitle)
{
    m_strTitle = strTitle;

    SS1BBE_RcvParamSetGUIDlg* form;

    form = new SS1BBE_RcvParamSetGUIDlg(parent);
 ...
 * @return szHelpID:长度。
*/
int SS1BBE_RcvParamSetGUI::GetContextHelpID(char* szHelpID)
{
...
*/
int SS1BBE_RcvParamSetGUI::GetModuleInfo(TLxTsspModuleInfo& moduleInfo) const
{
    ...
    /// 配置标识，如：D_Y17-61_008
    strcpy(moduleInfo.szConfigID, "D_Y17-92_111\0");
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS1BBE_RcvParamSetGUI::GetObjectInfo(TLxTsspObjectInfo& ObjectInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS1BBE_RcvParamSetGUI::PreInitialize()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS1BBE_RcvParamSetGUI::Initialize()
{
    //日志信息
    TLxTsspLog tmpLog;
    QString strLog;
    ...
    stCurPath = stCurPath + "/System/ModuleConfig/SS1BBE_RcvParamSetGUI.xml";
    //遍历xml文件，获取参数取值范围
    m_MapSumFlag = 0;
    if(QueryParaBlock(stCurPath) < 0)
    {
        ...
        strLog = QString("%1: 模式1接收参数设置: 读取配置文件失败！").arg(MODULENAME);
    ...
    strLog = "遥测解调参数设置: 读取配置文件初始化调制参数成功！";
...
//遍历xml文件，获取参数取值范围
int SS1BBE_RcvParamSetGUI::QueryParaBlock(QString ConfigPath)
{
...
int SS1BBE_RcvParamSetGUI::GetAllMapValueStringByPara(int MapSum,QDomNode pInDocNode_Para)
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS1BBE_RcvParamSetGUI::Start()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS1BBE_RcvParamSetGUI::Stop()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
bool SS1BBE_RcvParamSetGUI::ClearUp()
{
    return 1;
}
 ...
 *           其它错误依情况增加；
 */
int SS1BBE_RcvParamSetGUI::GetErrorNumber()
{
    return m_iErrCode;
}
 ...
 */
int SS1BBE_RcvParamSetGUI::GetErrorMsg(int iErrorNo, QString& strErrorMsg)
{
 ...
 */
bool SS1BBE_RcvParamSetGUI::IsOnline()
{
    //对象初始化
    //查找对象管理器
    LxTsspObjectManager* pObjectManager = LxTsspObjectManager::Instance();
 ...
 */
void SS1BBE_RcvParamSetGUI::SendMessage(TLxTsspMessage msg)
{
    return;
}
 ...
 * @return 1：成功；-1：失败。
 */
int SS1BBE_RcvParamSetGUI::SetParameter(QMap<quint64, QByteArray> parameterList)
{
 ...
 */
int SS1BBE_RcvParamSetGUI::SetParameter(quint64 ui64ParamID, QByteArray parameter)
{
 ...
 */
int SS1BBE_RcvParamSetGUI::GetParameterID(QList<quint64>& pParameterIDList) const
{
 ...
 * @return 大于等于0：参数块个数。-1：失败。
 */
int SS1BBE_RcvParamSetGUI::GetParameter(QMap<quint64, QByteArray>& parameterList) const
{
    SMCU::MCSSSRevParamStruct4 tmpCodeSynPara;       //接收机参数
    SMCU::MCSSSRevStatusStruct4* tmpCodeSynStatus = NULL;      //接收机状态参数
    ...
    if(tmpStateMgr != NULL)
        tmpCodeSynStatus  =
                (SMCU::MCSSSRevStatusStruct4*)tmpStateMgr->FindOneItem(m_StatusComSynID, stLen);

    if(tmpCodeSynStatus != NULL)
    {
        tmpCodeSynPara.MCSSSRevParam = tmpCodeSynStatus->SSRevStatu.tParams;
        for (int i = 0; i < MAX_TARGET_COUNT; i++)
            tmpCodeSynPara.MCSSSRevParamTarget[i] = tmpCodeSynStatus->targetSSRevStatu[i].tParams;

    }
    QByteArray  tmpCh;
    tmpCh.resize(sizeof(SMCU::MCSSSRevParamStruct4));
    memcpy(tmpCh.data(), &tmpCodeSynPara, sizeof(SMCU::MCSSSRevParamStruct4));
 ...
 * @return 1：成功；-1：失败。
 */
int SS1BBE_RcvParamSetGUI::GetParameter(quint64 ui64ParamID, QByteArray& parameter) const
{
    if(m_ParamList.contains(ui64ParamID))
    {
 ...
 */
int SS1BBE_RcvParamSetGUI::GetState(QMap<quint64, TLxTsspDeviceState>& stateList) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int SS1BBE_RcvParamSetGUI::GetState(quint64 ui64StatusID, TLxTsspDeviceState& status) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int SS1BBE_RcvParamSetGUI::GetStateDirect(quint64 ui64StatusID, TLxTsspDeviceState& status)
{
    if(m_StateList.contains(ui64StatusID))
    {
        //
 ...
 */
int SS1BBE_RcvParamSetGUI::WriteData(quint64 ui64DataID, uchar* aData, uint uiDataLen)
{
    return 0;
}
 ...
 */
int SS1BBE_RcvParamSetGUI::ReadData(quint64 ui64DataID, uchar* aData, uint& uiDataLen,
                             uint& uiReloadCount)
{
    return 0;
}

int SS1BBE_RcvParamSetGUI::CreateDefaultMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    SMCU::MCSSSRevParamStruct4 aMCSSSRevParam;
    /*公共参数*/
    //接收通道
    aMCSSSRevParam.MCSSSRevParam.ucRevChannel = m_ParaSetStruct[0][0].InitVal.lval + 1;
    //AGC时间常数
    aMCSSSRevParam.MCSSSRevParam.ucAgcTime = m_ParaSetStruct[0][1].InitVal.lval + 1;
    //载波环路带宽
    aMCSSSRevParam.MCSSSRevParam.ucPllBw = m_ParaSetStruct[0][2].InitVal.lval + 1;

    /*目标参数*/
    for (int nIndex = 0; nIndex < MAX_TARGET_COUNT; nIndex++)
    {
        //扩频码速率
        aMCSSSRevParam.MCSSSRevParamTarget[nIndex].ucPNRate = m_ParaSetStruct[nIndex + 1][0].InitVal.lval + 1;
        //功率比
        aMCSSSRevParam.MCSSSRevParamTarget[nIndex].siRatiodB = m_ParaSetStruct[nIndex + 1][1].InitVal.lval * 10;
        //码环带宽
        aMCSSSRevParam.MCSSSRevParamTarget[nIndex].usPNPIIBw = m_ParaSetStruct[nIndex + 1][2].InitVal.lval * 5;
        //捕获门限
        aMCSSSRevParam.MCSSSRevParamTarget[nIndex].ucCN0 = m_ParaSetStruct[nIndex + 1][3].InitVal.lval;
    }
    //取当前设置的参数，存入框架
    QByteArray byteArray;
    byteArray.resize(sizeof(aMCSSSRevParam));
    memcpy(byteArray.data(), &aMCSSSRevParam, sizeof(aMCSSSRevParam));
    if(pMacroObj != NULL)
    {
...
QWidget* SS1BBE_RcvParamSetGUI::ShowMeInEdit(QWidget* parent,uchar ucType,ILxTsspParameterMacro* pMacroObj)
{
    g_form = new SS1BBE_RcvParamSetGUIDlg(parent);
...
int SS1BBE_RcvParamSetGUI::SaveToMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    if(g_form != NULL)
    {
