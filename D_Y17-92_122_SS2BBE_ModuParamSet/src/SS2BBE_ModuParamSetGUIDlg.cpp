#include "SS2BBE_ModuParamSetGUIDlg.h"
#include "ui_SS2BBE_ModuParamSetGUIDlg.h"
#include <QCloseEvent>
#include <QMessageBox>
#include <QDateTime>
...
void SS2BBE_ModuParamSetGUIDlg::InitDLG(int Type, ILxTsspParameterMacro* pMacroObj)
{
    ...
    else if(Type == 1)//取宏里参数
    {
        ...
        ui->groupBox_4->setGeometry(50, 25, 291, 80);
        ui->groupBox_5->setGeometry(378, 25, 251, 80);
        ui->groupBox_6->setGeometry(660, 25, 261, 80);
        ui->groupBox_common->setGeometry(30, 80, 991, 130);
        ui->groupBox_target->setGeometry(30, 221, 991, 471);
        ...
        on_comboBox_ChannelOutputCtrlM_currentIndexChanged(ui->comboBox_ChannelOutputCtrlT->currentIndex());
        ...
        ui->label_OutputElLevel->setGeometry(40,10,100,30);
        ui->lineEdit_OutputElLevel->setGeometry(140,10,85,30);
        ui->label_2->setGeometry(230,10,30,30);
        ui->label_BackZeroRate->setGeometry(40,45,65,30);
        ui->lineEdit_BackZeroRate->setGeometry(140,45,85,30);
        ui->label_14->setGeometry(230,45,30,30);
        //显示参数
        ShowPara(1);
    }
}
...
void SS2BBE_ModuParamSetGUIDlg::ShowControls()
{
    ...
    if (m_pDoppler->flag == true)
    {
        ...
        ui->comboBox_CompensateType->setCurrentIndex(m_pDoppler->compensateType - 2);
    ...
    //ui->pushButton_PlblicUnitModify->hide();
    //ui->pushButton_PublicUnitSet->hide();
}
...
void SS2BBE_ModuParamSetGUIDlg::ShowPara(int type)
{
    ...
    {
        //回零速率
        USHORT retZVel = m_MCSSSUpModulParam.upModPara[0].m_MCSTargetMeasSSUpModulParam.usRetZVel;
    ...
    //补偿方式
    // ui->comboBox_CompensateType->setCurrentIndex(m_MCSSSB2SSUpModulParam.usDoppCompType);
    //补偿周期
    // ui->lineEdit_CompensateTime->setText(QString::number(m_MCSSSB2SSUpModulParam.usDoppCompPrd));
...
void SS2BBE_ModuParamSetGUIDlg::SetObjInfo(TLxTsspObjectInfo ObjectInfo, DOPPLERCOMPENSATE& doppler)
{
    m_pDoppler = &doppler;
...
void SS2BBE_ModuParamSetGUIDlg::EnabledControls(bool bFlag)
{
    ...
    //测量支路多普勒预置频率
    ui->lineEdit_DopplerPredFreM->setEnabled(bFlag);
    //测量支路输出功率衰减
...
int SS2BBE_ModuParamSetGUIDlg::SaveToMacro(ILxTsspParameterMacro* pMacroObj)
{
    ...
    else
    {
        GetPara();
        //宏特殊处理会零速率
        m_MCSSSUpModulParam.upModPara[0].m_MCSTargetMeasSSUpModulParam.usRetZVel = ui->lineEdit_BackZeroRate->text().toInt();
        m_MCSSSUpModulParam.upModPara[0].m_MCSTargetTCSSUpModulParam.usRetZVel = ui->lineEdit_BackZeroRate->text().toInt();
        //取当前设置的参数
        QByteArray byteArray;
...
void SS2BBE_ModuParamSetGUIDlg::GetPara()
{
    ...
    //回零速率
    // m_MCSSSB2SSUpModulParam.usRetZVel = ui->lineEdit_BackZeroRate->text().toInt();
    //补偿方式
    //m_MCSSSB2SSUpModulParam.usDoppCompType = ui->comboBox_CompensateType->currentIndex();
    //补偿周期
    //m_MCSSSB2SSUpModulParam.usDoppCompPrd = ui->lineEdit_CompensateTime->text().toint();
}
...
int SS2BBE_ModuParamSetGUIDlg::CheckPara()
{
    ...
    QString strTmp;
    bool ok = false;
    UINT cFre = (uint)ui->lineEdit_CompensateTime->text().toFloat();
    ...
    {
        strTmp = QString("回零速率正确范围是:%1～%2Hz/s!")
                .arg(m_DlgParaSetStruct[0][5].MinVal.lmin).arg(m_DlgParaSetStruct[0][5].MaxVal.lmax);
        goto flag;
    }
    if (ui->checkBox_dopplerSw->isChecked())
    {
        if (ui->comboBox_CompensateType->currentIndex() != 0)
        {
            //补偿周期
            if(ui->lineEdit_CompensateTime->text().isEmpty())
            {
                strTmp = QString("补偿周期不能为空!");
                goto flag;
            }
            if(cFre < m_DlgParaSetStruct[0][7].MinVal.lmin
                    || cFre > m_DlgParaSetStruct[0][7].MaxVal.lmax)
            {
                strTmp = QString("补偿周期正确范围是:%1～%2ms!")
    ...
    //测量支路多普勒预置频率
    if(ui->lineEdit_DopplerPredFreM->text().isEmpty())
    {
        strTmp = QString("测量支路多普勒预置频率不能为空!");
        goto flag;
    }
    //测量支路输出功率微调
    if(ui->lineEdit_OutputPowerDecreaseM->text().isEmpty())
    {
        strTmp = QString("测量支路输出功率微调不能为空!");
        goto flag;
    }
    //测量支路上下行载波频差
    if(ui->lineEdit_UpdownFDiff->text().isEmpty())
    {
        strTmp = QString("测量支路上下行载波频差不能为空!");
        goto flag;
    }
    ...
    //测量支路多普勒预置频率
    if(ui->lineEdit_DopplerPredFreM->text().toInt() < m_DlgParaSetStruct[1][6].MinVal.lmin
            || ui->lineEdit_DopplerPredFreM->text().toInt() > m_DlgParaSetStruct[1][6].MaxVal.lmax)
    {
        strTmp = QString("测量支路多普勒预置频率正确范围是：%1～%2kHz!")
                .arg(m_DlgParaSetStruct[1][6].MinVal.lmin).arg(m_DlgParaSetStruct[1][6].MaxVal.lmax);
        goto flag;
    }
    //测量支路输出功率微调
    if(ui->lineEdit_OutputPowerDecreaseM->text().toInt() < m_DlgParaSetStruct[1][7].MinVal.lmin
            || ui->lineEdit_OutputPowerDecreaseM->text().toInt() > m_DlgParaSetStruct[1][7].MaxVal.lmax)
    {
        strTmp = QString("测量支路输出功率微调正确范围是：%1～%2dB!")
                .arg(m_DlgParaSetStruct[1][7].MinVal.lmin).arg(m_DlgParaSetStruct[1][7].MaxVal.lmax);
        goto flag;
    }
    //测量支路上下行载波频差
    if(ui->lineEdit_UpdownFDiff->text().toInt() < m_DlgParaSetStruct[1][8].MinVal.lmin
            || ui->lineEdit_UpdownFDiff->text().toInt() > m_DlgParaSetStruct[1][8].MaxVal.lmax)
    {
        strTmp = QString("测量支路上下行载波频差正确范围是：%1～%2kHz!")
                .arg(m_DlgParaSetStruct[1][8].MinVal.lmin).arg(m_DlgParaSetStruct[1][8].MaxVal.lmax);
        goto flag;
    }
    return 1;
flag:
...
void SS2BBE_ModuParamSetGUIDlg::showTargetParam(int nIndex, int nType)
{
    if (nType == 0)
    {
        ...
        //测量支路扩频码速率
        ui->comboBox_FreExpandRateM->clear();
        for(i=0; i<DlgMapLineNum[nIndex * 8 + 8];i++)
        {
            ui->comboBox_FreExpandRateM->addItem(m_DlgMapMean[nIndex * 8 + 8][i]);
        }
        ui->comboBox_FreExpandRateM->setCurrentIndex(m_DlgParaSetStruct[nIndex + 1][9].InitVal.lval);
        //测量支路输出控制
        ui->comboBox_ChannelOutputCtrlM->clear();
        for(i=0; i<DlgMapLineNum[nIndex * 8 + 9];i++)
        {
            ui->comboBox_ChannelOutputCtrlM->addItem(m_DlgMapMean[nIndex * 8 + 9][i]);
        }
        ui->comboBox_ChannelOutputCtrlM->setCurrentIndex(m_DlgParaSetStruct[nIndex + 1][10].InitVal.lval);
        //测量支路伪码加调
        ui->comboBox_VirtualAddM->clear();
        for(i=0; i<DlgMapLineNum[nIndex * 8 + 10];i++)
        {
            ui->comboBox_VirtualAddM->addItem(m_DlgMapMean[nIndex * 8 + 10][i]);
        }
        ui->comboBox_VirtualAddM->setCurrentIndex(m_DlgParaSetStruct[nIndex + 1][11].InitVal.lval);
        //测量支路数据加调
        ui->comboBox_DataAddM->clear();
        for(i=0; i<DlgMapLineNum[nIndex * 8 + 11];i++)
        {
            ui->comboBox_DataAddM->addItem(m_DlgMapMean[nIndex * 8 + 11][i]);
        }
        ui->comboBox_DataAddM->setCurrentIndex(m_DlgParaSetStruct[nIndex + 1][12].InitVal.lval);
        //往静态控件中写入参数缺省值
        //遥测支路多普勒预置频率
        ...
        ui->label_OutputPowerDecreaseT->setText("输出功率微调\n[" + QString::number(m_DlgParaSetStruct[nIndex + 1][1].MinVal.lmin) +
                "~" + QString::number(m_DlgParaSetStruct[nIndex + 1][1].MaxVal.lmax) + "]dB");
        //测量支路多普勒预置频率
        ui->lineEdit_DopplerPredFreM->setText(QString::number(m_DlgParaSetStruct[nIndex + 1][6].InitVal.lval));
        ui->label_DopplerPredFreM->setText("多普勒预置频率\n[" + QString::number(m_DlgParaSetStruct[nIndex + 1][6].MinVal.lmin) +
                "~" + QString::number(m_DlgParaSetStruct[nIndex + 1][6].MaxVal.lmax) + "]kHz");
        //测量支路输出功率衰减
        ui->lineEdit_OutputPowerDecreaseM->setText(QString::number(m_DlgParaSetStruct[nIndex + 1][7].InitVal.lval));
        ui->label_OutputPowerDecreaseM->setText("输出功率微调\n[" + QString::number(m_DlgParaSetStruct[nIndex + 1][7].MinVal.lmin) +
                "~" + QString::number(m_DlgParaSetStruct[nIndex + 1][7].MaxVal.lmax) + "]dB");
        //测量支路输上下行载波频差
        ui->lineEdit_UpdownFDiff->setText(QString::number(m_DlgParaSetStruct[nIndex + 1][8].InitVal.lval));
        ui->label_UpdownFDiff->setText("星上下行载波频差\n[" + QString::number(m_DlgParaSetStruct[nIndex + 1][8].MinVal.lmin) +
                "~" + QString::number(m_DlgParaSetStruct[nIndex + 1][8].MaxVal.lmax) + "]kHz");
    ...
    else if (nType == 1)
    {
        ...
        //测量支路扩频码速率
        if (m_MCSSSUpModulParam.upModPara[nIndex].m_MCSTargetMeasSSUpModulParam.ucPNRate > 0)
            ui->comboBox_FreExpandRateM->setCurrentIndex(m_MCSSSUpModulParam.upModPara[nIndex].m_MCSTargetMeasSSUpModulParam.ucPNRate - 1);
        //测量支路输出控制
        if (m_MCSSSUpModulParam.upModPara[nIndex].m_MCSTargetMeasSSUpModulParam.ucOutputSwitch > 0)
            ui->comboBox_ChannelOutputCtrlM->setCurrentIndex(m_MCSSSUpModulParam.upModPara[nIndex].m_MCSTargetMeasSSUpModulParam.ucOutputSwitch - 1);
        //测量支路伪码加调
        if (m_MCSSSUpModulParam.upModPara[nIndex].m_MCSTargetMeasSSUpModulParam.ucPNIModuSw > 0)
            ui->comboBox_VirtualAddM->setCurrentIndex(m_MCSSSUpModulParam.upModPara[nIndex].m_MCSTargetMeasSSUpModulParam.ucPNIModuSw - 1);
        //测量支路数据加调
        if (m_MCSSSUpModulParam.upModPara[nIndex].m_MCSTargetMeasSSUpModulParam.ucDataModuSw > 0)
            ui->comboBox_DataAddM->setCurrentIndex(m_MCSSSUpModulParam.upModPara[nIndex].m_MCSTargetMeasSSUpModulParam.ucDataModuSw - 1);
        ...
        //遥测支路输出功率衰减
        ui->lineEdit_OutputPowerDecreaseT->setText(QString::number(((char)m_MCSSSUpModulParam.upModPara[nIndex].m_MCSTargetTCSSUpModulParam.cPowAdjust)*(-1)));
        //测量支路多普勒预置频率
        ui->lineEdit_DopplerPredFreM->setText(QString::number(m_MCSSSUpModulParam.upModPara[nIndex].m_MCSTargetMeasSSUpModulParam.ucPreDplValue));
        //测量支路输出功率衰减
        ui->lineEdit_OutputPowerDecreaseM->setText(QString::number(((char)m_MCSSSUpModulParam.upModPara[nIndex].m_MCSTargetMeasSSUpModulParam.cPowAdjust)*(-1)));
        //测量支路输上下行载波频差
        ui->lineEdit_UpdownFDiff->setText(QString::number(m_MCSSSUpModulParam.upModPara[nIndex].m_MCSTargetMeasSSUpModulParam.sCarrFreqDif));
...
void SS2BBE_ModuParamSetGUIDlg::getParamTarget()
{
    ...
    //测量支路多普勒预置频率
    m_MCSSSUpModulParam.upModPara[m_nTargetNum].m_MCSTargetMeasSSUpModulParam.ucPreDplValue
            = ui->lineEdit_DopplerPredFreM->text().toInt();
    //测量支路输出功率衰减
    m_MCSSSUpModulParam.upModPara[m_nTargetNum].m_MCSTargetMeasSSUpModulParam.cPowAdjust
            = ui->lineEdit_OutputPowerDecreaseM->text().toInt()*-1;
    //测量支路扩频码速率
    m_MCSSSUpModulParam.upModPara[m_nTargetNum].m_MCSTargetMeasSSUpModulParam.ucPNRate
            = ui->comboBox_FreExpandRateM->currentIndex() + 1;
    //测量支路通道输出控制
    m_MCSSSUpModulParam.upModPara[m_nTargetNum].m_MCSTargetMeasSSUpModulParam.ucOutputSwitch
            = ui->comboBox_ChannelOutputCtrlM->currentIndex() + 1;
    //测量支路伪码加调
    m_MCSSSUpModulParam.upModPara[m_nTargetNum].m_MCSTargetMeasSSUpModulParam.ucPNIModuSw
            = ui->comboBox_VirtualAddM->currentIndex() + 1;
    //测量支路数据加调
    m_MCSSSUpModulParam.upModPara[m_nTargetNum].m_MCSTargetMeasSSUpModulParam.ucDataModuSw
            = ui->comboBox_DataAddM->currentIndex() + 1;
    //上下行频差
    m_MCSSSUpModulParam.upModPara[m_nTargetNum].m_MCSTargetMeasSSUpModulParam.sCarrFreqDif
            = ui->lineEdit_UpdownFDiff->text().toInt();
}
...
void SS2BBE_ModuParamSetGUIDlg::on_pushButton_Modify_clicked()
{
    if(ui->pushButton_Modify->text() == "更改")
    {
        ...
        //通道输出控制
        on_comboBox_ChannelOutputCtrlT_currentIndexChanged(ui->comboBox_ChannelOutputCtrlT->currentIndex());
        on_comboBox_ChannelOutputCtrlM_currentIndexChanged(ui->comboBox_ChannelOutputCtrlM->currentIndex());
        //        on_lineEdit_DopplerPredFreT_textChanged(ui->lineEdit_DopplerPredFreT->text());
...
//多普勒补偿
void SS2BBE_ModuParamSetGUIDlg::on_checkBox_clicked()
{
    if(ui->pushButton_PlblicUnitModify->text() == "更改")
        return ;
...
//补偿方式
void SS2BBE_ModuParamSetGUIDlg::on_comboBox_CompensateType_currentIndexChanged(int index)
{
    if(ui->pushButton_PlblicUnitModify->text() == "更改")
        return ;
    if(ui->checkBox_dopplerSw->checkState()){
...
//通道输出控制
void SS2BBE_ModuParamSetGUIDlg::on_comboBox_ChannelOutputCtrlM_currentIndexChanged(int index)
{
    if(ui->pushButton_Modify->text() == "更改")
        return;
    if(index==0){
        //测量支路多普勒预置频率
        ui->lineEdit_DopplerPredFreM->setEnabled(true);
        //测量支路输出功率衰减
        ui->lineEdit_OutputPowerDecreaseM->setEnabled(true);
        //测量支路扩频码速率
        ui->comboBox_FreExpandRateM->setEnabled(true);

        //测量支路伪码加调
        ui->comboBox_VirtualAddM->setEnabled(true);
        //测量支路数据加调
        ui->comboBox_DataAddM->setEnabled(true);
        //上下行载波频率差
        ui->lineEdit_UpdownFDiff->setEnabled(true);
    }
    else{
        //测量支路多普勒预置频率
        ui->lineEdit_DopplerPredFreM->setEnabled(false);
        //测量支路输出功率衰减
        ui->lineEdit_OutputPowerDecreaseM->setEnabled(false);
        //测量支路扩频码速率
        ui->comboBox_FreExpandRateM->setEnabled(false);

        //测量支路伪码加调
        ui->comboBox_VirtualAddM->setEnabled(false);
        //测量支路数据加调
        ui->comboBox_DataAddM->setEnabled(false);
        //上下行载波频率差
        ui->lineEdit_UpdownFDiff->setEnabled(false);
    }
}
...
//            switch (ui->comboBox_CompensateType->currentIndex()) {
//            case 2:
//                dopplerCom.compensateType = 2;
//                break;
//            case 0:
//                dopplerCom.compensateType = 3;
//                break;
//            case 1:
//                dopplerCom.compensateType = 1;
//            default:
//                break;
//            }
            dopplerCom.compensateType = ui->comboBox_CompensateType->currentIndex() + 2;//补偿方式
            dopplerCom.compensatePeriod = qRound(1000.0/ui->lineEdit_CompensateTime->text().toInt());//补偿周期
...
void SS2BBE_ModuParamSetGUIDlg::on_checkBox_dopplerSw_clicked()
{
    on_checkBox_clicked();
}

void SS2BBE_ModuParamSetGUIDlg::on_checkBox_dopplerSw_toggled(bool checked)
{
...
//void SS2BBE_ModuParamSetGUIDlg::on_lineEdit_DopplerPredFreT_textChanged(const QString &arg1)
//{
//    ui->lineEdit_DopplerPredFreM->setText(arg1);
//}
