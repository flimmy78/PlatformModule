#include "SS2BBE_ModuParamSetGUI.h"
...
namespace
{
    SS2BBE_ModuParamSetGUIDlg* g_form = NULL;
}
extern quint64 GetID(ushort usStationID, uchar ucSubSysID, ushort usDeviceID, ushort usType,
              uchar ucSn);
SS2BBE_ModuParamSetGUI::SS2BBE_ModuParamSetGUI(TLxTsspObjectInfo ObjectInfo)
{
    //初始化
 ...
 * @return 界面对象的界面。
*/
QWidget* SS2BBE_ModuParamSetGUI::ShowMe(QWidget* parent, const QString& strTitle)
{
    m_strTitle = strTitle;

    SS2BBE_ModuParamSetGUIDlg* form;

    form = new SS2BBE_ModuParamSetGUIDlg(parent);

    //设置对象配置信息
    form->SetObjInfo(m_ObjectInfo, m_doppler);
    //设置界面信息
 ...
 * @return szHelpID:长度。
*/
int SS2BBE_ModuParamSetGUI::GetContextHelpID(char* szHelpID)
{
...
*/
int SS2BBE_ModuParamSetGUI::GetModuleInfo(TLxTsspModuleInfo& moduleInfo) const
{
    ...
    /// 配置标识，如：D_Y17-61_008
    strcpy(moduleInfo.szConfigID, "D_Y17-92_122\0");
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS2BBE_ModuParamSetGUI::GetObjectInfo(TLxTsspObjectInfo& ObjectInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS2BBE_ModuParamSetGUI::PreInitialize()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS2BBE_ModuParamSetGUI::Initialize()
{
    memset(&m_doppler, 0, sizeof(m_doppler));//fix me
    //日志信息
    TLxTsspLog tmpLog;
    QString strLog;
    ...
    stCurPath = stCurPath + "/System/ModuleConfig/SS2BBE_ModuParamSetGUI.xml";
    //遍历xml文件，获取参数取值范围
    m_MapSumFlag = 0;
...
//遍历xml文件，获取参数取值范围
int SS2BBE_ModuParamSetGUI::QueryParaBlock(QString ConfigPath)
{
...
int SS2BBE_ModuParamSetGUI::GetAllMapValueStringByPara(int MapSum,QDomNode pInDocNode_Para)
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS2BBE_ModuParamSetGUI::Start()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS2BBE_ModuParamSetGUI::Stop()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
bool SS2BBE_ModuParamSetGUI::ClearUp()
{
    return 1;
}
 ...
 *           其它错误依情况增加；
 */
int SS2BBE_ModuParamSetGUI::GetErrorNumber()
{
    return m_iErrCode;
}
 ...
 */
int SS2BBE_ModuParamSetGUI::GetErrorMsg(int iErrorNo, QString& strErrorMsg)
{
 ...
 */
bool SS2BBE_ModuParamSetGUI::IsOnline()
{
    //对象初始化
    //查找对象管理器
    LxTsspObjectManager* pObjectManager = LxTsspObjectManager::Instance();
 ...
 */
void SS2BBE_ModuParamSetGUI::SendMessage(TLxTsspMessage msg)
{
    return;
}
 ...
 * @return 1：成功；-1：失败。
 */
int SS2BBE_ModuParamSetGUI::SetParameter(QMap<quint64, QByteArray> parameterList)
{
 ...
 */
int SS2BBE_ModuParamSetGUI::SetParameter(quint64 ui64ParamID, QByteArray parameter)
{
 ...
 */
int SS2BBE_ModuParamSetGUI::GetParameterID(QList<quint64>& pParameterIDList) const
{
 ...
 * @return 大于等于0：参数块个数。-1：失败。
 */
int SS2BBE_ModuParamSetGUI::GetParameter(QMap<quint64, QByteArray>& parameterList) const
{
    SMCU2::MCSUpModulParamStruct4 tmpModuPara;       //码同步参数

    SMCU2::MCSUpModuleStatusStruct4* tmpModuStatus = NULL;      //码同步参数
    ...
    if(tmpStateMgr != NULL)
        tmpModuStatus  =
                (SMCU2::MCSUpModuleStatusStruct4*)tmpStateMgr->FindOneItem(m_StatusComSynID, stLen);

    if(tmpModuStatus != NULL)
    {
        tmpModuPara.m_MCSSSUpModulParam = tmpModuStatus->m_MCSSSUpModulStatus.tParams;
        for (int i = 0; i < MAX_TARGET_COUNT; i++)
        {
            tmpModuPara.upModPara[i].m_MCSTargetTCSSUpModulParam = tmpModuStatus->TcMeas[i].m_MCSTargetTCSSUpModulStatus.tParams;
            tmpModuPara.upModPara[i].m_MCSTargetMeasSSUpModulParam = tmpModuStatus->TcMeas[i].m_MCSTargetMeasSSUpModulStatus.tParams;
        }
    }
    QByteArray  tmpCh;
    tmpCh.resize(sizeof(SMCU2::MCSUpModulParamStruct4));
    memcpy(tmpCh.data(), &tmpModuPara, sizeof(SMCU2::MCSUpModulParamStruct4));
 ...
 * @return 1：成功；-1：失败。
 */
int SS2BBE_ModuParamSetGUI::GetParameter(quint64 ui64ParamID, QByteArray& parameter) const
{
    if(m_ParamList.contains(ui64ParamID))
    {
 ...
 */
int SS2BBE_ModuParamSetGUI::GetState(QMap<quint64, TLxTsspDeviceState>& stateList) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int SS2BBE_ModuParamSetGUI::GetState(quint64 ui64StatusID, TLxTsspDeviceState& status) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int SS2BBE_ModuParamSetGUI::GetStateDirect(quint64 ui64StatusID, TLxTsspDeviceState& status)
{
    if(m_StateList.contains(ui64StatusID))
    {
        //
 ...
 */
int SS2BBE_ModuParamSetGUI::WriteData(quint64 ui64DataID, uchar* aData, uint uiDataLen)
{
    return 0;
}
 ...
 */
int SS2BBE_ModuParamSetGUI::ReadData(quint64 ui64DataID, uchar* aData, uint& uiDataLen,
                             uint& uiReloadCount)
{
    return 0;
}

int SS2BBE_ModuParamSetGUI::CreateDefaultMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    SMCU2::MCSUpModulParamStruct4 tmpModuPara;
    //载波输出电平
    tmpModuPara.m_MCSSSUpModulParam.cWaveOutLevel = m_ParaSetStruct[0][0].InitVal.lval;
    //调制方式
    tmpModuPara.m_MCSSSUpModulParam.ucModulate = m_ParaSetStruct[0][1].InitVal.lval + 1;
    //载波控制
    tmpModuPara.m_MCSSSUpModulParam.ucWaveOutCtrl = m_ParaSetStruct[0][2].InitVal.lval + 1;
    //噪声源输出
    tmpModuPara.m_MCSSSUpModulParam.ucNoiseSw = m_ParaSetStruct[0][3].InitVal.lval + 1;
    //载波输出载噪比
    tmpModuPara.m_MCSSSUpModulParam.ucCN0 = m_ParaSetStruct[0][4].InitVal.lval;
    for (int i = 0; i < MAX_TARGET_COUNT; i++)
    {
        //遥测支路多普勒预置频率
        tmpModuPara.upModPara[i].m_MCSTargetTCSSUpModulParam.ucPreDplValue = m_ParaSetStruct[i + 1][0].InitVal.lval;
        //遥测支路输出功率微调
        tmpModuPara.upModPara[i].m_MCSTargetTCSSUpModulParam.cPowAdjust = -1*(char)m_ParaSetStruct[i + 1][1].InitVal.lval;
        //遥测支路扩频码速率
        tmpModuPara.upModPara[i].m_MCSTargetTCSSUpModulParam.ucPNRate = m_ParaSetStruct[i + 1][2].InitVal.lval + 1;
        //遥测支路通道输出控制
        tmpModuPara.upModPara[i].m_MCSTargetTCSSUpModulParam.ucOutputSwitch = m_ParaSetStruct[i + 1][3].InitVal.lval + 1;
        //遥测支路伪码加调
        tmpModuPara.upModPara[i].m_MCSTargetTCSSUpModulParam.ucPNIModuSw = m_ParaSetStruct[i + 1][4].InitVal.lval + 1;
        //遥测支路数据加调
        tmpModuPara.upModPara[i].m_MCSTargetTCSSUpModulParam.ucDataModuSw = m_ParaSetStruct[i + 1][5].InitVal.lval + 1;

        //测量支路多普勒预置频率
        tmpModuPara.upModPara[i].m_MCSTargetMeasSSUpModulParam.ucPreDplValue = m_ParaSetStruct[i + 1][6].InitVal.lval;
        //测量支路输出功率微调
        tmpModuPara.upModPara[i].m_MCSTargetMeasSSUpModulParam.cPowAdjust = -1*(char)m_ParaSetStruct[i + 1][7].InitVal.lval;
        //测量支路扩频码速率
        tmpModuPara.upModPara[i].m_MCSTargetMeasSSUpModulParam.ucPNRate = m_ParaSetStruct[i + 1][9].InitVal.lval + 1;
        //测量支路通道输出控制
        tmpModuPara.upModPara[i].m_MCSTargetMeasSSUpModulParam.ucOutputSwitch = m_ParaSetStruct[i + 1][10].InitVal.lval + 1;
        //测量支路伪码加调
        tmpModuPara.upModPara[i].m_MCSTargetMeasSSUpModulParam.ucPNIModuSw = m_ParaSetStruct[i + 1][11].InitVal.lval + 1;
        //测量支路数据加调
        tmpModuPara.upModPara[i].m_MCSTargetMeasSSUpModulParam.ucDataModuSw = m_ParaSetStruct[i + 1][12].InitVal.lval + 1;
        //测量支路上下行载波频差
        tmpModuPara.upModPara[i].m_MCSTargetMeasSSUpModulParam.sCarrFreqDif = m_ParaSetStruct[i + 1][8].InitVal.lval;

    }
    //取当前设置的参数，存入框架
    QByteArray byteArray;
    byteArray.resize(sizeof(tmpModuPara));
    memcpy(byteArray.data(),&tmpModuPara,sizeof(tmpModuPara));
    if(pMacroObj!=NULL)
    {
...
QWidget* SS2BBE_ModuParamSetGUI::ShowMeInEdit(QWidget* parent,uchar ucType,ILxTsspParameterMacro* pMacroObj)
{
    g_form = new SS2BBE_ModuParamSetGUIDlg(parent);

    g_form->SetObjInfo(m_ObjectInfo, m_doppler);
...
int SS2BBE_ModuParamSetGUI::SaveToMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    if(g_form!=NULL)
    {
