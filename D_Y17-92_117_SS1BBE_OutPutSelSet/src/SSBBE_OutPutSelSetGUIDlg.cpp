#include "SSBBE_OutPutSelSetGUIDlg.h"
#include "ui_SSBBE_OutPutSelSetGUIDlg.h"
#include <QCloseEvent>
#include <QMessageBox>
#include <QDateTime>
...
extern quint64 GetID(ushort usStationID, uchar ucSubSysID, ushort usDeviceID, ushort usType,
                     uchar ucSn);
SSBBE_OutPutSelSetGUIDlg::SSBBE_OutPutSelSetGUIDlg(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SSBBE_OutPutSelSetGUIDlg)
{
    ui->setupUi(this);
    m_pStateMgr = NULL;
...
void SSBBE_OutPutSelSetGUIDlg::updateTarget()
{
...
SSBBE_OutPutSelSetGUIDlg::~SSBBE_OutPutSelSetGUIDlg()
{
    killTimer(m_TimerID);
    m_timer1->stop();
    delete m_timer1;
...
void SSBBE_OutPutSelSetGUIDlg::setText(const QString& strTitle, ParaStruct xmlsetText[3][60], QString MapMean[50][30], int MapNum[50], uchar Type, ILxTsspParameterMacro* pMacroObj)
{
    m_strTitle = strTitle;
...
//初始化对话框
void SSBBE_OutPutSelSetGUIDlg::InitDLG(int Type, ILxTsspParameterMacro* pMacroObj)
{
    ...
    if(Type == 0)//取当前参数
    {
        ...
        if(m_pStateMgr != NULL)
        {
            ...
            m_MCSSysStatus = (SMCU::MCSSysTarget4StatusStruct*)m_pStateMgr->FindOneItem(m_StatusParaSysID, stLen);
            if(m_MCSSysStatus == NULL || stLen != sizeof(SMCU::MCSSysTarget4StatusStruct))
                return;
        }
        //启动定时器
    ...
    else if(Type == 1)//取宏里参数
    {
        ...
        if(pMacroObj != NULL)
        {
            ...
            memcpy(&m_MCSSysParam, byteArray.data(), sizeof(SMCU::MCSSysTarget4ParamStruct));
        }
...
//设置输入格式
void SSBBE_OutPutSelSetGUIDlg::InitInput()
{
...
void SSBBE_OutPutSelSetGUIDlg::ShowControls()
{
    int i;
    //输出方式
...
//显示参数
void SSBBE_OutPutSelSetGUIDlg::ShowPara(int type)
{
    if(type == 0)       //参数设置
    {
        if(m_MCSSysStatus != NULL)
        {
            m_MCSSysParam.sysParam = m_MCSSysStatus->sysStatu.tParams;
        }
        else
        {
            DWORD stLen = sizeof(SMCU::MCSSysTarget4StatusStruct);
    ...
    //输出方式
    if (m_MCSSysParam.sysParam.ucTrackOutType > 0)
        ui->comboBox_OutputMd->setCurrentIndex(m_MCSSysParam.sysParam.ucTrackOutType - 1);
    //输出通道
    ui->comboBox_OutputChannel->setEnabled(false);
    //输出目标
    updateOutTargets();
...
//设置帧长范围
void SSBBE_OutPutSelSetGUIDlg::SetFrameLenRange(int iWordLen)
{
    /* if(iWordLen ==8)
    {
...
void SSBBE_OutPutSelSetGUIDlg::SetObjInfo(TLxTsspObjectInfo ObjectInfo)
{
...
//使能或不使能控件
void SSBBE_OutPutSelSetGUIDlg::EnabledControls(bool bFlag)
{
    /*************************Group1*************************/
    //输出方式
    ...
    //保持门限
    ui->lineEdit_KeepThresold->setEnabled(bFlag);
    //判决时间
    ui->lineEdit_ProPeriod->setEnabled(bFlag);
    //输出门限
    ui->lineEdit_OutputThresold->setEnabled(bFlag);
    /*************************Group1*************************/
...
//保存参数宏
int SSBBE_OutPutSelSetGUIDlg::SaveToMacro(ILxTsspParameterMacro* pMacroObj)
{
    //判断合法性
    int iRet = CheckPara();
    if(iRet == -1)
    ...
    else
    {
        ...
        QString strLog;
        if(-1 == pMacroObj->UpdateParameterBlock(m_ParaSysID, byteArray))
            strLog += QString("%1: 保存宏失败！").arg(MODULENAME);
        else
...
//从界面取参数
void SSBBE_OutPutSelSetGUIDlg::GetPara()
{
    //输出方式
    m_MCSSysParam.sysParam.ucTrackOutType = ui->comboBox_OutputMd->currentIndex() + 1;
    //输出目标
    m_MCSSysParam.sysParam.ucTrackDefCH = ui->comboBox_OutputTarget->currentIndex() + 1;
    //输出通道
    m_MCSSysParam.sysParam.ucTrackChnlSel = ui->comboBox_OutputChannel->currentIndex() + 1;
}

//验证参数范围
int SSBBE_OutPutSelSetGUIDlg::CheckPara()
{
    QMessageBox msgBox;
    /*//距离预报值
...
//定时器
void SSBBE_OutPutSelSetGUIDlg::timerEvent(QTimerEvent *event)
{
    ShowPara(0);
}

//更改参数按钮休息处理函数
void SSBBE_OutPutSelSetGUIDlg::on_pushButton_Modify_clicked()
{
    if(ui->pushButton_Modify->text() == "更改")
    {
...
void SSBBE_OutPutSelSetGUIDlg::on_pushButton_Set_clicked()
{
    //验证参数范围
    if(CheckPara() == 1)
    {
        ...
        if(m_pHAManager != NULL)
        {
            ...
            data.target = 1;
            data.sys=m_MCSSysParam.sysParam;
...
void SSBBE_OutPutSelSetGUIDlg::closeEvent(QCloseEvent *event)
{
    event->accept();
}

QSize SSBBE_OutPutSelSetGUIDlg::sizeHint() const
{
    return QSize(this->width(), this->height());
}

void SSBBE_OutPutSelSetGUIDlg::SubFrmEn(bool bFlag)
{
    //副帧长度
...
//有无ID副帧
void SSBBE_OutPutSelSetGUIDlg::IDSubFrmEn(bool bFlag)
{
...
//有无循环副帧
void SSBBE_OutPutSelSetGUIDlg::CycSubFrmEn(bool bFlag)
{
    //循环副帧
...
//有无反码副帧
void SSBBE_OutPutSelSetGUIDlg::ReserveSubFrmEn(bool bFlag)
{

}

void SSBBE_OutPutSelSetGUIDlg::SendMessage(char* pData, unsigned short  usLen, unsigned short usC, unsigned short usTID)
{
    static uint seq = 0;
...
DWORDLONG SSBBE_OutPutSelSetGUIDlg::StrToSynCode(const char * pStr)
{
    char c,C;
    const char*  p;
...
//输出方式
void SSBBE_OutPutSelSetGUIDlg::on_comboBox_OutputMd_currentIndexChanged(int index)
{
    if(ui->pushButton_Modify->text() == "更改")
        return;
    if(index==0){
        ui->comboBox_OutputTarget->setEnabled(true);
        ...
        ui->comboBox_OutputTarget->setEnabled(false);
        ui->lineEdit_KeepThresold->setEnabled(true);
        ui->lineEdit_ProPeriod->setEnabled(true);
        ui->lineEdit_OutputThresold->setEnabled(true);
...
//输出目标
void SSBBE_OutPutSelSetGUIDlg::on_comboBox_OutputTarget_currentIndexChanged(int index)
{
    if(ui->pushButton_Modify->text() == "更改")
        return;
}

void SSBBE_OutPutSelSetGUIDlg::updateOutTargets()
{
    int target = m_MCSSysStatus ? m_MCSSysStatus->sysStatu.tParams.ucTargetNum : 4;
    ui->comboBox_OutputTarget->clear();
    switch (target) {
    case 2:
        ui->comboBox_OutputTarget->addItem(m_strListOutTargets[0]);
    ...
    target = m_MCSSysParam.sysParam.ucTrackDefCH;
