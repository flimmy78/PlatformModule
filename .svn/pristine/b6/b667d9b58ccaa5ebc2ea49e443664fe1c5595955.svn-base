...
namespace
{
    TestNet_SwitchGUIDlg* g_form = NULL;
}
extern quint64 GetID(ushort usStationID, uchar ucSubSysID, ushort usDeviceID, ushort usType,
              uchar ucSn);
TestNet_SwitchGUI::TestNet_SwitchGUI(TLxTsspObjectInfo ObjectInfo)
{
    //初始化
 ...
 * @return 界面对象的界面。
*/
QWidget* TestNet_SwitchGUI::ShowMe(QWidget* parent, const QString& strTitle)
{
    m_strTitle = strTitle;

    TestNet_SwitchGUIDlg* form;

    form = new TestNet_SwitchGUIDlg(parent);
 ...
 * @return szHelpID:长度。
*/
int TestNet_SwitchGUI::GetContextHelpID(char* szHelpID)
{
...
*/
int TestNet_SwitchGUI::GetModuleInfo(TLxTsspModuleInfo& moduleInfo) const
{
    ...
    /// 配置标识，如：D_Y17-61_008
    strcpy(moduleInfo.szConfigID, "D_Y17-92_126\0");
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int TestNet_SwitchGUI::GetObjectInfo(TLxTsspObjectInfo& ObjectInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int TestNet_SwitchGUI::PreInitialize()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int TestNet_SwitchGUI::Initialize()
{
    //日志信息
    TLxTsspLog tmpLog;
    QString strLog;
    ...
    //20170317 没有此模块xml配置将其注释
    /*QString stCurPath = QDir::currentPath();
    stCurPath = stCurPath + "/System/ModuleConfig/Net_SwitchGUI.xml";
    //遍历xlm文件，获取参数取值范围
    m_MapSumFlag = 0;
...
//遍历xml文件，获取参数取值范围
int TestNet_SwitchGUI::QueryParaBlock(QString ConfigPath)
{
...
int TestNet_SwitchGUI::GetAllMapValueStringByPara(int MapSum,QDomNode pInDocNode_Para)
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
bool TestNet_SwitchGUI::ClearUp()
{
    return 1;
}
 ...
 *           其它错误依情况增加；
 */
int TestNet_SwitchGUI::GetErrorNumber()
{
    return m_iErrCode;
}
 ...
 */
int TestNet_SwitchGUI::GetErrorMsg(int iErrorNo, QString& strErrorMsg)
{
 ...
 */
bool TestNet_SwitchGUI::IsOnline()
{
    //对象初始化
    //查找对象管理器
    LxTsspObjectManager* pObjectManager = LxTsspObjectManager::Instance();
 ...
 */
void TestNet_SwitchGUI::SendMessage(TLxTsspMessage msg)
{
    return;
}
 ...
 * @return 1：成功；-1：失败。
 */
int TestNet_SwitchGUI::SetParameter(QMap<quint64, QByteArray> parameterList)
{
 ...
 */
int TestNet_SwitchGUI::SetParameter(quint64 ui64ParamID, QByteArray parameter)
{
 ...
 */
int TestNet_SwitchGUI::GetParameterID(QList<quint64>& pParameterIDList) const
{
 ...
 * @return 大于等于0：参数块个数。-1：失败。
 */
int TestNet_SwitchGUI::GetParameter(QMap<quint64, QByteArray>& parameterList) const
{
    TEST_SWMATPara tmpComPara;          //参数

    TEST_SWMATStatu *tmpComStatus = NULL;  //状态
    ...
    if(tmpStateMgr != NULL)
        tmpComStatus  =
                (TEST_SWMATStatu*)tmpStateMgr->FindOneItem(m_StatusComSynID, stLen);
    ...
    QByteArray tmpCh;
    tmpCh.resize(sizeof(TEST_SWMATPara));
    memcpy(tmpCh.data(), &tmpComPara, sizeof(TEST_SWMATPara));
 ...
 * @return 1：成功；-1：失败。
 */
int TestNet_SwitchGUI::GetParameter(quint64 ui64ParamID, QByteArray& parameter) const
{
    if(m_ParamList.contains(ui64ParamID))
    {
 ...
 */
int TestNet_SwitchGUI::GetState(QMap<quint64, TLxTsspDeviceState>& stateList) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int TestNet_SwitchGUI::GetState(quint64 ui64StatusID, TLxTsspDeviceState& status) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int TestNet_SwitchGUI::GetStateDirect(quint64 ui64StatusID, TLxTsspDeviceState& status)
{
    if(m_StateList.contains(ui64StatusID))
    {
        //
 ...
 */
int TestNet_SwitchGUI::WriteData(quint64 ui64DataID, uchar* aData, uint uiDataLen)
{
    return 0;
}
 ...
 */
int TestNet_SwitchGUI::ReadData(quint64 ui64DataID, uchar* aData, uint& uiDataLen,
                             uint& uiReloadCount)
{
    return 0;
}

int TestNet_SwitchGUI::CreateDefaultMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    TEST_SWMATPara comPara;

    //取当前设置的参数，存入框架
    QByteArray byteArray;
    byteArray.resize(sizeof(TEST_SWMATPara));
    memcpy(byteArray.data(), &comPara, sizeof(TEST_SWMATPara));
    if(pMacroObj != NULL)
    {
...
QWidget* TestNet_SwitchGUI::ShowMeInEdit(QWidget* parent,uchar ucType,ILxTsspParameterMacro* pMacroObj)
{
    g_form = new TestNet_SwitchGUIDlg(parent);
...
int TestNet_SwitchGUI::SaveToMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    if(g_form != NULL)
    {
