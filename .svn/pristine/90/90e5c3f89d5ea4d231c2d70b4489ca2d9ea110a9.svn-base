 ...
 * @return 界面对象的界面。
*/
QWidget* USBRSPDcuGUI::ShowMe(QWidget* parent, const QString& strTitle)
{
    m_strTitle = strTitle;

    USBRSPDcuGUIDlg* form;
    form = new USBRSPDcuGUIDlg(parent);
 ...
 * @return szHelpID:长度。
*/
int USBRSPDcuGUI::GetContextHelpID(char* szHelpID)
{
...
*/
int USBRSPDcuGUI::GetModuleInfo(TLxTsspModuleInfo& moduleInfo) const
{
    ...
    /// 配置标识，如：D_Y17-61_008
    strcpy(moduleInfo.szConfigID, "D_Y17-92_130\0");
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int USBRSPDcuGUI::GetObjectInfo(TLxTsspObjectInfo& ObjectInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int USBRSPDcuGUI::PreInitialize()
{
    return 1;
}
...
int USBRSPDcuGUI::Initialize()
{
    ...
    stCurPath = stCurPath + "/System/ModuleConfig/USBRSPDcuGUI.xml";
    //遍历xlm文件，获取参数取值范围
    m_MapSumFlag = 0;
...
//遍历xlm文件，获取参数取值范围
int USBRSPDcuGUI::QueryParaBlock(QString ConfigPath)
{
...
int USBRSPDcuGUI::GetAllMapValueStringByPara(int MapSum,QDomNode pInDocNode_Para)
{
 ...
 *           其它错误依情况增加；
 */
int USBRSPDcuGUI::GetErrorNumber()
{
    return m_iErrCode;
}
 ...
 */
int USBRSPDcuGUI::GetErrorMsg(int iErrorNo, QString& strErrorMsg)
{
 ...
 */
void USBRSPDcuGUI::SendMessage(TLxTsspMessage msg)
{
    return;
}
 ...
 * @return 1：成功；-1：失败。
 */
int USBRSPDcuGUI::SetParameter(QMap<quint64, QByteArray> parameterList)
{
 ...
 */
int USBRSPDcuGUI::SetParameter(quint64 ui64ParamID, QByteArray parameter)
{
 ...
 */
int USBRSPDcuGUI::GetParameterID(QList<quint64>& pParameterIDList) const
{
 ...
 * @return 大于等于0：参数块个数。-1：失败。
 */
int USBRSPDcuGUI::GetParameter(QMap<quint64, QByteArray>& parameterList) const
{
    COMBTRANSPara tmpComPara;          //参数

    COMBTRANSStatu *tmpComStatus = NULL;  //状态参数
    ...
    if(tmpStateMgr != NULL)
        tmpComStatus  =
                (COMBTRANSStatu*)tmpStateMgr->FindOneItem(m_StatusComSynID, stLen);
    ...
    QByteArray  tmpCh;
    tmpCh.resize(sizeof(COMBTRANSPara));
    memcpy(tmpCh.data(), tmpComStatus, sizeof(COMBTRANSPara));
 ...
 * @return 1：成功；-1：失败。
 */
int USBRSPDcuGUI::GetParameter(quint64 ui64ParamID, QByteArray& parameter) const
{
    if(m_ParamList.contains(ui64ParamID))
    {
 ...
 */
int USBRSPDcuGUI::GetState(QMap<quint64, TLxTsspDeviceState>& stateList) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int USBRSPDcuGUI::GetState(quint64 ui64StatusID, TLxTsspDeviceState& status) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int USBRSPDcuGUI::GetStateDirect(quint64 ui64StatusID, TLxTsspDeviceState& status)
{
    if(m_StateList.contains(ui64StatusID))
    {
        //
 ...
 */
int USBRSPDcuGUI::WriteData(quint64 ui64DataID, uchar* aData, uint uiDataLen)
{
    return 0;
}
 ...
 */
int USBRSPDcuGUI::ReadData(quint64 ui64DataID, uchar* aData, uint& uiDataLen,
                             uint& uiReloadCount)
{
    return 0;
}

int USBRSPDcuGUI::CreateDefaultMacroObj(ILxTsspParameterMacro* pMacroObj)
{
...
QWidget* USBRSPDcuGUI::ShowMeInEdit(QWidget* parent,uchar ucType,ILxTsspParameterMacro* pMacroObj)
{
    g_form = new USBRSPDcuGUIDlg(parent);
...
int USBRSPDcuGUI::SaveToMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    if(g_form != NULL)
    {
