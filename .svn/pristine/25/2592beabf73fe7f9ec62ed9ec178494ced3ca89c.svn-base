...
namespace
{
    SSBBE_SysParamSetGUIDlg* g_form = NULL;
}
extern quint64 GetID(ushort usStationID, uchar ucSubSysID, ushort usDeviceID, ushort usType,
              uchar ucSn);
SSBBE_SysParamSetGUI::SSBBE_SysParamSetGUI(TLxTsspObjectInfo ObjectInfo)
{
    //初始化
 ...
 * @return 界面对象的界面。
*/
QWidget* SSBBE_SysParamSetGUI::ShowMe(QWidget* parent, const QString& strTitle)
{
    m_strTitle = strTitle;

    SSBBE_SysParamSetGUIDlg* form;

    form = new SSBBE_SysParamSetGUIDlg(parent);
 ...
 * @return szHelpID:长度。
*/
int SSBBE_SysParamSetGUI::GetContextHelpID(char* szHelpID)
{
...
*/
int SSBBE_SysParamSetGUI::GetModuleInfo(TLxTsspModuleInfo& moduleInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SSBBE_SysParamSetGUI::GetObjectInfo(TLxTsspObjectInfo& ObjectInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SSBBE_SysParamSetGUI::PreInitialize()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SSBBE_SysParamSetGUI::Initialize()
{
    //日志信息
    TLxTsspLog tmpLog;
    QString strLog;
    ...
    stCurPath = stCurPath + "/System/ModuleConfig/SSBBE_SysParamSetGUI.xml";
    //遍历xml文件，获取参数取值范围
    m_MapSumFlag = 0;
...
//遍历xml文件，获取参数取值范围
int SSBBE_SysParamSetGUI::QueryParaBlock(QString ConfigPath)
{
...
int SSBBE_SysParamSetGUI::GetAllMapValueStringByPara(int MapSum, QDomNode pInDocNode_Para)
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SSBBE_SysParamSetGUI::Start()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
bool SSBBE_SysParamSetGUI::ClearUp()
{
    return 1;
}
 ...
 *           其它错误依情况增加；
 */
int SSBBE_SysParamSetGUI::GetErrorNumber()
{
    return m_iErrCode;
}
 ...
 */
int SSBBE_SysParamSetGUI::GetErrorMsg(int iErrorNo, QString& strErrorMsg)
{
 ...
 */
bool SSBBE_SysParamSetGUI::IsOnline()
{
    //对象初始化
    //查找对象管理器
    LxTsspObjectManager* pObjectManager = LxTsspObjectManager::Instance();
 ...
 */
void SSBBE_SysParamSetGUI::SendMessage(TLxTsspMessage msg)
{
    return;
}
 ...
 * @return 1：成功；-1：失败。
 */
int SSBBE_SysParamSetGUI::SetParameter(QMap<quint64, QByteArray> parameterList)
{
 ...
 */
int SSBBE_SysParamSetGUI::SetParameter(quint64 ui64ParamID, QByteArray parameter)
{
 ...
 */
int SSBBE_SysParamSetGUI::GetParameterID(QList<quint64>& pParameterIDList) const
{
 ...
 * @return 大于等于0：参数块个数。-1：失败。
 */
int SSBBE_SysParamSetGUI::GetParameter(QMap<quint64, QByteArray>& parameterList) const
{
    SMCU::MCSSysTarget4ParamStruct tmpComPara;          //分集合成参数

    SMCU::MCSSysTarget4StatusStruct *tmpComParamStatus = NULL;  //分集合成状态参数
    ...
    if(tmpStateMgr != NULL)
        tmpComParamStatus  =
                (SMCU::MCSSysTarget4StatusStruct*)tmpStateMgr->FindOneItem(m_StatusComSynID, stLen);

    if(tmpComParamStatus != NULL)
    {
        tmpComPara.sysParam = tmpComParamStatus->sysStatu.tParams;
        for (int i = 0; i< MAX_TARGET_COUNT; i++)
            tmpComPara.targetSysParam[i] = tmpComParamStatus->targetSysStatu[i].tParams;
    }

    QByteArray  tmpCh;
    tmpCh.resize(sizeof(SMCU::MCSSysTarget4ParamStruct));
    memcpy(tmpCh.data(), &tmpComPara, sizeof(SMCU::MCSSysTarget4ParamStruct));
 ...
 * @return 1：成功；-1：失败。
 */
int SSBBE_SysParamSetGUI::GetParameter(quint64 ui64ParamID, QByteArray& parameter) const
{
    if(m_ParamList.contains(ui64ParamID))
    {
 ...
 */
int SSBBE_SysParamSetGUI::GetState(QMap<quint64, TLxTsspDeviceState>& stateList) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int SSBBE_SysParamSetGUI::GetState(quint64 ui64StatusID, TLxTsspDeviceState& status) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int SSBBE_SysParamSetGUI::GetStateDirect(quint64 ui64StatusID, TLxTsspDeviceState& status)
{
    if(m_StateList.contains(ui64StatusID))
    {
        //
 ...
 */
int SSBBE_SysParamSetGUI::WriteData(quint64 ui64DataID, uchar* aData, uint uiDataLen)
{
    return 0;
}
 ...
 */
int SSBBE_SysParamSetGUI::ReadData(quint64 ui64DataID, uchar* aData, uint& uiDataLen,
                             uint& uiReloadCount)
{
    return 0;
}

TLxTsspParamMacroInformation* SSBBE_SysParamSetGUI::getMacroInfo()
{
    LxTsspObjectManager* pObjectManager = LxTsspObjectManager::Instance();
...
void SSBBE_SysParamSetGUI::updateParam(SMCU::MCSSysTarget4ParamStruct& comPara, ILxTsspParameterMacro* pMacroObj)
{
...
int SSBBE_SysParamSetGUI::CreateDefaultMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    SMCU::MCSSysTarget4ParamStruct tmpComPara;          //参数
    ...
    //目标数
    tmpComPara.sysParam.ucTargetNum = m_ParaSetStruct[0][0].InitVal.lval + 1;
    //基带送数方式
    tmpComPara.sysParam.ucSendMode = m_ParaSetStruct[0][1].InitVal.lval + 1;
    ...
    for(int nIndex = 0; nIndex < MAX_TARGET_COUNT; nIndex++)
    {
        ...
        //上行频点
        tmpComPara.targetSysParam[nIndex].dwlAnswerRevUpFreq = m_ParaSetStruct[nIndex + 1][1].InitVal.lval;
        //下行频点
        tmpComPara.targetSysParam[nIndex].dwlAnswerRevDownFreq = m_ParaSetStruct[nIndex + 1][2].InitVal.lval;
        //任务标识
        tmpComPara.targetSysParam[nIndex].uiM = m_ParaSetStruct[nIndex + 1][3].InitVal.lval;
        //任务代号
        memcpy(tmpComPara.targetSysParam[nIndex].ucTaskDescribe, &m_ParaSetStruct[nIndex + 1][4].CVal.cValue, 6);
        //测速数据标志码
        tmpComPara.targetSysParam[nIndex].ucCsCode = m_ParaSetStruct[nIndex + 1][5].InitVal.lval;
        //遥测数据标志码
        tmpComPara.targetSysParam[nIndex].ucTmCode = m_ParaSetStruct[nIndex + 1][6].InitVal.lval;
        //测距数据标识码
        tmpComPara.targetSysParam[nIndex].ucCjCode = m_ParaSetStruct[nIndex + 1][7].InitVal.lval;
        //测速采样周期
        tmpComPara.targetSysParam[nIndex].ucCSSendRate = m_ParaSetStruct[nIndex + 1][8].InitVal.lval + 1;
        //测速采样率
        tmpComPara.targetSysParam[nIndex].ucCJSendRate = m_ParaSetStruct[nIndex + 1][9].InitVal.lval + 1;
    }
...
QWidget* SSBBE_SysParamSetGUI::ShowMeInEdit(QWidget* parent,uchar ucType,ILxTsspParameterMacro* pMacroObj)
{
    g_form = new SSBBE_SysParamSetGUIDlg(parent);
...
int SSBBE_SysParamSetGUI::SaveToMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    if(g_form != NULL)
    {
