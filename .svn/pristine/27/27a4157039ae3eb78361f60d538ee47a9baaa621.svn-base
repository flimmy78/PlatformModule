...
namespace
{
DDTBBE_SysParaSetGUIDlg* g_form = NULL;
}
extern quint64 GetID(ushort usStationID, uchar ucSubSysID, ushort usDeviceID, ushort usType,
                     uchar ucSn);
DDTBBE_SysParaSetGUI::DDTBBE_SysParaSetGUI(TLxTsspObjectInfo ObjectInfo)
{
    //初始化
 ...
 * @return 界面对象的界面。
*/
QWidget* DDTBBE_SysParaSetGUI::ShowMe(QWidget* parent, const QString& strTitle)
{
    m_strTitle = strTitle;

    DDTBBE_SysParaSetGUIDlg* form;

    form = new DDTBBE_SysParaSetGUIDlg(parent);
 ...
 * @return szHelpID:长度。
*/
int DDTBBE_SysParaSetGUI::GetContextHelpID(char* szHelpID)
{
...
*/
int DDTBBE_SysParaSetGUI::GetModuleInfo(TLxTsspModuleInfo& moduleInfo) const
{
    ...
    /// 配置标识，如：D_Y17-61_008
    strcpy(moduleInfo.szConfigID, "D_Y17-92_201\0");

    /// 版本
    strcpy(moduleInfo.szVersion, "01.00\0");

    /// 开发者，如：大卫.奥巴马
    strcpy(moduleInfo.szCreator, "wanglei\0");
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int DDTBBE_SysParaSetGUI::GetObjectInfo(TLxTsspObjectInfo& ObjectInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int DDTBBE_SysParaSetGUI::PreInitialize()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int DDTBBE_SysParaSetGUI::Initialize()
{
    //日志信息
    TLxTsspLog tmpLog;
    QString strLog;
    ...
    stCurPath = stCurPath + "/System/ModuleConfig/DDTBBE_SysParaSetGUI.xml";
    //遍历xlm文件，获取参数取值范围
    m_MapSumFlag = 0;
    ...
    if (pItem->vSubItem.size() > 0)
    {
        ...
        /*************************框架**********************************/
        m_ParaMCSSysSynID = GetID(sItem.usStationID, sItem.ucSubSysID, sItem.usDeviceID,
                                  sItem.usTID, sItem.ucSN);
        /*************************框架**********************************/

        /*************************状态**********************************/
        m_StatusParaMCSSysSynID = GetID(sItem.usStationID, sItem.ucSubSysID, sItem.usDeviceID,
...
//遍历xlm文件，获取参数取值范围
int DDTBBE_SysParaSetGUI::QueryParaBlock(QString ConfigPath)
{
...
int DDTBBE_SysParaSetGUI::GetAllMapValueStringByPara(int MapSum,QDomNode pInDocNode_Para)
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int DDTBBE_SysParaSetGUI::Start()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
bool DDTBBE_SysParaSetGUI::ClearUp()
{
    return 1;
}
 ...
 *           其它错误依情况增加；
 */
int DDTBBE_SysParaSetGUI::GetErrorNumber()
{
    return m_iErrCode;
}
 ...
 */
int DDTBBE_SysParaSetGUI::GetErrorMsg(int iErrorNo, QString& strErrorMsg)
{
 ...
 */
bool DDTBBE_SysParaSetGUI::IsOnline()
{
    //对象初始化
    //查找对象管理器
    LxTsspObjectManager* pObjectManager = LxTsspObjectManager::Instance();
 ...
 */
void DDTBBE_SysParaSetGUI::SendMessage(TLxTsspMessage msg)
{
    return;
}
 ...
 * @return 1：成功；-1：失败。
 */
int DDTBBE_SysParaSetGUI::SetParameter(QMap<quint64, QByteArray> parameterList)
{
 ...
 */
int DDTBBE_SysParaSetGUI::SetParameter(quint64 ui64ParamID, QByteArray parameter)
{
 ...
 */
int DDTBBE_SysParaSetGUI::GetParameterID(QList<quint64>& pParameterIDList) const
{
 ...
 * @return 大于等于0：参数块个数。-1：失败。
 */
int DDTBBE_SysParaSetGUI::GetParameter(QMap<quint64, QByteArray>& parameterList) const
{
    MCSSysDataStruct tmpMCSSysPara;         //公共参数单元界面

    MCSSysDataStatusStruct *tmpMCSSysStatus = NULL;     //公共单元状态数据元素
    ...
    if(tmpStateMgr != NULL)
        tmpMCSSysStatus  =
                (MCSSysDataStatusStruct*)tmpStateMgr->FindOneItem(m_StatusParaMCSSysSynID, stLen);

    if(tmpMCSSysStatus != NULL)
    {
        tmpMCSSysPara = tmpMCSSysStatus->tParams;
    }

    QByteArray  tmpCh;
    tmpCh.resize(sizeof(MCSSysDataStruct));
    memcpy(tmpCh.data(), &tmpMCSSysPara, sizeof(MCSSysDataStruct));
    parameterList.insert(m_ParaMCSSysSynID, tmpCh);
 ...
 * @return 1：成功；-1：失败。
 */
int DDTBBE_SysParaSetGUI::GetParameter(quint64 ui64ParamID, QByteArray& parameter) const
{
    if(m_ParamList.contains(ui64ParamID))
    {
 ...
 */
int DDTBBE_SysParaSetGUI::GetState(QMap<quint64, TLxTsspDeviceState>& stateList) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int DDTBBE_SysParaSetGUI::GetState(quint64 ui64StatusID, TLxTsspDeviceState& status) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int DDTBBE_SysParaSetGUI::GetStateDirect(quint64 ui64StatusID, TLxTsspDeviceState& status)
{
    if(m_StateList.contains(ui64StatusID))
    {
        //
 ...
 */
int DDTBBE_SysParaSetGUI::WriteData(quint64 ui64DataID, uchar* aData, uint uiDataLen)
{
    return 0;
}
 ...
 */
int DDTBBE_SysParaSetGUI::ReadData(quint64 ui64DataID, uchar* aData, uint& uiDataLen,
                                   uint& uiReloadCount)
{
    return 0;
}

TLxTsspParamMacroInformation* DDTBBE_SysParaSetGUI::getMacroInfo()
{
    LxTsspObjectManager* pObjectManager = LxTsspObjectManager::Instance();
    ...
    qDebug() << pMacroInfo->aTaskID;
    qDebug() << pMacroInfo->aTaskMark;
//    if(pMacroInfo->ucTargetSum < 1 || pMacroInfo->ucTargetSum > 4)return NULL;
    return pMacroInfo;
}
void DDTBBE_SysParaSetGUI::updateParam(MCSSysDataStruct& comPara, ILxTsspParameterMacro* pMacroObj)
{
...
int DDTBBE_SysParaSetGUI::CreateDefaultMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    MCSSysDataStruct MCSSynPara;
    //基带送数选择
    MCSSynPara.ucSendMode = (UCHAR)m_ParaSetStruct[0][1].InitVal.lval + 1;
    //任务代号
    memcpy(MCSSynPara.ucTaskDescribe, &m_ParaSetStruct[0][0].CVal.cValue, 6);
    //数据处理标识码
    MCSSynPara.ucDDTCode = (UCHAR)m_ParaSetStruct[0][2].InitVal.lval;
    //数传类别
    MCSSynPara.ucDDTOutFormat = (UCHAR)m_ParaSetStruct[0][3].InitVal.lval + 1;
    //任务标识
    MCSSynPara.uiM = m_ParaSetStruct[0][4].InitVal.lval;
    //下行频率
    MCSSynPara.dwlAnswerRevDownFreq = m_ParaSetStruct[0][5].InitVal.lval;
    //RTP负载类型
    MCSSynPara.ucRTPPT = m_ParaSetStruct[0][6].InitVal.lval + 1;

    //取当前设置的参数，存入框架
    updateParam(MCSSynPara, pMacroObj);
    QByteArray byteArray;
    byteArray.resize(sizeof(MCSSynPara));
    memcpy(byteArray.data(), &MCSSynPara, sizeof(MCSSynPara));
    if(pMacroObj != NULL)
    {
        pMacroObj->UpdateParameterBlock(m_ParaMCSSysSynID, byteArray);
    }
    return 1;
}

QWidget* DDTBBE_SysParaSetGUI::ShowMeInEdit(QWidget* parent,uchar ucType,ILxTsspParameterMacro* pMacroObj)
{
    g_form = new DDTBBE_SysParaSetGUIDlg(parent);
...
int DDTBBE_SysParaSetGUI::SaveToMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    if(g_form != NULL)
    {
