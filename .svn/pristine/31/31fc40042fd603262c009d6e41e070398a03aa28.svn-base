...
namespace
{
    USBBBE_TMCJRecSetGUIDlg* g_form = NULL;
}
extern quint64 GetID(ushort usStationID, uchar ucSubSysID, ushort usDeviceID, ushort usType,
              uchar ucSn);
USBBBE_TMCJRecSetGUI::USBBBE_TMCJRecSetGUI(TLxTsspObjectInfo ObjectInfo)
{
    //初始化
 ...
 * @return 界面对象的界面。
*/
QWidget* USBBBE_TMCJRecSetGUI::ShowMe(QWidget* parent, const QString& strTitle)
{
    m_strTitle = strTitle;

    USBBBE_TMCJRecSetGUIDlg* form;

    form = new USBBBE_TMCJRecSetGUIDlg(parent);
 ...
 * @return szHelpID:长度。
*/
int USBBBE_TMCJRecSetGUI::GetContextHelpID(char* szHelpID)
{
...
*/
int USBBBE_TMCJRecSetGUI::GetModuleInfo(TLxTsspModuleInfo& moduleInfo) const
{
    ...
    /// 配置标识，如：D_Y17-61_008
    strcpy(moduleInfo.szConfigID, "D_Y17-92_102\0");
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int USBBBE_TMCJRecSetGUI::GetObjectInfo(TLxTsspObjectInfo& ObjectInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int USBBBE_TMCJRecSetGUI::PreInitialize()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int USBBBE_TMCJRecSetGUI::Initialize()
{
    //日志信息
    TLxTsspLog tmpLog;
    QString strLog;
    ...
    stCurPath = stCurPath + "/System/ModuleConfig/USBBBE_TMCJRecSetGUI.xml";
    //遍历xml文件，获取参数取值范围
    m_MapSumFlag = 0;
    ...
    if (pItem->vSubItem.size() > 0)
    {
        ...
        /*************************框架**********************************/
        m_ParaTMCJID = GetID(sItem.usStationID, sItem.ucSubSysID, sItem.usDeviceID,
                                     sItem.usTID, sItem.ucSN);
        /*************************框架**********************************/

        /*************************状态**********************************/
        m_StatusTMCJID = GetID(sItem.usStationID, sItem.ucSubSysID, sItem.usDeviceID,
...
//遍历xml文件，获取参数取值范围
int USBBBE_TMCJRecSetGUI::QueryParaBlock(QString ConfigPath)
{
...
int USBBBE_TMCJRecSetGUI::GetAllMapValueStringByPara(int MapSum,QDomNode pInDocNode_Para)
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int USBBBE_TMCJRecSetGUI::Start()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
bool USBBBE_TMCJRecSetGUI::ClearUp()
{
    return 1;
}
 ...
 *           其它错误依情况增加；
 */
int USBBBE_TMCJRecSetGUI::GetErrorNumber()
{
    return m_iErrCode;
}
 ...
 */
int USBBBE_TMCJRecSetGUI::GetErrorMsg(int iErrorNo, QString& strErrorMsg)
{
 ...
 */
bool USBBBE_TMCJRecSetGUI::IsOnline()
{
    //对象初始化
    //查找对象管理器
    LxTsspObjectManager* pObjectManager = LxTsspObjectManager::Instance();
 ...
 */
void USBBBE_TMCJRecSetGUI::SendMessage(TLxTsspMessage msg)
{
    return;
}
 ...
 * @return 1：成功；-1：失败。
 */
int USBBBE_TMCJRecSetGUI::SetParameter(QMap<quint64, QByteArray> parameterList)
{
 ...
 */
int USBBBE_TMCJRecSetGUI::SetParameter(quint64 ui64ParamID, QByteArray parameter)
{
 ...
 */
int USBBBE_TMCJRecSetGUI::GetParameterID(QList<quint64>& pParameterIDList) const
{
 ...
 * @return 大于等于0：参数块个数。-1：失败。
 */
int USBBBE_TMCJRecSetGUI::GetParameter(QMap<quint64, QByteArray>& parameterList) const
{
    MCSUSBRevParamStruct tmpMCSUSBRevParam;       //码同步参数

    MCSUSBRevStatusStruct* tmpMCSUSBRevStatus = NULL;      //码同步参数
    ...
    if(tmpStateMgr != NULL)
        tmpMCSUSBRevStatus  =
                (MCSUSBRevStatusStruct*)tmpStateMgr->FindOneItem(m_StatusTMCJID, stLen);

    if(tmpMCSUSBRevStatus != NULL)
    {
        tmpMCSUSBRevParam = tmpMCSUSBRevStatus->tParams;
    }

    QByteArray  tmpCh;
    tmpCh.resize(sizeof(MCSUSBRevParamStruct));
    memcpy(tmpCh.data(), &tmpMCSUSBRevParam, sizeof(MCSUSBRevParamStruct));
    parameterList.insert(m_ParaTMCJID, tmpCh);
 ...
 * @return 1：成功；-1：失败。
 */
int USBBBE_TMCJRecSetGUI::GetParameter(quint64 ui64ParamID, QByteArray& parameter) const
{
    if(m_ParamList.contains(ui64ParamID))
    {
 ...
 */
int USBBBE_TMCJRecSetGUI::GetState(QMap<quint64, TLxTsspDeviceState>& stateList) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int USBBBE_TMCJRecSetGUI::GetState(quint64 ui64StatusID, TLxTsspDeviceState& status) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int USBBBE_TMCJRecSetGUI::GetStateDirect(quint64 ui64StatusID, TLxTsspDeviceState& status)
{
    if(m_StateList.contains(ui64StatusID))
    {
        //
 ...
 */
int USBBBE_TMCJRecSetGUI::WriteData(quint64 ui64DataID, uchar* aData, uint uiDataLen)
{
    return 0;
}
 ...
 */
int USBBBE_TMCJRecSetGUI::ReadData(quint64 ui64DataID, uchar* aData, uint& uiDataLen,
                             uint& uiReloadCount)
{
    return 0;
}

int USBBBE_TMCJRecSetGUI::CreateDefaultMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    MCSUSBRevParamStruct aMCSUSBRevParam;
    //AGC时间常数
    aMCSUSBRevParam.ucAgcTime = m_ParaSetStruct[0][2].InitVal.lval + 1;
    //载波环路带宽
    aMCSUSBRevParam.ucPllBw = m_ParaSetStruct[0][1].InitVal.lval + 1;
    //接收通道
    aMCSUSBRevParam.ucRevChannel = m_ParaSetStruct[0][0].InitVal.lval + 1;
    //载波中频
    aMCSUSBRevParam.uiCarrIF = (UINT)(m_ParaSetStruct[0][3].InitVal.lval * 1000000.0);
    //取当前设置的参数，存入框架
    QByteArray byteArray;
    byteArray.resize(sizeof(aMCSUSBRevParam));
    memcpy(byteArray.data(), &aMCSUSBRevParam, sizeof(aMCSUSBRevParam));
    if(pMacroObj != NULL)
    {
        pMacroObj->UpdateParameterBlock(m_ParaTMCJID, byteArray);
    }
    return 1;
}

QWidget* USBBBE_TMCJRecSetGUI::ShowMeInEdit(QWidget* parent, uchar ucType, ILxTsspParameterMacro* pMacroObj)
{
    g_form = new USBBBE_TMCJRecSetGUIDlg(parent);
...
int USBBBE_TMCJRecSetGUI::SaveToMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    if(g_form != NULL)
    {
