...
namespace
{
    SS2BBE_OutPutSelSetGUIDlg* g_form = NULL;
}
extern quint64 GetID(ushort usStationID, uchar ucSubSysID, ushort usDeviceID, ushort usType,
              uchar ucSn);
SS2BBE_OutPutSelSetGUI::SS2BBE_OutPutSelSetGUI(TLxTsspObjectInfo ObjectInfo)
{
    //初始化
 ...
 * @return 界面对象的界面。
*/
QWidget* SS2BBE_OutPutSelSetGUI::ShowMe(QWidget* parent, const QString& strTitle)
{
    m_strTitle = strTitle;

    SS2BBE_OutPutSelSetGUIDlg* form;

    form = new SS2BBE_OutPutSelSetGUIDlg(parent);
 ...
 * @return szHelpID:长度。
*/
int SS2BBE_OutPutSelSetGUI::GetContextHelpID(char* szHelpID)
{
...
*/
int SS2BBE_OutPutSelSetGUI::GetModuleInfo(TLxTsspModuleInfo& moduleInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS2BBE_OutPutSelSetGUI::GetObjectInfo(TLxTsspObjectInfo& ObjectInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS2BBE_OutPutSelSetGUI::PreInitialize()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS2BBE_OutPutSelSetGUI::Initialize()
{
    //日志信息
    TLxTsspLog tmpLog;
    QString strLog;
    ...
    stCurPath = stCurPath + "/System/ModuleConfig/SS2BBE_OutPutSelSetGUI.xml";
    //遍历xml文件，获取参数取值范围
    m_MapSumFlag = 0;
...
//遍历xml文件，获取参数取值范围
int SS2BBE_OutPutSelSetGUI::QueryParaBlock(QString ConfigPath)
{
...
int SS2BBE_OutPutSelSetGUI::GetAllMapValueStringByPara(int MapSum,QDomNode pInDocNode_Para)
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS2BBE_OutPutSelSetGUI::Start()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS2BBE_OutPutSelSetGUI::Stop()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
bool SS2BBE_OutPutSelSetGUI::ClearUp()
{
    return 1;
}
 ...
 *           其它错误依情况增加；
 */
int SS2BBE_OutPutSelSetGUI::GetErrorNumber()
{
    return m_iErrCode;
}
 ...
 */
int SS2BBE_OutPutSelSetGUI::GetErrorMsg(int iErrorNo, QString& strErrorMsg)
{
 ...
 */
bool SS2BBE_OutPutSelSetGUI::IsOnline()
{
    //对象初始化
    //查找对象管理器
    LxTsspObjectManager* pObjectManager = LxTsspObjectManager::Instance();
 ...
 */
void SS2BBE_OutPutSelSetGUI::SendMessage(TLxTsspMessage msg)
{
    return;
}
 ...
 * @return 1：成功；-1：失败。
 */
int SS2BBE_OutPutSelSetGUI::SetParameter(QMap<quint64, QByteArray> parameterList)
{
 ...
 */
int SS2BBE_OutPutSelSetGUI::SetParameter(quint64 ui64ParamID, QByteArray parameter)
{
 ...
 */
int SS2BBE_OutPutSelSetGUI::GetParameterID(QList<quint64>& pParameterIDList) const
{
 ...
 * @return 大于等于0：参数块个数。-1：失败。
 */
int SS2BBE_OutPutSelSetGUI::GetParameter(QMap<quint64, QByteArray>& parameterList) const
{
    SMCU2::MCSSysParamStruct4 tmpComPara;          //参数

    SMCU2::MCSSysTarget4StatusStruct *tmpComStatus = NULL;  //状态参数
    ...
    {
            tmpComPara.MCSSysParam = tmpComStatus->MCSSysStatu.tParams;
            for (int i = 0; i< MAX_TARGET_COUNT; i++)
                tmpComPara.MCSSSB2TargetSysParam[i] = tmpComStatus->MCSSSB2TargetSysStatu[i].tParams;
    }
 ...
 * @return 1：成功；-1：失败。
 */
int SS2BBE_OutPutSelSetGUI::GetParameter(quint64 ui64ParamID, QByteArray& parameter) const
{
    if(m_ParamList.contains(ui64ParamID))
    {
 ...
 */
int SS2BBE_OutPutSelSetGUI::GetState(QMap<quint64, TLxTsspDeviceState>& stateList) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int SS2BBE_OutPutSelSetGUI::GetState(quint64 ui64StatusID, TLxTsspDeviceState& status) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int SS2BBE_OutPutSelSetGUI::GetStateDirect(quint64 ui64StatusID, TLxTsspDeviceState& status)
{
    if(m_StateList.contains(ui64StatusID))
    {
        //
 ...
 */
int SS2BBE_OutPutSelSetGUI::WriteData(quint64 ui64DataID, uchar* aData, uint uiDataLen)
{
    return 0;
}
 ...
 */
int SS2BBE_OutPutSelSetGUI::ReadData(quint64 ui64DataID, uchar* aData, uint& uiDataLen,
                             uint& uiReloadCount)
{
    return 0;
}

int SS2BBE_OutPutSelSetGUI::CreateDefaultMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    SMCU2::MCSSysParamStruct4 tmpComPara;          //参数
    QByteArray byteArray;
    byteArray.resize(sizeof(tmpComPara));
    if(1 == pMacroObj->GetParameterBlock(m_ParaComSynID, byteArray))
        memcpy(&tmpComPara, byteArray.data(), sizeof(tmpComPara));
    //        //保持门限
    //        tmpComPara.MCSSysParam.ucRMHCheck = m_ParaSetStruct[0][3].InitVal.lval;
    //        //判决时间
    //        tmpComPara.MCSSysParam.ucPreRangDataMd = m_ParaSetStruct[0][4].InitVal.lval;
    //        //输出门限
    //        tmpComPara.MCSSysParam.ulPridictR = m_ParaSetStruct[0][5].InitVal.lval;
    //输出方式
    tmpComPara.MCSSysParam.ucTrackOutType = m_ParaSetStruct[0][0].InitVal.lval + 1;
    //输出通道
    tmpComPara.MCSSysParam.ucTrackType = m_ParaSetStruct[0][2].InitVal.lval + 1;
    //输出目标
    tmpComPara.MCSSysParam.ucTrackDefCH = m_ParaSetStruct[0][1].InitVal.lval + 1;
...
QWidget* SS2BBE_OutPutSelSetGUI::ShowMeInEdit(QWidget* parent, uchar ucType, ILxTsspParameterMacro* pMacroObj)
{
    g_form = new SS2BBE_OutPutSelSetGUIDlg(parent);
...
int SS2BBE_OutPutSelSetGUI::SaveToMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    if(g_form != NULL)
    {
