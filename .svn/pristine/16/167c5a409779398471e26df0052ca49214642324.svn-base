#include "TCBBE_TCParamSetGUIDlg.h"
#include "ui_TCBBE_TCParamSetGUIDlg.h"
#include <QCloseEvent>
#include <QMessageBox>
#include <QDateTime>
...
extern quint64 GetID(ushort usStationID, uchar ucSubSysID, ushort usDeviceID, ushort usType,
                     uchar ucSn);
TCBBE_TCParamSetGUIDlg::TCBBE_TCParamSetGUIDlg(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::TCBBE_TCParamSetGUIDlg)
{
    ui->setupUi(this);
    m_pStateMgr = NULL;
...
TCBBE_TCParamSetGUIDlg::~TCBBE_TCParamSetGUIDlg ()
{
    killTimer(m_TimerID);
    m_timer1->stop();
    delete m_timer1;
...
void TCBBE_TCParamSetGUIDlg::setText(const QString& strTitle, ParaStruct xmlsetText[5][60], QString MapMean[50][30], int MapNum[50], uchar Type, ILxTsspParameterMacro* pMacroObj)
{
    m_strTitle = strTitle;
...
//初始化对话框
void TCBBE_TCParamSetGUIDlg::InitDLG(int Type, ILxTsspParameterMacro* pMacroObj)
{
    iParaSetFlag = Type;
    //设置输入格式
    InitInput();
...
//设置输入格式
void TCBBE_TCParamSetGUIDlg::InitInput()
{
    //正整数
...
void TCBBE_TCParamSetGUIDlg::ShowControls()
{
    showTargetParam(0, 0);
    //默认选择目标1
...
//显示参数
void TCBBE_TCParamSetGUIDlg::ShowPara(int type)
{
    if(type == 0)       //参数设置
    {
        //码同步
...
//设置帧长范围
void TCBBE_TCParamSetGUIDlg::SetFrameLenRange(int iWordLen)
{
    /* if(iWordLen ==8)
    {
...
void TCBBE_TCParamSetGUIDlg::SetObjInfo(TLxTsspObjectInfo ObjectInfo)
{
...
//使能或不使能控件
void TCBBE_TCParamSetGUIDlg::EnabledControls(bool bFlag)
{
    /*************************Group1*************************/
    //工作期启动方式
...
//保存参数宏
int TCBBE_TCParamSetGUIDlg::SaveToMacro(ILxTsspParameterMacro* pMacroObj)
{
    //判断合法性
    int iRet = CheckPara();
    if(iRet == -1)
    ...
    else
    {
        ...
        int iRes = pMacroObj->UpdateParameterBlock(m_ParaMCSSSTCID,byteArray);
...
//从界面取参数
void TCBBE_TCParamSetGUIDlg::GetPara()
{
    getParamTarget();
}

//验证参数范围
int TCBBE_TCParamSetGUIDlg::CheckPara()
{
    QMessageBox msgBox;
    QString strTmp;
...
//定时器
void TCBBE_TCParamSetGUIDlg::timerEvent(QTimerEvent *event)
{
    ShowPara(0);
}

//更改参数按钮休息处理函数
void TCBBE_TCParamSetGUIDlg::on_pushButton_Modify_clicked()
{
    if(ui->pushButton_Modify->text() == "更改")
    {
...
void TCBBE_TCParamSetGUIDlg::on_pushButton_Set_clicked()
{
    //验证参数范围
    if(CheckPara() == 1)
    {
...
void TCBBE_TCParamSetGUIDlg::closeEvent(QCloseEvent *event)
{
    event->accept();
}

QSize TCBBE_TCParamSetGUIDlg::sizeHint() const
{
    return QSize(this->width(), this->height());
}

void TCBBE_TCParamSetGUIDlg::SubFrmEn(bool bFlag)
{
    //副帧长度
...
//有无ID副帧
void TCBBE_TCParamSetGUIDlg::IDSubFrmEn(bool bFlag)
{
...
//有无循环副帧
void TCBBE_TCParamSetGUIDlg::CycSubFrmEn(bool bFlag)
{
    //循环副帧
...
//有无反码副帧
void TCBBE_TCParamSetGUIDlg::ReserveSubFrmEn(bool bFlag)
{

}

void TCBBE_TCParamSetGUIDlg::SendMessage(char* pData, unsigned short  usLen, unsigned short usC, unsigned short usTID)
{
    static uint seq = 0;
...
void TCBBE_TCParamSetGUIDlg::showTargetParam(int nIndex, int nType)
{
    if (nType == 0)
    {
        //向下拉框控件中写入从xml中读取的内容
...
void TCBBE_TCParamSetGUIDlg::getParamTarget()
{
    bool ok;
    /***********************************码同步****************************/
    //码型
...
DWORDLONG TCBBE_TCParamSetGUIDlg::StrToSynCode(const char * pStr)
{
    char c,C;
    const char*  p;
...
void TCBBE_TCParamSetGUIDlg::on_radioButton_Target1_clicked()
{
    if (CheckPara() != 1)
    {
...
void TCBBE_TCParamSetGUIDlg::on_radioButton_Target2_clicked()
{
    if (CheckPara() != 1)
    {
        set_radioButton_Target();
...
void TCBBE_TCParamSetGUIDlg::on_radioButton_Target3_clicked()
{
    if (CheckPara() != 1)
    {
        set_radioButton_Target();
...
void TCBBE_TCParamSetGUIDlg::on_radioButton_Target4_clicked()
{
    if (CheckPara() != 1)
    {
        set_radioButton_Target();
...
int TCBBE_TCParamSetGUIDlg::getSharedMemoryTargetNum()
{
    return 1;
...
void TCBBE_TCParamSetGUIDlg::showRadioButton()
{
...
//遥控转发方式
void TCBBE_TCParamSetGUIDlg::on_comboBox_TeleSendType_currentIndexChanged(int index)
{
    if(ui->pushButton_Modify->text() == "更改")
        return ;
    if(index==0){
...
void TCBBE_TCParamSetGUIDlg::on_comboBox_BCHEncode_currentIndexChanged(int index)
{
    if(ui->pushButton_Modify->text() == "更改")
        return ;
    if(index==0)
...
//加扰控制
void TCBBE_TCParamSetGUIDlg::on_comboBox_RandCtrl_currentIndexChanged(int index)
{
    if(ui->pushButton_Modify->text() == "更改")
        return ;
    if(index==0)
...
//起始序列控制
void TCBBE_TCParamSetGUIDlg::on_comboBox_BeginCtrl_currentIndexChanged(int index)
{
    if(ui->pushButton_Modify->text() == "更改")
        return ;
...
//结尾序列控制
void TCBBE_TCParamSetGUIDlg::on_comboBox_LateCtrl_currentIndexChanged(int index)
{
    if(ui->pushButton_Modify->text() == "更改")
        return ;
    if(index==1 && ui->comboBox_BCHEncode->currentIndex() == 1)
...
void TCBBE_TCParamSetGUIDlg::updateRadioButton()
{
...
void TCBBE_TCParamSetGUIDlg::set_radioButton_Target()
{
    switch(m_nTargetNum)
    {
    case 0:
...
void TCBBE_TCParamSetGUIDlg::on_lineEdit_ExeCode_textChanged(const QString &arg1)
{
