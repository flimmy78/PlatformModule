...
namespace
{
    USBBBE_ZKsetGUIDlg* g_form = NULL;
}
extern quint64 GetID(ushort usStationID, uchar ucSubSysID, ushort usDeviceID, ushort usType,
              uchar ucSn);
USBBBE_ZKsetGUI::USBBBE_ZKsetGUI(TLxTsspObjectInfo ObjectInfo)
{
    //初始化
 ...
 * @return 界面对象的界面。
*/
QWidget* USBBBE_ZKsetGUI::ShowMe(QWidget* parent, const QString& strTitle)
{
    m_strTitle = strTitle;

    USBBBE_ZKsetGUIDlg* form;

    form = new USBBBE_ZKsetGUIDlg(parent);
 ...
 * @return szHelpID:长度。
*/
int USBBBE_ZKsetGUI::GetContextHelpID(char* szHelpID)
{
...
*/
int USBBBE_ZKsetGUI::GetModuleInfo(TLxTsspModuleInfo& moduleInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int USBBBE_ZKsetGUI::GetObjectInfo(TLxTsspObjectInfo& ObjectInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int USBBBE_ZKsetGUI::PreInitialize()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int USBBBE_ZKsetGUI::Initialize()
{
    //日志信息
    TLxTsspLog tmpLog;
    QString strLog;
    ...
    stCurPath = stCurPath + "/System/ModuleConfig/USBBBE_ZKsetGUI.xml";
    //遍历xml文件，获取参数取值范围
    m_MapSumFlag = 0;
    ...
    if (pItem->vSubItem.size() > 0)
    {
        ...
        /*************************框架**********************************/
        m_ParaZKSetID = GetID(sItem.usStationID, sItem.ucSubSysID, sItem.usDeviceID,
                                     sItem.usTID, sItem.ucSN);
        /*************************框架**********************************/

        /*************************状态**********************************/
        m_StatusZKSetID = GetID(sItem.usStationID, sItem.ucSubSysID, sItem.usDeviceID,
...
//遍历xml文件，获取参数取值范围
int USBBBE_ZKsetGUI::QueryParaBlock(QString ConfigPath)
{
...
int USBBBE_ZKsetGUI::GetAllMapValueStringByPara(int MapSum,QDomNode pInDocNode_Para)
{
 ...
 *           其它错误依情况增加；
 */
int USBBBE_ZKsetGUI::GetErrorNumber()
{
    return m_iErrCode;
}
 ...
 */
int USBBBE_ZKsetGUI::GetErrorMsg(int iErrorNo, QString& strErrorMsg)
{
 ...
 */
void USBBBE_ZKsetGUI::SendMessage(TLxTsspMessage msg)
{
    return;
}
 ...
 * @return 1：成功；-1：失败。
 */
int USBBBE_ZKsetGUI::SetParameter(QMap<quint64, QByteArray> parameterList)
{
 ...
 */
int USBBBE_ZKsetGUI::SetParameter(quint64 ui64ParamID, QByteArray parameter)
{
 ...
 */
int USBBBE_ZKsetGUI::GetParameterID(QList<quint64>& pParameterIDList) const
{
 ...
 * @return 大于等于0：参数块个数。-1：失败。
 */
int USBBBE_ZKsetGUI::GetParameter(QMap<quint64, QByteArray>& parameterList) const
{
    MCSUSBTCParamStruct tmpMCSUSBTCParam;       //码同步参数

    MCSUSBTCStatusStruct* tmpMCSUSBTCStatus = NULL;      //码同步参数
    ...
    if(tmpStateMgr != NULL)
        tmpMCSUSBTCStatus  =
                (MCSUSBTCStatusStruct*)tmpStateMgr->FindOneItem(m_StatusZKSetID, stLen);

    if(tmpMCSUSBTCStatus != NULL)
    {
        tmpMCSUSBTCParam = tmpMCSUSBTCStatus->tParams;
    }

    QByteArray  tmpCh;
    tmpCh.resize(sizeof(MCSUSBTCParamStruct));
    memcpy(tmpCh.data(), &tmpMCSUSBTCParam, sizeof(MCSUSBTCParamStruct));
    parameterList.insert(m_ParaZKSetID, tmpCh);
 ...
 * @return 1：成功；-1：失败。
 */
int USBBBE_ZKsetGUI::GetParameter(quint64 ui64ParamID, QByteArray& parameter) const
{
    if(m_ParamList.contains(ui64ParamID))
    {
 ...
 */
int USBBBE_ZKsetGUI::GetState(QMap<quint64, TLxTsspDeviceState>& stateList) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int USBBBE_ZKsetGUI::GetState(quint64 ui64StatusID, TLxTsspDeviceState& status) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int USBBBE_ZKsetGUI::GetStateDirect(quint64 ui64StatusID, TLxTsspDeviceState& status)
{
    if(m_StateList.contains(ui64StatusID))
    {
        //
 ...
 */
int USBBBE_ZKsetGUI::WriteData(quint64 ui64DataID, uchar* aData, uint uiDataLen)
{
    return 0;
}
 ...
 */
int USBBBE_ZKsetGUI::ReadData(quint64 ui64DataID, uchar* aData, uint& uiDataLen,
                             uint& uiReloadCount)
{
    return 0;
}

int USBBBE_ZKsetGUI::CreateDefaultMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    MCSUSBTCParamStruct aMCSUSBTCParam;
    //副载波调制体制
    aMCSUSBTCParam.ucModStyle = m_ParaSetStruct[0][0].InitVal.lval + 1;
    //遥控工作模式
    aMCSUSBTCParam.ucSWaveMd = m_ParaSetStruct[0][1].InitVal.lval + 1;
    //工作期启动方式
    aMCSUSBTCParam.ucTCWorkCyc = m_ParaSetStruct[0][2].InitVal.lval + 1;
    //码型
    aMCSUSBTCParam.ucCodeStyle = m_ParaSetStruct[0][3].InitVal.lval + 1;
    //码速率
    aMCSUSBTCParam.uiCodeRate = m_ParaSetStruct[0][4].InitVal.lval;
    //调制频率
    aMCSUSBTCParam.uiSwaveFreq_1 = m_ParaSetStruct[0][5].InitVal.lval;
    //副载波频1频率（f1）
    aMCSUSBTCParam.uiSwaveFreq_2 = m_ParaSetStruct[0][6].InitVal.lval;
    //副载频填充频率（f填充）
    aMCSUSBTCParam.uiSwaveFreq_3 = m_ParaSetStruct[0][7].InitVal.lval;
    //副载频执行频率（f执行）
    aMCSUSBTCParam.uiSwaveFreq_4 = m_ParaSetStruct[0][8].InitVal.lval;
    //前补序列码样（Hex）
    aMCSUSBTCParam.ucFCodeMinU = m_ParaSetStruct[0][9].InitVal.lval;
    //后补序列码样（Hex）
    aMCSUSBTCParam.ucBCodeMinU = m_ParaSetStruct[0][10].InitVal.lval;
    //执行序列码样（Hex）
    aMCSUSBTCParam.ucECodeMinU = m_ParaSetStruct[0][11].InitVal.lval;
    //空闲序列码样（Hex）
    aMCSUSBTCParam.ucIndleCode = m_ParaSetStruct[0][12].InitVal.lval;
    //前补序列长度
    aMCSUSBTCParam.usFQueueLen = m_ParaSetStruct[0][13].InitVal.lval;
    //后补序列长度
    aMCSUSBTCParam.usBQueueLen = m_ParaSetStruct[0][14].InitVal.lval;
    //执行编码长度
    aMCSUSBTCParam.ucECodeLen = m_ParaSetStruct[0][15].InitVal.lval;
    //指令保护间隙
    aMCSUSBTCParam.usTCProctC = m_ParaSetStruct[0][16].InitVal.lval;
    //BCH编码
    aMCSUSBTCParam.ucBCHCode = m_ParaSetStruct[0][17].InitVal.lval + 1;
    //BCH编码起始序列控制
    aMCSUSBTCParam.ucBCHFQueueCheck = m_ParaSetStruct[0][18].InitVal.lval + 1;
    //起始序列图样
    aMCSUSBTCParam.usBCHFQueue = m_ParaSetStruct[0][19].InitVal.lval;
    //加扰控制
    aMCSUSBTCParam.ucturbCtrl = m_ParaSetStruct[0][20].InitVal.lval + 1;
    //结尾序列控制
    aMCSUSBTCParam.ucBCHBQueueCheck = m_ParaSetStruct[0][21].InitVal.lval + 1;
    //结尾序列图样
    aMCSUSBTCParam.usBCHBQueue = m_ParaSetStruct[0][22].InitVal.lval;
    //加扰多项式长度
    aMCSUSBTCParam.ucRandLen = m_ParaSetStruct[0][23].InitVal.lval;
    //加扰多项式（Hex）
    aMCSUSBTCParam.uiRandPoly = m_ParaSetStruct[0][24].InitVal.lval;
    //加扰初相
    aMCSUSBTCParam.uiPhase = m_ParaSetStruct[0][25].InitVal.lval;
    //取当前设置的参数，存入框架
    QByteArray byteArray;
    byteArray.resize(sizeof(aMCSUSBTCParam));
    memcpy(byteArray.data(),&aMCSUSBTCParam,sizeof(aMCSUSBTCParam));
    if(pMacroObj!=NULL)
    {
        pMacroObj->UpdateParameterBlock(m_ParaZKSetID,byteArray);
    }
    return 1;
}

QWidget* USBBBE_ZKsetGUI::ShowMeInEdit(QWidget* parent,uchar ucType,ILxTsspParameterMacro* pMacroObj)
{
    g_form = new USBBBE_ZKsetGUIDlg(parent);
...
int USBBBE_ZKsetGUI::SaveToMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    if(g_form!=NULL)
    {
