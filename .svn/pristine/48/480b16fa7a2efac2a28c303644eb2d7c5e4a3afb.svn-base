...
void CHelper::sendData(uchar* pPack, int packLen, QString strID)
{
    ...
    if (!pObjectManager)
        return;
    ILxTsspCommunicationManager * pCommunicationMgr = pObjectManager->GetCommunicationManager();
    if (!pCommunicationMgr)
        return;
    ICommunicationChannel *pChannel = NULL;
    pCommunicationMgr->GetChannel(strID, pChannel);
    if (NULL == pChannel)
    {
