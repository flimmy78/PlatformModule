 ...
 * @return 界面对象的界面。
*/
QWidget* SswitchDcuGUI::ShowMe(QWidget* parent, const QString& strTitle)
{
    m_strTitle = strTitle;

    SswitchDcuGUIDlg* form;

    form = new SswitchDcuGUIDlg(parent);
 ...
 * @return szHelpID:长度。
*/
int SswitchDcuGUI::GetContextHelpID(char* szHelpID)
{
...
*/
int SswitchDcuGUI::GetModuleInfo(TLxTsspModuleInfo& moduleInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SswitchDcuGUI::GetObjectInfo(TLxTsspObjectInfo& ObjectInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SswitchDcuGUI::PreInitialize()
{
    return 1;
}
...
//遍历xml文件，获取参数取值范围
int SswitchDcuGUI::QueryParaBlock(QString ConfigPath)
{
...
int SswitchDcuGUI::GetAllMapValueStringByPara(int MapSum,QDomNode pInDocNode_Para)
{
 ...
 *           其它错误依情况增加；
 */
int SswitchDcuGUI::GetErrorNumber()
{
    return m_iErrCode;
}
 ...
 */
int SswitchDcuGUI::GetErrorMsg(int iErrorNo, QString& strErrorMsg)
{
 ...
 */
void SswitchDcuGUI::SendMessage(TLxTsspMessage msg)
{
    return;
}
 ...
 * @return 1：成功；-1：失败。
 */
int SswitchDcuGUI::SetParameter(QMap<quint64, QByteArray> parameterList)
{
 ...
 */
int SswitchDcuGUI::SetParameter(quint64 ui64ParamID, QByteArray parameter)
{
 ...
 */
int SswitchDcuGUI::GetParameterID(QList<quint64>& pParameterIDList) const
{
 ...
 * @return 大于等于0：参数块个数。-1：失败。
 */
int SswitchDcuGUI::GetParameter(QMap<quint64, QByteArray>& parameterList) const
{
    SSWITCHDCUPara tmpComPara;          //参数

    SSWITCHDCUStatu *tmpComStatus = NULL;  //状态
    ...
    if(tmpStateMgr != NULL)
        tmpComStatus  =
                (SSWITCHDCUStatu*)tmpStateMgr->FindOneItem(m_StatusComSynID, stLen);
    ...
    QByteArray tmpCh;
    tmpCh.resize(sizeof(SSWITCHDCUPara));
    memcpy(tmpCh.data(), &tmpComPara, sizeof(SSWITCHDCUPara));
 ...
 * @return 1：成功；-1：失败。
 */
int SswitchDcuGUI::GetParameter(quint64 ui64ParamID, QByteArray& parameter) const
{
    if(m_ParamList.contains(ui64ParamID))
    {
 ...
 */
int SswitchDcuGUI::GetState(QMap<quint64, TLxTsspDeviceState>& stateList) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int SswitchDcuGUI::GetState(quint64 ui64StatusID, TLxTsspDeviceState& status) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int SswitchDcuGUI::GetStateDirect(quint64 ui64StatusID, TLxTsspDeviceState& status)
{
    if(m_StateList.contains(ui64StatusID))
    {
        //
 ...
 */
int SswitchDcuGUI::WriteData(quint64 ui64DataID, uchar* aData, uint uiDataLen)
{
    return 0;
}
 ...
 */
int SswitchDcuGUI::ReadData(quint64 ui64DataID, uchar* aData, uint& uiDataLen,
                             uint& uiReloadCount)
{
    return 0;
}

int SswitchDcuGUI::CreateDefaultMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    ...
    memcpy(byteArray.data(), &comPara, sizeof(SSWITCHDCUPara));
    if(pMacroObj != NULL)
    {
...
QWidget* SswitchDcuGUI::ShowMeInEdit(QWidget* parent,uchar ucType,ILxTsspParameterMacro* pMacroObj)
{
    g_form = new SswitchDcuGUIDlg(parent);
...
int SswitchDcuGUI::SaveToMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    if(g_form != NULL)
    {
