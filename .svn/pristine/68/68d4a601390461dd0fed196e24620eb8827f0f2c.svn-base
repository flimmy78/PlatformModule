...
namespace
{
TCBBE_TCParamSetGUIDlg* g_form = NULL;
}
extern quint64 GetID(ushort usStationID, uchar ucSubSysID, ushort usDeviceID, ushort usType,
                     uchar ucSn);
TCBBE_TCParamSetGUI::TCBBE_TCParamSetGUI(TLxTsspObjectInfo ObjectInfo)
{
    //初始化
 ...
 * @return 界面对象的界面。
*/
QWidget* TCBBE_TCParamSetGUI::ShowMe(QWidget* parent, const QString& strTitle)
{
    m_strTitle = strTitle;

    TCBBE_TCParamSetGUIDlg* form;

    form = new TCBBE_TCParamSetGUIDlg(parent);
 ...
 * @return szHelpID:长度。
*/
int TCBBE_TCParamSetGUI::GetContextHelpID(char* szHelpID)
{
...
*/
int TCBBE_TCParamSetGUI::GetModuleInfo(TLxTsspModuleInfo& moduleInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int TCBBE_TCParamSetGUI::GetObjectInfo(TLxTsspObjectInfo& ObjectInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int TCBBE_TCParamSetGUI::PreInitialize()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int TCBBE_TCParamSetGUI::Initialize()
{
    //日志信息
    TLxTsspLog tmpLog;
    QString strLog;
    ...
    stCurPath = stCurPath + "/System/ModuleConfig/TCBBE_TCParamSetGUI.xml";
    //遍历xml文件，获取参数取值范围
    m_MapSumFlag = 0;
...
//遍历xml文件，获取参数取值范围
int TCBBE_TCParamSetGUI::QueryParaBlock(QString ConfigPath)
{
...
int TCBBE_TCParamSetGUI::GetAllMapValueStringByPara(int MapSum,QDomNode pInDocNode_Para)
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
bool TCBBE_TCParamSetGUI::ClearUp()
{
    return 1;
}
 ...
 *           其它错误依情况增加；
 */
int TCBBE_TCParamSetGUI::GetErrorNumber()
{
    return m_iErrCode;
}
 ...
 */
int TCBBE_TCParamSetGUI::GetErrorMsg(int iErrorNo, QString& strErrorMsg)
{
 ...
 */
bool TCBBE_TCParamSetGUI::IsOnline()
{
    //对象初始化
    //查找对象管理器
    LxTsspObjectManager* pObjectManager = LxTsspObjectManager::Instance();
...
bool TCBBE_TCParamSetGUI::isAK(unsigned short localDID, int& connectIndex)
{
 ...
 */
void TCBBE_TCParamSetGUI::SendMessage(TLxTsspMessage msg)
{
    return;
}
 ...
 * @return 1：成功；-1：失败。
 */
int TCBBE_TCParamSetGUI::SetParameter(QMap<quint64, QByteArray> parameterList)
{
 ...
 */
int TCBBE_TCParamSetGUI::SetParameter(quint64 ui64ParamID, QByteArray parameter)
{
 ...
 */
int TCBBE_TCParamSetGUI::GetParameterID(QList<quint64>& pParameterIDList) const
{
 ...
 * @return 大于等于0：参数块个数。-1：失败。
 */
int TCBBE_TCParamSetGUI::GetParameter(QMap<quint64, QByteArray>& parameterList) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int TCBBE_TCParamSetGUI::GetParameter(quint64 ui64ParamID, QByteArray& parameter) const
{
    if(m_ParamList.contains(ui64ParamID))
    {
 ...
 */
int TCBBE_TCParamSetGUI::GetState(QMap<quint64, TLxTsspDeviceState>& stateList) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int TCBBE_TCParamSetGUI::GetState(quint64 ui64StatusID, TLxTsspDeviceState& status) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int TCBBE_TCParamSetGUI::GetStateDirect(quint64 ui64StatusID, TLxTsspDeviceState& status)
{
    if(m_StateList.contains(ui64StatusID))
    {
        //
 ...
 */
int TCBBE_TCParamSetGUI::WriteData(quint64 ui64DataID, uchar* aData, uint uiDataLen)
{
    return 0;
}
 ...
 */
int TCBBE_TCParamSetGUI::ReadData(quint64 ui64DataID, uchar* aData, uint& uiDataLen,
                                   uint& uiReloadCount)
{
    return 0;
}

int TCBBE_TCParamSetGUI::CreateDefaultMacroObj(ILxTsspParameterMacro* pMacroObj)
{
...
QWidget* TCBBE_TCParamSetGUI::ShowMeInEdit(QWidget* parent,uchar ucType,ILxTsspParameterMacro* pMacroObj)
{
    g_form = new TCBBE_TCParamSetGUIDlg(parent);
...
int TCBBE_TCParamSetGUI::SaveToMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    if(g_form!=NULL)
    {
