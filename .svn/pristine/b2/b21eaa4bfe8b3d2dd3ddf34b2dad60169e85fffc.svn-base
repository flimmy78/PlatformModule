...
void RecvDataThread::processDatagram(QByteArray& datagram, QString chlID)
{
    ...
    if(chlID == "TMRemote")
    {
        ...
        else if (tmp == STATUS)
        {
            ...
            if (m_pPlatformRes->m_pStatMgr->FindOneItem(ds.id, ds.len) == NULL)
            {
                sz = ds.len;
                uchar* pItem = m_pPlatformRes->m_pStatMgr->ApplyOneItem(ds.id, sz);
                if(pItem && sz == ds.len)
                    m_pPlatformRes->m_pStatMgr->UpdateOneItem(ds.id, ds.data, ds.len);
            }else m_pPlatformRes->m_pStatMgr->UpdateOneItem(ds.id, ds.data, ds.len);
