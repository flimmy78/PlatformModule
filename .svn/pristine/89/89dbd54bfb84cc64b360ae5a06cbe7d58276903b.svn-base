...
namespace
{
    SS1PNCodeSetGUIDlg* g_form = NULL;
}
extern quint64 GetID(ushort usStationID, uchar ucSubSysID, ushort usDeviceID, ushort usType,
              uchar ucSn);
SS1PNCodeSetGUI::SS1PNCodeSetGUI(TLxTsspObjectInfo ObjectInfo)
{
    //初始化
 ...
 * @return 界面对象的界面。
*/
QWidget* SS1PNCodeSetGUI::ShowMe(QWidget* parent, const QString& strTitle)
{
    m_strTitle = strTitle;

    SS1PNCodeSetGUIDlg* form;

    form = new SS1PNCodeSetGUIDlg(parent);
 ...
 * @return szHelpID:长度。
*/
int SS1PNCodeSetGUI::GetContextHelpID(char* szHelpID)
{
...
*/
int SS1PNCodeSetGUI::GetModuleInfo(TLxTsspModuleInfo& moduleInfo) const
{
    ...
    /// 配置标识，如：D_Y17-61_008
    strcpy(moduleInfo.szConfigID, "D_Y17-91_118\0");
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS1PNCodeSetGUI::GetObjectInfo(TLxTsspObjectInfo& ObjectInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS1PNCodeSetGUI::PreInitialize()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS1PNCodeSetGUI::Initialize()
{
    //日志信息
    TLxTsspLog tmpLog;
    QString strLog;
    ...
    stCurPath = stCurPath + "/System/ModuleConfig/SS1PNCodeSetGUI.xml";
    //遍历xml文件，获取参数取值范围
    m_MapSumFlag = 0;
...
//遍历xml文件，获取参数取值范围
int SS1PNCodeSetGUI::QueryParaBlock(QString ConfigPath)
{
...
int SS1PNCodeSetGUI::GetAllMapValueStringByPara(int MapSum,QDomNode pInDocNode_Para)
{
 ...
 *           其它错误依情况增加；
 */
int SS1PNCodeSetGUI::GetErrorNumber()
{
    return m_iErrCode;
}
 ...
 */
int SS1PNCodeSetGUI::GetErrorMsg(int iErrorNo, QString& strErrorMsg)
{
 ...
 */
void SS1PNCodeSetGUI::SendMessage(TLxTsspMessage msg)
{
    return;
}
 ...
 * @return 1：成功；-1：失败。
 */
int SS1PNCodeSetGUI::SetParameter(QMap<quint64, QByteArray> parameterList)
{
 ...
 */
int SS1PNCodeSetGUI::SetParameter(quint64 ui64ParamID, QByteArray parameter)
{
 ...
 */
int SS1PNCodeSetGUI::GetParameterID(QList<quint64>& pParameterIDList) const
{
 ...
 * @return 大于等于0：参数块个数。-1：失败。
 */
int SS1PNCodeSetGUI::GetParameter(QMap<quint64, QByteArray>& parameterList) const
{
    SMCU::MCSSSPNCODEParamStruct4 tmpComPara;          //参数

    SMCU::MCSSSPNCODEStatusStruct4 *tmpComStatus = NULL;  //状态参数
    ...
    if(tmpStateMgr != NULL)
        tmpComStatus  =
                (SMCU::MCSSSPNCODEStatusStruct4*)tmpStateMgr->FindOneItem(m_StatusComSynID, stLen);

    if(tmpComStatus != NULL)
    {
        for (int i = 0; i< MAX_TARGET_COUNT; i++)
            tmpComPara.SSPNCODEParam[i] = tmpComStatus->SSPNCODEStatu[i].tParams;
    }

    QByteArray  tmpCh;
    tmpCh.resize(sizeof(SMCU::MCSSSPNCODEParamStruct4));
    memcpy(tmpCh.data(), &tmpComPara, sizeof(SMCU::MCSSSPNCODEParamStruct4));
 ...
 * @return 1：成功；-1：失败。
 */
int SS1PNCodeSetGUI::GetParameter(quint64 ui64ParamID, QByteArray& parameter) const
{
    if(m_ParamList.contains(ui64ParamID))
    {
 ...
 */
int SS1PNCodeSetGUI::GetState(QMap<quint64, TLxTsspDeviceState>& stateList) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int SS1PNCodeSetGUI::GetState(quint64 ui64StatusID, TLxTsspDeviceState& status) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int SS1PNCodeSetGUI::GetStateDirect(quint64 ui64StatusID, TLxTsspDeviceState& status)
{
    if(m_StateList.contains(ui64StatusID))
    {
        //
 ...
 */
int SS1PNCodeSetGUI::WriteData(quint64 ui64DataID, uchar* aData, uint uiDataLen)
{
    return 0;
}
 ...
 */
int SS1PNCodeSetGUI::ReadData(quint64 ui64DataID, uchar* aData, uint& uiDataLen,
                             uint& uiReloadCount)
{
    return 0;
}

int SS1PNCodeSetGUI::CreateDefaultMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    SMCU::MCSSSPNCODEParamStruct4 tmpComPara;          //参数
    for(int nIndex = 0; nIndex < MAX_TARGET_COUNT; nIndex++)
    {
        /********************************目标受伪码*********************************/
        //伪码来源
        //tmpComPara.SSPNCODEParam[nIndex].ucPNSource = m_ParaSetStruct[nIndex][0].InitVal.lval;
        //上行短码伪码初相1
        tmpComPara.SSPNCODEParam[nIndex].usUpTCSPNInitPhaseA = m_ParaSetStruct[nIndex][1].InitVal.lval;
        //上行短码伪码初相2
        tmpComPara.SSPNCODEParam[nIndex].usUpTCSPNInitPhaseB = m_ParaSetStruct[nIndex][2].InitVal.lval;
        //上行短码伪码本源多项式1
        tmpComPara.SSPNCODEParam[nIndex].usUpTCSPNPolyA = m_ParaSetStruct[nIndex][3].InitVal.lval;
        //上行短码伪码本源多项式2
        tmpComPara.SSPNCODEParam[nIndex].usUpTCSPNPolyB = m_ParaSetStruct[nIndex][4].InitVal.lval;
        //上行长码伪码初相1
        tmpComPara.SSPNCODEParam[nIndex].usUpMessSPNInitPhaseA = m_ParaSetStruct[nIndex][5].InitVal.lval;
        //上行长码伪码本源多项式1
        tmpComPara.SSPNCODEParam[nIndex].usUpMessSPNPolyA = m_ParaSetStruct[nIndex][6].InitVal.lval;
        //下行短码伪码初相1
        tmpComPara.SSPNCODEParam[nIndex].usDnTCSPNInitPhaseA = m_ParaSetStruct[nIndex][7].InitVal.lval;
        //下行短码伪码初相2
        tmpComPara.SSPNCODEParam[nIndex].usDnTCSPNInitPhaseB = m_ParaSetStruct[nIndex][8].InitVal.lval;
        //下行短码伪码本源多项式1
        tmpComPara.SSPNCODEParam[nIndex].usDnTCSPNPolyA = m_ParaSetStruct[nIndex][9].InitVal.lval;
        //下行短码伪码本源多项式2
        tmpComPara.SSPNCODEParam[nIndex].usDnTCSPNPolyB = m_ParaSetStruct[nIndex][10].InitVal.lval;
        //下行长码伪码初相1
        tmpComPara.SSPNCODEParam[nIndex].usDnMessSPNInitPhaseA = m_ParaSetStruct[nIndex][11].InitVal.lval;
        //下行长码伪码本源多项式1
        tmpComPara.SSPNCODEParam[nIndex].usDnMessSPNPolyA = m_ParaSetStruct[nIndex][12].InitVal.lval;
    }
...
QWidget* SS1PNCodeSetGUI::ShowMeInEdit(QWidget* parent, uchar ucType, ILxTsspParameterMacro* pMacroObj)
{
    g_form = new SS1PNCodeSetGUIDlg(parent);
...
int SS1PNCodeSetGUI::SaveToMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    if(g_form != NULL)
    {
