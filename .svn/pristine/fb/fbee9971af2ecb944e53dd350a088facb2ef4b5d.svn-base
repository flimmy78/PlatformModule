...
/**
 * 显示界面 ：显示界面，
#include "../Common/DeviceMap.h"当外部对象调用此函数时，本对象应弹出对应的界面。
 * @param parentForm：父窗口。
 * @return 界面对象的界面。
*/
QWidget* Net_SwitchGUI::ShowMe(QWidget* parent, const QString& strTitle)
{
    m_strTitle = strTitle;

    Net_SwitchGUIDlg* form;

    form = new Net_SwitchGUIDlg(parent);
 ...
 * @return szHelpID:长度。
*/
int Net_SwitchGUI::GetContextHelpID(char* szHelpID)
{
...
*/
int Net_SwitchGUI::GetModuleInfo(TLxTsspModuleInfo& moduleInfo) const
{
    ...
    /// 配置标识，如：D_Y17-61_008
    strcpy(moduleInfo.szConfigID, "D_Y17-92_127\0");
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int Net_SwitchGUI::GetObjectInfo(TLxTsspObjectInfo& ObjectInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int Net_SwitchGUI::PreInitialize()
{
    return 1;
}
...
//遍历xml文件，获取参数取值范围
int Net_SwitchGUI::QueryParaBlock(QString ConfigPath)
{
...
int Net_SwitchGUI::GetAllMapValueStringByPara(int MapSum,QDomNode pInDocNode_Para)
{
 ...
 *           其它错误依情况增加；
 */
int Net_SwitchGUI::GetErrorNumber()
{
    return m_iErrCode;
}
 ...
 */
int Net_SwitchGUI::GetErrorMsg(int iErrorNo, QString& strErrorMsg)
{
 ...
 */
void Net_SwitchGUI::SendMessage(TLxTsspMessage msg)
{
    return;
}
 ...
 * @return 1：成功；-1：失败。
 */
int Net_SwitchGUI::SetParameter(QMap<quint64, QByteArray> parameterList)
{
 ...
 */
int Net_SwitchGUI::SetParameter(quint64 ui64ParamID, QByteArray parameter)
{
 ...
 */
int Net_SwitchGUI::GetParameterID(QList<quint64>& pParameterIDList) const
{
 ...
 * @return 大于等于0：参数块个数。-1：失败。
 */
int Net_SwitchGUI::GetParameter(QMap<quint64, QByteArray>& parameterList) const
{
    HK_IFSWMatPara tmpComPara;          //参数

    HK_IFSWMatStatu *tmpComStatus = NULL;  //状态
    ...
    if(tmpStateMgr != NULL)
        tmpComStatus  =
                (HK_IFSWMatStatu*)tmpStateMgr->FindOneItem(m_StatusComSynID, stLen);
    ...
    QByteArray  tmpCh;
    tmpCh.resize(sizeof(HK_IFSWMatPara));
    memcpy(tmpCh.data(), &tmpComPara, sizeof(HK_IFSWMatPara));
 ...
 * @return 1：成功；-1：失败。
 */
int Net_SwitchGUI::GetParameter(quint64 ui64ParamID, QByteArray& parameter) const
{
    if(m_ParamList.contains(ui64ParamID))
    {
 ...
 */
int Net_SwitchGUI::GetState(QMap<quint64, TLxTsspDeviceState>& stateList) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int Net_SwitchGUI::GetState(quint64 ui64StatusID, TLxTsspDeviceState& status) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int Net_SwitchGUI::GetStateDirect(quint64 ui64StatusID, TLxTsspDeviceState& status)
{
    if(m_StateList.contains(ui64StatusID))
    {
        //
 ...
 */
int Net_SwitchGUI::WriteData(quint64 ui64DataID, uchar* aData, uint uiDataLen)
{
    return 0;
}
 ...
 */
int Net_SwitchGUI::ReadData(quint64 ui64DataID, uchar* aData, uint& uiDataLen,
                             uint& uiReloadCount)
{
    return 0;
}

int Net_SwitchGUI::CreateDefaultMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    HK_IFSWMatPara DivComPara;

    //取当前设置的参数，存入框架
    QByteArray byteArray;
    byteArray.resize(sizeof(HK_IFSWMatPara));
    memcpy(byteArray.data(), &DivComPara, sizeof(HK_IFSWMatPara));
    if(pMacroObj != NULL)
    {
...
QWidget* Net_SwitchGUI::ShowMeInEdit(QWidget* parent,uchar ucType,ILxTsspParameterMacro* pMacroObj)
{
    g_form = new Net_SwitchGUIDlg(parent);
...
int Net_SwitchGUI::SaveToMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    if(g_form != NULL)
    {
