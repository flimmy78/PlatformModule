...
namespace
{
    SSBBE_OutPutSelSetGUIDlg* g_form = NULL;
}
extern quint64 GetID(ushort usStationID, uchar ucSubSysID, ushort usDeviceID, ushort usType,
              uchar ucSn);
SSBBE_OutPutSelSetGUI::SSBBE_OutPutSelSetGUI(TLxTsspObjectInfo ObjectInfo)
{
    //初始化
 ...
 * @return 界面对象的界面。
*/
QWidget* SSBBE_OutPutSelSetGUI::ShowMe(QWidget* parent, const QString& strTitle)
{
    m_strTitle = strTitle;

    SSBBE_OutPutSelSetGUIDlg* form;

    form = new SSBBE_OutPutSelSetGUIDlg(parent);
 ...
 * @return szHelpID:长度。
*/
int SSBBE_OutPutSelSetGUI::GetContextHelpID(char* szHelpID)
{
...
*/
int SSBBE_OutPutSelSetGUI::GetModuleInfo(TLxTsspModuleInfo& moduleInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SSBBE_OutPutSelSetGUI::GetObjectInfo(TLxTsspObjectInfo& ObjectInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SSBBE_OutPutSelSetGUI::PreInitialize()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SSBBE_OutPutSelSetGUI::Initialize()
{
    //日志信息
    TLxTsspLog tmpLog;
    QString strLog;
    ...
    stCurPath = stCurPath + "/System/ModuleConfig/SS1BBE_OutPutSelSetGUI.xml";
    //遍历xml文件，获取参数取值范围
    m_MapSumFlag = 0;
...
//遍历xml文件，获取参数取值范围
int SSBBE_OutPutSelSetGUI::QueryParaBlock(QString ConfigPath)
{
...
int SSBBE_OutPutSelSetGUI::GetAllMapValueStringByPara(int MapSum,QDomNode pInDocNode_Para)
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SSBBE_OutPutSelSetGUI::Start()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SSBBE_OutPutSelSetGUI::Stop()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
bool SSBBE_OutPutSelSetGUI::ClearUp()
{
    return 1;
}
 ...
 *           其它错误依情况增加；
 */
int SSBBE_OutPutSelSetGUI::GetErrorNumber()
{
    return m_iErrCode;
}
 ...
 */
int SSBBE_OutPutSelSetGUI::GetErrorMsg(int iErrorNo, QString& strErrorMsg)
{
 ...
 */
bool SSBBE_OutPutSelSetGUI::IsOnline()
{
    //对象初始化
    //查找对象管理器
    LxTsspObjectManager* pObjectManager = LxTsspObjectManager::Instance();
 ...
 */
void SSBBE_OutPutSelSetGUI::SendMessage(TLxTsspMessage msg)
{
    return;
}
 ...
 * @return 1：成功；-1：失败。
 */
int SSBBE_OutPutSelSetGUI::SetParameter(QMap<quint64, QByteArray> parameterList)
{
 ...
 */
int SSBBE_OutPutSelSetGUI::SetParameter(quint64 ui64ParamID, QByteArray parameter)
{
 ...
 */
int SSBBE_OutPutSelSetGUI::GetParameterID(QList<quint64>& pParameterIDList) const
{
 ...
 * @return 大于等于0：参数块个数。-1：失败。
 */
int SSBBE_OutPutSelSetGUI::GetParameter(QMap<quint64, QByteArray>& parameterList) const
{
    SMCU::MCSSysTarget4ParamStruct tmpComPara;          //参数

    SMCU::MCSSysTarget4StatusStruct *tmpComStatus = NULL;  //状态参数
    ...
    if(tmpComStatus != NULL)
    {
        for (int i = 0; i< MAX_TARGET_COUNT; i++)
            tmpComPara.targetSysParam[i] = tmpComStatus->targetSysStatu[i].tParams;
    }
 ...
 * @return 1：成功；-1：失败。
 */
int SSBBE_OutPutSelSetGUI::GetParameter(quint64 ui64ParamID, QByteArray& parameter) const
{
    if(m_ParamList.contains(ui64ParamID))
    {
 ...
 */
int SSBBE_OutPutSelSetGUI::GetState(QMap<quint64, TLxTsspDeviceState>& stateList) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int SSBBE_OutPutSelSetGUI::GetState(quint64 ui64StatusID, TLxTsspDeviceState& status) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int SSBBE_OutPutSelSetGUI::GetStateDirect(quint64 ui64StatusID, TLxTsspDeviceState& status)
{
    if(m_StateList.contains(ui64StatusID))
    {
        //
 ...
 */
int SSBBE_OutPutSelSetGUI::WriteData(quint64 ui64DataID, uchar* aData, uint uiDataLen)
{
    return 0;
}
 ...
 */
int SSBBE_OutPutSelSetGUI::ReadData(quint64 ui64DataID, uchar* aData, uint& uiDataLen,
                             uint& uiReloadCount)
{
    return 0;
}

int SSBBE_OutPutSelSetGUI::CreateDefaultMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    SMCU::MCSSysTarget4ParamStruct tmpComPara;          //参数
    ...
    //输出方式
    tmpComPara.sysParam.ucTrackOutType = m_ParaSetStruct[0][0].InitVal.lval + 1;
    //输出通道
    tmpComPara.sysParam.ucTrackChnlSel = m_ParaSetStruct[0][2].InitVal.lval + 1;
    //输出目标
    tmpComPara.sysParam.ucTrackDefCH = m_ParaSetStruct[0][1].InitVal.lval;
...
QWidget* SSBBE_OutPutSelSetGUI::ShowMeInEdit(QWidget* parent, uchar ucType, ILxTsspParameterMacro* pMacroObj)
{
    g_form = new SSBBE_OutPutSelSetGUIDlg(parent);
...
int SSBBE_OutPutSelSetGUI::SaveToMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    if(g_form != NULL)
    {
