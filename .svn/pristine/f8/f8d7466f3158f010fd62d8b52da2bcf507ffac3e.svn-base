#include "SS1BBE_ModuParamSetGUI.h"
...
namespace
{
    SS1BBE_ModuParamSetGUIDlg* g_form = NULL;
}
extern quint64 GetID(ushort usStationID, uchar ucSubSysID, ushort usDeviceID, ushort usType,
              uchar ucSn);
SS1BBE_ModuParamSetGUI::SS1BBE_ModuParamSetGUI(TLxTsspObjectInfo ObjectInfo)
{
    //初始化
 ...
 * @return 界面对象的界面。
*/
QWidget* SS1BBE_ModuParamSetGUI::ShowMe(QWidget* parent, const QString& strTitle)
{
    m_strTitle = strTitle;

    SS1BBE_ModuParamSetGUIDlg* form;

    form = new SS1BBE_ModuParamSetGUIDlg(parent);
 ...
 * @return szHelpID:长度。
*/
int SS1BBE_ModuParamSetGUI::GetContextHelpID(char* szHelpID)
{
...
*/
int SS1BBE_ModuParamSetGUI::GetModuleInfo(TLxTsspModuleInfo& moduleInfo) const
{
    ...
    /// 配置标识，如：D_Y17-61_008
    strcpy(moduleInfo.szConfigID, "D_Y17-92_112\0");
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS1BBE_ModuParamSetGUI::GetObjectInfo(TLxTsspObjectInfo& ObjectInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS1BBE_ModuParamSetGUI::PreInitialize()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS1BBE_ModuParamSetGUI::Initialize()
{
    memset(&m_doppler, 0, sizeof(m_doppler));
    //日志信息
    TLxTsspLog tmpLog;
    QString strLog;
    ...
    stCurPath = stCurPath + "/System/ModuleConfig/SS1BBE_ModuParamSetGUI.xml";
    //遍历xml文件，获取参数取值范围
    m_MapSumFlag = 0;
    ...
    if (pItem->vSubItem.size() > 0)
    {
        ...
        /*************************框架**********************************/
        m_ParaModuID = GetID(sItem.usStationID, sItem.ucSubSysID, sItem.usDeviceID,
                                     sItem.usTID, sItem.ucSN);
        /*************************框架**********************************/

        /*************************状态**********************************/
        m_StatusModuID = GetID(sItem.usStationID, sItem.ucSubSysID, sItem.usDeviceID,
...
//遍历xml文件，获取参数取值范围
int SS1BBE_ModuParamSetGUI::QueryParaBlock(QString ConfigPath)
{
...
int SS1BBE_ModuParamSetGUI::GetAllMapValueStringByPara(int MapSum,QDomNode pInDocNode_Para)
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS1BBE_ModuParamSetGUI::Start()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS1BBE_ModuParamSetGUI::Stop()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
bool SS1BBE_ModuParamSetGUI::ClearUp()
{
    return 1;
}
 ...
 *           其它错误依情况增加；
 */
int SS1BBE_ModuParamSetGUI::GetErrorNumber()
{
    return m_iErrCode;
}
 ...
 */
int SS1BBE_ModuParamSetGUI::GetErrorMsg(int iErrorNo, QString& strErrorMsg)
{
 ...
 */
bool SS1BBE_ModuParamSetGUI::IsOnline()
{
    //对象初始化
    //查找对象管理器
    LxTsspObjectManager* pObjectManager = LxTsspObjectManager::Instance();
 ...
 */
void SS1BBE_ModuParamSetGUI::SendMessage(TLxTsspMessage msg)
{
    return;
}
 ...
 * @return 1：成功；-1：失败。
 */
int SS1BBE_ModuParamSetGUI::SetParameter(QMap<quint64, QByteArray> parameterList)
{
 ...
 */
int SS1BBE_ModuParamSetGUI::SetParameter(quint64 ui64ParamID, QByteArray parameter)
{
 ...
 */
int SS1BBE_ModuParamSetGUI::GetParameterID(QList<quint64>& pParameterIDList) const
{
 ...
 * @return 大于等于0：参数块个数。-1：失败。
 */
int SS1BBE_ModuParamSetGUI::GetParameter(QMap<quint64, QByteArray>& parameterList) const
{
    SMCU::MCSSSUTargetTCSSUpModulParamsStruct tmpComPara;   //参数
    SMCU::MCSSSUTargetTCSSUpModulStatusStruct* tmpComStatus = NULL; //状态
    ...
    if(tmpStateMgr != NULL)
    {
        tmpComStatus = (SMCU::MCSSSUTargetTCSSUpModulStatusStruct*)tmpStateMgr->FindOneItem(m_StatusModuID, stLen);
    }

    if(tmpComStatus != NULL)
    {
        tmpComPara.SSUpModulParam = tmpComStatus->SSUpModulStatu.tParams;
        for (int i = 0; i < MAX_TARGET_COUNT; i++)
        {
            tmpComPara.targetTCSSUpModulParam[i] = tmpComStatus->targetTCSSUpModulStatu[i].tParams;
        }
    }
    QByteArray  tmpCh;
    tmpCh.resize(sizeof(SMCU::MCSSSUTargetTCSSUpModulParamsStruct));
    memcpy(tmpCh.data(), &tmpComPara, sizeof(SMCU::MCSSSUTargetTCSSUpModulParamsStruct));
    parameterList.insert(m_ParaModuID, tmpCh);
 ...
 * @return 1：成功；-1：失败。
 */
int SS1BBE_ModuParamSetGUI::GetParameter(quint64 ui64ParamID, QByteArray& parameter) const
{
    if(m_ParamList.contains(ui64ParamID))
    {
 ...
 */
int SS1BBE_ModuParamSetGUI::GetState(QMap<quint64, TLxTsspDeviceState>& stateList) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int SS1BBE_ModuParamSetGUI::GetState(quint64 ui64StatusID, TLxTsspDeviceState& status) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int SS1BBE_ModuParamSetGUI::GetStateDirect(quint64 ui64StatusID, TLxTsspDeviceState& status)
{
    if(m_StateList.contains(ui64StatusID))
    {
        //
 ...
 */
int SS1BBE_ModuParamSetGUI::WriteData(quint64 ui64DataID, uchar* aData, uint uiDataLen)
{
    return 0;
}
 ...
 */
int SS1BBE_ModuParamSetGUI::ReadData(quint64 ui64DataID, uchar* aData, uint& uiDataLen,
                             uint& uiReloadCount)
{
    return 0;
}

int SS1BBE_ModuParamSetGUI::CreateDefaultMacroObj(ILxTsspParameterMacro* pMacroObj)
{

    SMCU::MCSSSUTargetTCSSUpModulParamsStruct m_MCSSSUTargetModulParam;
    /*公共参数*/
    //载波输出电平
    m_MCSSSUTargetModulParam.SSUpModulParam.cWaveOutLevel = m_ParaSetStruct[0][0].InitVal.lval;
    //调制方式
    m_MCSSSUTargetModulParam.SSUpModulParam.ucModulate = m_ParaSetStruct[0][1].InitVal.lval + 1;
    //载波控制
    m_MCSSSUTargetModulParam.SSUpModulParam.ucWaveOutCtrl = m_ParaSetStruct[0][2].InitVal.lval + 1;
    //噪声源输出
    m_MCSSSUTargetModulParam.SSUpModulParam.ucNoiseSw = m_ParaSetStruct[0][3].InitVal.lval + 1;
    //载波输出载噪比
    m_MCSSSUTargetModulParam.SSUpModulParam.ucCN0 = m_ParaSetStruct[0][4].InitVal.lval;

    /*目标参数*/
    for (int i = 0; i < MAX_TARGET_COUNT; i++)
    {
        //多普勒预置频率
        m_MCSSSUTargetModulParam.targetTCSSUpModulParam[i].ucPreDplValue = m_ParaSetStruct[i + 1][0].InitVal.lval;
        //输出功率衰减
        m_MCSSSUTargetModulParam.targetTCSSUpModulParam[i].cPowAdjust = m_ParaSetStruct[i + 1][1].InitVal.lval;
        //扩频码速率
        m_MCSSSUTargetModulParam.targetTCSSUpModulParam[i].ucPNRate = m_ParaSetStruct[i + 1][2].InitVal.lval + 1;
        //通道输出控制
        m_MCSSSUTargetModulParam.targetTCSSUpModulParam[i].ucOutputSwitch = m_ParaSetStruct[i + 1][3].InitVal.lval + 1;
        //IQ功率比
        m_MCSSSUTargetModulParam.targetTCSSUpModulParam[i].ucIQRatioP = m_ParaSetStruct[i + 1][4].InitVal.lval;
        //I路伪码加调
        m_MCSSSUTargetModulParam.targetTCSSUpModulParam[i].ucPNIModuSw = m_ParaSetStruct[i + 1][5].InitVal.lval + 1;
        //Q路伪码加调
        m_MCSSSUTargetModulParam.targetTCSSUpModulParam[i].ucQPNModuSw = m_ParaSetStruct[i + 1][6].InitVal.lval + 1;
        //数据加调
        m_MCSSSUTargetModulParam.targetTCSSUpModulParam[i].ucTCModuSw = m_ParaSetStruct[i + 1][7].InitVal.lval + 1;
    }

    //取当前设置的参数，存入框架
    QByteArray byteArray;
    byteArray.resize(sizeof(SMCU::MCSSSUTargetTCSSUpModulParamsStruct));
    memcpy(byteArray.data(),&m_MCSSSUTargetModulParam,sizeof(m_MCSSSUTargetModulParam));
    if(pMacroObj != NULL)
    {
        pMacroObj->UpdateParameterBlock(m_ParaModuID,byteArray);
    }
    return 1;
}

QWidget* SS1BBE_ModuParamSetGUI::ShowMeInEdit(QWidget* parent,uchar ucType,ILxTsspParameterMacro* pMacroObj)
{
    g_form = new SS1BBE_ModuParamSetGUIDlg(parent);
...
int SS1BBE_ModuParamSetGUI::SaveToMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    if(g_form!=NULL)
    {
