...
namespace
{
SK_BBEDivCombaraSetGUIDlg* g_form = NULL;
}
extern quint64 GetID(ushort usStationID, uchar ucSubSysID, ushort usDeviceID, ushort usType,
                     uchar ucSn);
SK_BBEDivCombaraSetGUI::SK_BBEDivCombaraSetGUI(TLxTsspObjectInfo ObjectInfo)
{
    //初始化
 ...
 * @return 界面对象的界面。
*/
QWidget* SK_BBEDivCombaraSetGUI::ShowMe(QWidget* parent, const QString& strTitle)
{
    m_strTitle = strTitle;

    SK_BBEDivCombaraSetGUIDlg* form;

    form = new SK_BBEDivCombaraSetGUIDlg(parent);
 ...
 * @return szHelpID:长度。
*/
int SK_BBEDivCombaraSetGUI::GetContextHelpID(char* szHelpID)
{
...
*/
int SK_BBEDivCombaraSetGUI::GetModuleInfo(TLxTsspModuleInfo& moduleInfo) const
{
    ...
    /// 配置标识，如：D_Y17-61_008
    strcpy(moduleInfo.szConfigID, "D_Y17-92_204\0");
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SK_BBEDivCombaraSetGUI::GetObjectInfo(TLxTsspObjectInfo& ObjectInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SK_BBEDivCombaraSetGUI::PreInitialize()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SK_BBEDivCombaraSetGUI::Initialize()
{
    //日志信息
    TLxTsspLog tmpLog;
    QString strLog;
    ...
    stCurPath = stCurPath + "/System/ModuleConfig/SK_BBEDivCombaraSetGUI.xml";
    //遍历xlm文件，获取参数取值范围
    m_MapSumFlag = 0;
...
//遍历xlm文件，获取参数取值范围
int SK_BBEDivCombaraSetGUI::QueryParaBlock(QString ConfigPath)
{
        ...
        if(nodeList.item(j).nodeName().compare("参数块") == 0)
        {
            ...
            for(int i=0; i<nodeListchild.length(); i++)
            {
                ...
                if(strcmp(m_ParaSetStruct[iCountParaBlock][i].cParaType,"float") == 0)
                {
                    ...
                    m_ParaSetStruct[iCountParaBlock][i].InitVal.fval = nodeListchild.item(i).childNodes().item(0).toElement().text().toFloat();
                }
                else if(strcmp(m_ParaSetStruct[iCountParaBlock][i].cParaType,"double") == 0)
                {
                    ...
                    m_ParaSetStruct[iCountParaBlock][i].InitVal.dval = nodeListchild.item(i).childNodes().item(0).toElement().text().toDouble();
                }
                else
                {
...
int SK_BBEDivCombaraSetGUI::GetAllMapValueStringByPara(int MapSum,QDomNode pInDocNode_Para)
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SK_BBEDivCombaraSetGUI::Start()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SK_BBEDivCombaraSetGUI::Stop()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
bool SK_BBEDivCombaraSetGUI::ClearUp()
{
    return 1;
}
 ...
 *           其它错误依情况增加；
 */
int SK_BBEDivCombaraSetGUI::GetErrorNumber()
{
    return m_iErrCode;
}
 ...
 */
int SK_BBEDivCombaraSetGUI::GetErrorMsg(int iErrorNo, QString& strErrorMsg)
{
 ...
 */
bool SK_BBEDivCombaraSetGUI::IsOnline()
{
    //对象初始化
    //查找对象管理器
    LxTsspObjectManager* pObjectManager = LxTsspObjectManager::Instance();
 ...
 */
void SK_BBEDivCombaraSetGUI::SendMessage(TLxTsspMessage msg)
{
    return;
}
 ...
 * @return 1：成功；-1：失败。
 */
int SK_BBEDivCombaraSetGUI::SetParameter(QMap<quint64, QByteArray> parameterList)
{
 ...
 */
int SK_BBEDivCombaraSetGUI::SetParameter(quint64 ui64ParamID, QByteArray parameter)
{
 ...
 */
int SK_BBEDivCombaraSetGUI::GetParameterID(QList<quint64>& pParameterIDList) const
{
 ...
 * @return 大于等于0：参数块个数。-1：失败。
 */
int SK_BBEDivCombaraSetGUI::GetParameter(QMap<quint64, QByteArray>& parameterList) const
{
    MCSDivComParamStruct tmpDivComPara;          //分集合成参数

    MCSDivComStatusStruct *tmpComParamStatus = NULL;  //分集合成状态参数
    ...
    if(tmpStateMgr != NULL)
        tmpComParamStatus  =
                (MCSDivComStatusStruct*)tmpStateMgr->FindOneItem(m_StatusMCSDivComSynID, stLen);

    if(tmpComParamStatus != NULL)
    {
        tmpDivComPara = tmpComParamStatus->tParams;
    }

    QByteArray  tmpCh;
    tmpCh.resize(sizeof(MCSDivComParamStruct));
    memcpy(tmpCh.data(), &tmpDivComPara, sizeof(MCSDivComParamStruct));
    parameterList.insert(m_ParaMCSDivComSynID, tmpCh);
 ...
 * @return 1：成功；-1：失败。
 */
int SK_BBEDivCombaraSetGUI::GetParameter(quint64 ui64ParamID, QByteArray& parameter) const
{
    if(m_ParamList.contains(ui64ParamID))
    {
 ...
 */
int SK_BBEDivCombaraSetGUI::GetState(QMap<quint64, TLxTsspDeviceState>& stateList) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int SK_BBEDivCombaraSetGUI::GetState(quint64 ui64StatusID, TLxTsspDeviceState& status) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int SK_BBEDivCombaraSetGUI::GetStateDirect(quint64 ui64StatusID, TLxTsspDeviceState& status)
{
    if(m_StateList.contains(ui64StatusID))
    {
        //
 ...
 */
int SK_BBEDivCombaraSetGUI::WriteData(quint64 ui64DataID, uchar* aData, uint uiDataLen)
{
    return 0;
}
 ...
 */
int SK_BBEDivCombaraSetGUI::ReadData(quint64 ui64DataID, uchar* aData, uint& uiDataLen,
                                     uint& uiReloadCount)
{
    return 0;
}

int SK_BBEDivCombaraSetGUI::CreateDefaultMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    MCSDivComParamStruct DivComPara;          //分集合成参数
    //合成参数
    DivComPara.ucCombType = m_ParaSetStruct[0][0].InitVal.lval + 1;
    //差模环路带宽
    DivComPara.usDiffPllBW = (USHORT)m_ParaSetStruct[0][1].InitVal.fval * 10;
    //取当前设置的参数，存入框架
    QByteArray byteArray;
    byteArray.resize(sizeof(MCSDivComParamStruct));
    memcpy(byteArray.data(), &DivComPara, sizeof(DivComPara));
    if(pMacroObj != NULL)
    {
...
QWidget* SK_BBEDivCombaraSetGUI::ShowMeInEdit(QWidget* parent,uchar ucType,ILxTsspParameterMacro* pMacroObj)
{
    g_form = new SK_BBEDivCombaraSetGUIDlg(parent);
...
int SK_BBEDivCombaraSetGUI::SaveToMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    if(g_form != NULL)
    {
