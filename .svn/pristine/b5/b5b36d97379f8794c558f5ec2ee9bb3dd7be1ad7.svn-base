#ifndef SS1BBE_MODUPARAMSETGUIDLG_H
#define SS1BBE_MODUPARAMSETGUIDLG_H
...
namespace Ui {
class SS1BBE_ModuParamSetGUIDlg;
}

class SS1BBE_ModuParamSetGUIDlg : public QDialog
{
    Q_OBJECT

public:
    explicit SS1BBE_ModuParamSetGUIDlg(QWidget *parent = 0);
    QSize sizeHint() const;
    ~SS1BBE_ModuParamSetGUIDlg();
...
private:
    Ui::SS1BBE_ModuParamSetGUIDlg *ui;

    QString m_strTitle;
    TLxTsspObjectInfo m_ObjectInfo;
    ILxTsspLogManager* m_pILxTsspLogManager;

//    SMCU::MCSSSUpModulParamStruct m_MCSSSUpModulParam;       //公共参数
//    SMCU::MCSTargetTCSSUpModulParamStruct m_MCSTargetTCSSUpModulParam;     //目标参数

//    SMCU::MCSSSUpModulStatusStruct* m_MCSSSUpModulStatus;       //公共参数状态
//    SMCU::MCSTargetTCSSUpModulStatusStruct* m_MCSTargetTCSSUpModulStatus;     //目标参数状态

    SMCU::MCSSSUTargetTCSSUpModulParamsStruct m_MCSSSUTargetTCSSUpModulParam;
    SMCU::MCSSSUTargetTCSSUpModulStatusStruct* m_MCSSSUTargetTCSSUpModulStatus;
    ...
    quint64 m_ParaMCSSSUTargetTCSSUpModulID;          //参数ID

    quint64 m_StatusParaMCSSSUTargetTCSSUpModulID;    //状态ID
...
public:
    ...
    //设置对象信息
    void SetObjInfo(TLxTsspObjectInfo ObjectInfo, DOPPLERCOMPENSATE& m_doppler);
    //保存参数宏
    ...
    void SendMessage(char* pData, unsigned short  usLen, unsigned short usC, unsigned short usTID, unsigned char ucSN = 0);
    //获取当前目标的值并赋值到结构体
    void getParamTarget();
...
private slots:
    ...
    void on_pushButton_ModifyCommon_clicked();
    void on_pushButton_SetCommon_clicked();
    void on_comboBox_NoiseSourOutput_currentIndexChanged(int index);
    void on_comboBox_CompensateType_currentIndexChanged(int index);
    void on_comboBox_ChannelOutputCtrl_currentIndexChanged(int index);

    void on_checkBox_dopplerSw_clicked();
};

#endif // SS1BBE_MODUPARAMSETGUIDLG_H
