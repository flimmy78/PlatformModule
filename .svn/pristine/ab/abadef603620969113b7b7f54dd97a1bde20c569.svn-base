...
extern "C" int GetConfigFormat(QString& strConfigFormat)
{
    ...
    strConfigFormat = QString("<Module Name=\"libSS1BBE_RcvParamSet.so\" StationID=\"2311\" SubSysID=\"04\" DevID=\"0401\" Note=\"%1\"> \n "
    ...
    for(int i=0; i<nodeList.length(); i++)
    {
        ...
        if(usObjID == 0x5146)
        {
            for(int k=iStart; k<iStop; k++)
            {
                ...
                strcpy(ObjectInfo.szConfigID, "D_Y17-92_111");
                strcpy(ObjectInfo.szAlias, objAlias.toUtf8().data());
                strcpy(ObjectInfo.szComment, objNote.toUtf8().data());
                g_ObjectList.insert(ui64ObjectID, new SS1BBE_RcvParamSetGUI(ObjectInfo));
            }
        }
    }
