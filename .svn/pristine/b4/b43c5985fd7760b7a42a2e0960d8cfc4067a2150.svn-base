 ...
 * @return 界面对象的界面。
*/
QWidget* ZEROSTDDcuGUI::ShowMe(QWidget* parent, const QString& strTitle)
{
    m_strTitle = strTitle;

    ZEROSTDDcuGUIDlg* form;

    form = new ZEROSTDDcuGUIDlg(parent);
 ...
 * @return szHelpID:长度。
*/
int ZEROSTDDcuGUI::GetContextHelpID(char* szHelpID)
{
...
*/
int ZEROSTDDcuGUI::GetModuleInfo(TLxTsspModuleInfo& moduleInfo) const
{
    ...
    /// 配置标识，如：D_Y17-61_008
    strcpy(moduleInfo.szConfigID, "D_Y17-92_132\0");
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int ZEROSTDDcuGUI::GetObjectInfo(TLxTsspObjectInfo& ObjectInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int ZEROSTDDcuGUI::PreInitialize()
{
    return 1;
}
...
int ZEROSTDDcuGUI::Initialize()
{
    ...
    stCurPath = stCurPath + "/System/ModuleConfig/ZEROSTDDcuGUI.xml";
    //遍历xlm文件，获取参数取值范围
    m_MapSumFlag = 0;
...
//遍历xlm文件，获取参数取值范围
int ZEROSTDDcuGUI::QueryParaBlock(QString ConfigPath)
{
...
int ZEROSTDDcuGUI::GetAllMapValueStringByPara(int MapSum,QDomNode pInDocNode_Para)
{
 ...
 *           其它错误依情况增加；
 */
int ZEROSTDDcuGUI::GetErrorNumber()
{
    return m_iErrCode;
}
 ...
 */
int ZEROSTDDcuGUI::GetErrorMsg(int iErrorNo, QString& strErrorMsg)
{
 ...
 */
void ZEROSTDDcuGUI::SendMessage(TLxTsspMessage msg)
{
    return;
}
 ...
 * @return 1：成功；-1：失败。
 */
int ZEROSTDDcuGUI::SetParameter(QMap<quint64, QByteArray> parameterList)
{
 ...
 */
int ZEROSTDDcuGUI::SetParameter(quint64 ui64ParamID, QByteArray parameter)
{
 ...
 */
int ZEROSTDDcuGUI::GetParameterID(QList<quint64>& pParameterIDList) const
{
 ...
 * @return 大于等于0：参数块个数。-1：失败。
 */
int ZEROSTDDcuGUI::GetParameter(QMap<quint64, QByteArray>& parameterList) const
{
    ZEROCALIBBEACONPara tmpComPara;          //参数

    ZEROCALIBBEACONStatu *tmpComStatus = NULL;  //状态参数
    ...
    if(tmpStateMgr != NULL)
        tmpComStatus  =
                (ZEROCALIBBEACONStatu*)tmpStateMgr->FindOneItem(m_StatusComSynID, stLen);

    if(tmpComStatus != NULL)
    {
        tmpComPara = tmpComStatus->tParam;
    }

    QByteArray  tmpCh;
    tmpCh.resize(sizeof(ZEROCALIBBEACONPara));
    memcpy(tmpCh.data(), &tmpComPara, sizeof(ZEROCALIBBEACONPara));
 ...
 * @return 1：成功；-1：失败。
 */
int ZEROSTDDcuGUI::GetParameter(quint64 ui64ParamID, QByteArray& parameter) const
{
    if(m_ParamList.contains(ui64ParamID))
    {
 ...
 */
int ZEROSTDDcuGUI::GetState(QMap<quint64, TLxTsspDeviceState>& stateList) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int ZEROSTDDcuGUI::GetState(quint64 ui64StatusID, TLxTsspDeviceState& status) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int ZEROSTDDcuGUI::GetStateDirect(quint64 ui64StatusID, TLxTsspDeviceState& status)
{
    if(m_StateList.contains(ui64StatusID))
    {
        //
 ...
 */
int ZEROSTDDcuGUI::WriteData(quint64 ui64DataID, uchar* aData, uint uiDataLen)
{
    return 0;
}
 ...
 */
int ZEROSTDDcuGUI::ReadData(quint64 ui64DataID, uchar* aData, uint& uiDataLen,
                             uint& uiReloadCount)
{
    return 0;
}

int ZEROSTDDcuGUI::CreateDefaultMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    ZEROCALIBBEACONPara tmpComPara;          //参数
    //在线标记
    tmpComPara.ucOnlineMark = m_ParaSetStruct[0][0].InitVal.ival;
    //输入射频频率
    tmpComPara.ucInputFreq = m_ParaSetStruct[0][1].InitVal.ival;
    //输出射频频率
    tmpComPara.ucOutputFreq = m_ParaSetStruct[0][2].InitVal.ival;
    //信标输出频率
    tmpComPara.ucBeaconFreq = m_ParaSetStruct[0][3].InitVal.ival;
    //数控衰减器1
    tmpComPara.sAttenuation1 = m_ParaSetStruct[0][4].InitVal.ival;
    //数控衰减器2
    tmpComPara.sAttenuation2 = m_ParaSetStruct[0][5].InitVal.ival;
    //参考源选择
    tmpComPara.usReferentSel = m_ParaSetStruct[0][6].InitVal.ival;
    //取当前设置的参数，存入框架
    QByteArray byteArray;
    byteArray.resize(sizeof(ZEROCALIBBEACONPara));
    memcpy(byteArray.data(), &tmpComPara, sizeof(ZEROCALIBBEACONPara));
    if(pMacroObj != NULL)
    {
...
QWidget* ZEROSTDDcuGUI::ShowMeInEdit(QWidget* parent,uchar ucType,ILxTsspParameterMacro* pMacroObj)
{
    g_form = new ZEROSTDDcuGUIDlg(parent);
...
int ZEROSTDDcuGUI::SaveToMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    if(g_form != NULL)
    {
