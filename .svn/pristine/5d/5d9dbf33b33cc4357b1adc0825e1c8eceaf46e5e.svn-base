...
extern "C" int GetConfigFormat(QString& strConfigFormat)
{
    ...
    strConfigFormat =
            "<Module Name=\"SS2BBE_ModuParamSetGUI.so\" StationID=\"2311\" SubSysID=\"04\" DevID=\"0401\" Note=\"BPSK遥测解调参数设置\"> \n \
    ...
    for(int i=0; i<nodeList.length(); i++)
    {
        ...
        if(usObjID == 0x5158)
        {
            for(int k=iStart; k<iStop; k++)
            {
                ...
                g_ObjectList.insert(ui64ObjectID, new SS2BBE_ModuParamSetGUI(ObjectInfo));
            }
        }
    }
