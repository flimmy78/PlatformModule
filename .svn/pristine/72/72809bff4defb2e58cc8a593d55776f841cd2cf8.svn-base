#ifndef UPNET_SWITCHGUIDLG_H
#define UPNET_SWITCHGUIDLG_H

#include <QComboBox>
#include <QDialog>
#include <QLineEdit>
#include <QTextCodec>
#include "UPNet_SwitchGUI.h"
#include "XmlReader.h"
...
class UPNet_SwitchGUIDlg : public QDialog
{
    Q_OBJECT
public:
    explicit UPNet_SwitchGUIDlg(QWidget *parent = 0);
...
private:
    ...
    //获取保存后的文件内容
    void loadSaveInfo(QString strPath);
    void saveData2File(QString strPath, const QString &name);
    //将信息加载到列表框控件中
    void updateCombo();
    //设置显示的信息到界面的右侧显示控件上
    void setShowInfo(QStringList infoList);
    //初始发送的指令信息
