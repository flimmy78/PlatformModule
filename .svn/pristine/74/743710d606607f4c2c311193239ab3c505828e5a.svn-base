...
#include "SS2BBE_TMSimu1ParaSetGUI_global.h"
#include "../../Common/LxTsspCommonDefinition.h"

#include "../../Common/DataStruct2311.h"
#include "../../Common/DeviceMap.h"
#define MODULENAME ("扩频遥测模拟源参数设置")

class SS2BBE_TMSIMU1PARASETGUISHARED_EXPORT SS2BBE_TMSimu1ParaSetGUI : public ILxTsspShow
{

public:
    SS2BBE_TMSimu1ParaSetGUI(TLxTsspObjectInfo ObjectInfo);
...
protected:
    ...
    quint64 m_ParaTMSiID;      //参数框架ID
    quint64 m_StatusTMSiID;    //参数状态ID

    ParaStruct m_ParaSetStruct[4][60];  //保存参数描述信息
    QString m_MapMean[100][30];      //用于保存map映射类数据项的含义值，往控件中写入
    int MapLineNum[100];     //用于保存各个map映射类数据项的行数（即映射项数）。(数组长度可根据xml文件中map类型的个数自定义)
