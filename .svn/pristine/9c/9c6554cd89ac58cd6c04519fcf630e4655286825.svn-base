...
namespace
{
SS2PNCodeSetGUIDlg* g_form = NULL;
}
extern quint64 GetID(ushort usStationID, uchar ucSubSysID, ushort usDeviceID, ushort usType,
                     uchar ucSn);
SS2PNCodeSetGUI::SS2PNCodeSetGUI(TLxTsspObjectInfo ObjectInfo)
{
    //初始化
 ...
 * @return 界面对象的界面。
*/
QWidget* SS2PNCodeSetGUI::ShowMe(QWidget* parent, const QString& strTitle)
{
    m_strTitle = strTitle;

    SS2PNCodeSetGUIDlg* form;

    form = new SS2PNCodeSetGUIDlg(parent);
 ...
 * @return szHelpID:长度。
*/
int SS2PNCodeSetGUI::GetContextHelpID(char* szHelpID)
{
...
*/
int SS2PNCodeSetGUI::GetModuleInfo(TLxTsspModuleInfo& moduleInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS2PNCodeSetGUI::GetObjectInfo(TLxTsspObjectInfo& ObjectInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS2PNCodeSetGUI::PreInitialize()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS2PNCodeSetGUI::Initialize()
{
    //日志信息
    TLxTsspLog tmpLog;
    QString strLog;
    ...
    stCurPath = stCurPath + "/System/ModuleConfig/SS2PNCodeSetGUI.xml";
    //遍历xml文件，获取参数取值范围
    m_MapSumFlag = 0;
    ...
    if (pItem->vSubItem.size() > 0)
    {
        ...
        /*************************框架**********************************/
        m_ParaSS2PNCID = GetID(sItem.usStationID, sItem.ucSubSysID, sItem.usDeviceID,
                               sItem.usTID, sItem.ucSN);
        /*************************框架**********************************/

        /*************************状态**********************************/
        m_StatusSS2PNCID = GetID(sItem.usStationID, sItem.ucSubSysID, sItem.usDeviceID,
...
//遍历xml文件，获取参数取值范围
int SS2PNCodeSetGUI::QueryParaBlock(QString ConfigPath)
{
...
int SS2PNCodeSetGUI::GetAllMapValueStringByPara(int MapSum,QDomNode pInDocNode_Para)
{
 ...
 *           其它错误依情况增加；
 */
int SS2PNCodeSetGUI::GetErrorNumber()
{
    return m_iErrCode;
}
 ...
 */
int SS2PNCodeSetGUI::GetErrorMsg(int iErrorNo, QString& strErrorMsg)
{
 ...
 */
void SS2PNCodeSetGUI::SendMessage(TLxTsspMessage msg)
{
    return;
}
 ...
 * @return 1：成功；-1：失败。
 */
int SS2PNCodeSetGUI::SetParameter(QMap<quint64, QByteArray> parameterList)
{
 ...
 */
int SS2PNCodeSetGUI::SetParameter(quint64 ui64ParamID, QByteArray parameter)
{
 ...
 */
int SS2PNCodeSetGUI::GetParameterID(QList<quint64>& pParameterIDList) const
{
 ...
 * @return 大于等于0：参数块个数。-1：失败。
 */
int SS2PNCodeSetGUI::GetParameter(QMap<quint64, QByteArray>& parameterList) const
{
    ...
    if(tmpStateMgr != NULL)
        tmpCodeSynStatus  =
                (SMCU2::MCSSSB2PNCODEStatusStruct4*)tmpStateMgr->FindOneItem(m_StatusSS2PNCID, stLen);
    ...
    parameterList.insert(m_ParaSS2PNCID, tmpCh);
 ...
 * @return 1：成功；-1：失败。
 */
int SS2PNCodeSetGUI::GetParameter(quint64 ui64ParamID, QByteArray& parameter) const
{
    if(m_ParamList.contains(ui64ParamID))
    {
 ...
 */
int SS2PNCodeSetGUI::GetState(QMap<quint64, TLxTsspDeviceState>& stateList) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int SS2PNCodeSetGUI::GetState(quint64 ui64StatusID, TLxTsspDeviceState& status) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int SS2PNCodeSetGUI::GetStateDirect(quint64 ui64StatusID, TLxTsspDeviceState& status)
{
    if(m_StateList.contains(ui64StatusID))
    {
        //
 ...
 */
int SS2PNCodeSetGUI::WriteData(quint64 ui64DataID, uchar* aData, uint uiDataLen)
{
    return 0;
}
 ...
 */
int SS2PNCodeSetGUI::ReadData(quint64 ui64DataID, uchar* aData, uint& uiDataLen,
                              uint& uiReloadCount)
{
    return 0;
}

int SS2PNCodeSetGUI::CreateDefaultMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    /*码同步参数*/
    ...
    if(pMacroObj != NULL)
    {
        pMacroObj->UpdateParameterBlock(m_ParaSS2PNCID, byteArray);
    }
    return 1;
}

QWidget* SS2PNCodeSetGUI::ShowMeInEdit(QWidget* parent,uchar ucType,ILxTsspParameterMacro* pMacroObj)
{
    g_form = new SS2PNCodeSetGUIDlg(parent);
...
int SS2PNCodeSetGUI::SaveToMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    if(g_form != NULL)
    {
