...
extern "C" int GetModuleInfo(TLxTsspModuleInfo& moduleInfo)
{
    ...
    /// 配置标识，如：D_Y17-61_008
    strcpy(moduleInfo.szConfigID, "D_Y17-92_118\0");
...
extern "C" int GetConfigFormat(QString& strConfigFormat)
{
    ...
    strConfigFormat = QString("<Module Name=\"libSS1PNCodeSet.so\" StationID=\"2311\" SubSysID=\"04\" DevID=\"0401\" Note=\"%1\"> \n "
    ...
    for(int i=0; i<nodeList.length(); i++)
    {
        ...
        if(usObjID == 0x5151)
        {
            for(int k=iStart; k<iStop; k++)
            {
                ...
                strcpy(ObjectInfo.szConfigID, "D_Y17-92_118");
                strcpy(ObjectInfo.szAlias, objAlias.toUtf8().data());
                strcpy(ObjectInfo.szComment, objNote.toUtf8().data());
                g_ObjectList.insert(ui64ObjectID, new SS1PNCodeSetGUI(ObjectInfo));
            }
        }
    }
