...
void Matrix::SetFontSize(int nSize)
{
    m_nFontSize = nSize;
}
...
void Matrix::paintEvent(QPaintEvent *)
{
    ...
    //要绘制的单元个数
    int nNum = m_nNum;
    int pointSize = m_nFontSize;
    //绘制文字
    QFont font("Arial", pointSize);
    painter.setFont(font);

    //计算间距
    int nXS = MAX_NUM / (nNum * 2 * 1.0);
    int nSpance = (rect.width() - 10 - 10 - 10 - 50 * nXS) / nNum;
    int nLeftPos = 20 * (2 + nXS) / 2;  //左上角左边位置
    int nTopPos = nSpance + 40 * (1 + nXS / 2); //左上角上边距位置
    //绘制 类型：分控
