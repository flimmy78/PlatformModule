...
extern "C" int GetModuleInfo(TLxTsspModuleInfo& moduleInfo)
{
    ...
    /// 模块名称，如：系统工作参数设置
    strcpy(moduleInfo.szModuleName, "测控远控与系统监控接口\0");

    /// 配置标识，如：D_Y17-61_008
    strcpy(moduleInfo.szConfigID, "D_Y17-92_147\0");
...
extern "C" int GetConfigFormat(QString& strConfigFormat)
{
    ...
    strConfigFormat =
            "<Module Name=\"TTCDevCtlInterface.so\" StationID=\"2311\" SubSysID=\"01\" DevID=\"0400\" Note=\"测控分机设备接口\"> \n \
            <Object ID=\"5136\" Num=\"1\" IndexStart=\"0\" Alias =\" \" Note=\"测控分机设备接口\">...</Object> \n \
            ... \n \
            </Module>";
...
{
    // 没有固定格式，内容供配置时参考。
    strResourceReq = "接收分机状态并形成信息元，向各个分机发送控制命令。";
                             ...
                             QMap<quint64, void*>& ObjList)
{
    ...
    g_context.m_usDeviceID = usDeviceID;
    ...
    for(int i=0; i<nodeList.length(); i++)
    {
        ...
        if(usObjID == 0x6020)
        {
            for(int k=iStart; k<iStop; k++)
            {
                ...
                strcpy(ObjectInfo.szConfigID, "D_Y17-92_213");
                strcpy(ObjectInfo.szAlias, objAlias.toUtf8().data());
                strcpy(ObjectInfo.szComment, objNote.toUtf8().data());                
                g_devCtrl = new CDevCtlInterfaceGUI(ObjectInfo);
                g_ObjectList.insert(ui64ObjectID, g_devCtrl);
            }
        }
    }
...
extern "C" int DestroyObjects()
{
    ...
    {
        delete (CDevCtlInterfaceGUI*)g_ObjectList[key];
