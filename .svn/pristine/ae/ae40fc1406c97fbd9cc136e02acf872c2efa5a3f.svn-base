...
namespace
{
SSBBE_TCParamSetGUIDlg* g_form = NULL;
}
extern quint64 GetID(ushort usStationID, uchar ucSubSysID, ushort usDeviceID, ushort usType,
                     uchar ucSn);
SSBBE_TCParamSetGUI::SSBBE_TCParamSetGUI(TLxTsspObjectInfo ObjectInfo)
{
    //初始化
 ...
 * @return 界面对象的界面。
*/
QWidget* SSBBE_TCParamSetGUI::ShowMe(QWidget* parent, const QString& strTitle)
{
    m_strTitle = strTitle;

    SSBBE_TCParamSetGUIDlg* form;

    form = new SSBBE_TCParamSetGUIDlg(parent);
 ...
 * @return szHelpID:长度。
*/
int SSBBE_TCParamSetGUI::GetContextHelpID(char* szHelpID)
{
...
*/
int SSBBE_TCParamSetGUI::GetModuleInfo(TLxTsspModuleInfo& moduleInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SSBBE_TCParamSetGUI::GetObjectInfo(TLxTsspObjectInfo& ObjectInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SSBBE_TCParamSetGUI::PreInitialize()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SSBBE_TCParamSetGUI::Initialize()
{
    //日志信息
    TLxTsspLog tmpLog;
    QString strLog;
    ...
    stCurPath = stCurPath + "/System/ModuleConfig/SSBBE_TCParamSetGUI.xml";
    //遍历xml文件，获取参数取值范围
    m_MapSumFlag = 0;
...
//遍历xml文件，获取参数取值范围
int SSBBE_TCParamSetGUI::QueryParaBlock(QString ConfigPath)
{
...
int SSBBE_TCParamSetGUI::GetAllMapValueStringByPara(int MapSum,QDomNode pInDocNode_Para)
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
bool SSBBE_TCParamSetGUI::ClearUp()
{
    return 1;
}
 ...
 *           其它错误依情况增加；
 */
int SSBBE_TCParamSetGUI::GetErrorNumber()
{
    return m_iErrCode;
}
 ...
 */
int SSBBE_TCParamSetGUI::GetErrorMsg(int iErrorNo, QString& strErrorMsg)
{
 ...
 */
bool SSBBE_TCParamSetGUI::IsOnline()
{
    //对象初始化
    //查找对象管理器
    LxTsspObjectManager* pObjectManager = LxTsspObjectManager::Instance();
 ...
 */
void SSBBE_TCParamSetGUI::SendMessage(TLxTsspMessage msg)
{
    return;
}
 ...
 * @return 1：成功；-1：失败。
 */
int SSBBE_TCParamSetGUI::SetParameter(QMap<quint64, QByteArray> parameterList)
{
 ...
 */
int SSBBE_TCParamSetGUI::SetParameter(quint64 ui64ParamID, QByteArray parameter)
{
 ...
 */
int SSBBE_TCParamSetGUI::GetParameterID(QList<quint64>& pParameterIDList) const
{
 ...
 * @return 大于等于0：参数块个数。-1：失败。
 */
int SSBBE_TCParamSetGUI::GetParameter(QMap<quint64, QByteArray>& parameterList) const
{
    SMCU::MCSSSTCParamStruct4 tmpMCSSSTCParam;       //参数

    SMCU::MCSSSTCStatusStruct4* tmpMCSSSTCStatus = NULL;      //状态
    ...
    if(tmpStateMgr != NULL)
        tmpMCSSSTCStatus  =
                (SMCU::MCSSSTCStatusStruct4*)tmpStateMgr->FindOneItem(m_StatusTCPID, stLen);

    if(tmpMCSSSTCStatus != NULL)
    {
        for (int i = 0; i < MAX_TARGET_COUNT; i++)
            tmpMCSSSTCParam.SSTCParamStruct[i] = tmpMCSSSTCStatus->SSTCStatusStruct[i].tParams;
    }

    QByteArray  tmpCh;
    tmpCh.resize(sizeof(SMCU::MCSSSTCParamStruct4));
    memcpy(tmpCh.data(), &tmpMCSSSTCParam, sizeof(SMCU::MCSSSTCParamStruct4));
 ...
 * @return 1：成功；-1：失败。
 */
int SSBBE_TCParamSetGUI::GetParameter(quint64 ui64ParamID, QByteArray& parameter) const
{
    if(m_ParamList.contains(ui64ParamID))
    {
 ...
 */
int SSBBE_TCParamSetGUI::GetState(QMap<quint64, TLxTsspDeviceState>& stateList) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int SSBBE_TCParamSetGUI::GetState(quint64 ui64StatusID, TLxTsspDeviceState& status) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int SSBBE_TCParamSetGUI::GetStateDirect(quint64 ui64StatusID, TLxTsspDeviceState& status)
{
    if(m_StateList.contains(ui64StatusID))
    {
        //
 ...
 */
int SSBBE_TCParamSetGUI::WriteData(quint64 ui64DataID, uchar* aData, uint uiDataLen)
{
    return 0;
}
 ...
 */
int SSBBE_TCParamSetGUI::ReadData(quint64 ui64DataID, uchar* aData, uint& uiDataLen,
                                  uint& uiReloadCount)
{
    return 0;
}

int SSBBE_TCParamSetGUI::CreateDefaultMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    SMCU::MCSSSTCParamStruct4 aMCSSSTCParam;
    for (int i = 0; i < MAX_TARGET_COUNT; i ++)
    {
        //码型
        aMCSSSTCParam.SSTCParamStruct[i].ucCodeStyle = m_ParaSetStruct[i][0].InitVal.lval + 1;
        //码速率
        aMCSSSTCParam.SSTCParamStruct[i].uiCodeRate = m_ParaSetStruct[i][1].InitVal.lval;
        //遥控发送方式
        aMCSSSTCParam.SSTCParamStruct[i].ucSWaveMd = m_ParaSetStruct[i][2].InitVal.lval + 1;
        //工作期启动方式
        aMCSSSTCParam.SSTCParamStruct[i].ucTCWorkCyc = m_ParaSetStruct[i][3].InitVal.lval + 1;
        //前补序列
        aMCSSSTCParam.SSTCParamStruct[i].ucFCodeMinU = m_ParaSetStruct[i][4].InitVal.lval;
        //后补序列
        aMCSSSTCParam.SSTCParamStruct[i].ucBCodeMinU = m_ParaSetStruct[i][5].InitVal.lval;
        //执行序列
        aMCSSSTCParam.SSTCParamStruct[i].ucECodeMinU = m_ParaSetStruct[i][6].InitVal.lval;
        //空闲序列
        aMCSSSTCParam.SSTCParamStruct[i].ucIndleCode = m_ParaSetStruct[i][7].InitVal.lval;
        //前补序列长度
        aMCSSSTCParam.SSTCParamStruct[i].usFQueueLen = m_ParaSetStruct[i][8].InitVal.lval;
        //后补序列长度
        aMCSSSTCParam.SSTCParamStruct[i].usBQueueLen = m_ParaSetStruct[i][9].InitVal.lval;
        //执行序列长度
        aMCSSSTCParam.SSTCParamStruct[i].ucECodeLen = m_ParaSetStruct[i][10].InitVal.lval;
        //保护间隙
        aMCSSSTCParam.SSTCParamStruct[i].usTCProctC = m_ParaSetStruct[i][11].InitVal.lval;
        //应急填充长度
        aMCSSSTCParam.SSTCParamStruct[i].ucC8Data = m_ParaSetStruct[i][12].InitVal.lval;
        //编码控制
        aMCSSSTCParam.SSTCParamStruct[i].ucBCHCode = m_ParaSetStruct[i][13].InitVal.lval + 1;
        //起始序列控制
        aMCSSSTCParam.SSTCParamStruct[i].ucBCHBeginSeriesCtrl = m_ParaSetStruct[i][14].InitVal.lval + 1;
        //起始序列图样
        aMCSSSTCParam.SSTCParamStruct[i].usBCHBeginSeries = m_ParaSetStruct[i][15].InitVal.lval;
        //加扰控制
        aMCSSSTCParam.SSTCParamStruct[i].ucDisturboCtrl = m_ParaSetStruct[i][16].InitVal.lval + 1;
        //结尾序列控制
        aMCSSSTCParam.SSTCParamStruct[i].ucBCHEndCtrl = m_ParaSetStruct[i][17].InitVal.lval + 1;
        //结尾序列图样
        aMCSSSTCParam.SSTCParamStruct[i].dwlBCHEndSeries = m_ParaSetStruct[i][18].InitVal.lval;
        //加扰多项式长度
        aMCSSSTCParam.SSTCParamStruct[i].ucDisturboLen = m_ParaSetStruct[i][19].InitVal.lval + 1;
        //加扰多项式
        aMCSSSTCParam.SSTCParamStruct[i].uiDisPoly = m_ParaSetStruct[i][20].InitVal.lval;
        //加扰初相
        aMCSSSTCParam.SSTCParamStruct[i].uiDisPhase = m_ParaSetStruct[i][21].InitVal.lval;
    }
    //取当前设置的参数，存入框架
...
QWidget* SSBBE_TCParamSetGUI::ShowMeInEdit(QWidget* parent,uchar ucType,ILxTsspParameterMacro* pMacroObj)
{
    g_form = new SSBBE_TCParamSetGUIDlg(parent);
...
int SSBBE_TCParamSetGUI::SaveToMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    if(g_form!=NULL)
    {
