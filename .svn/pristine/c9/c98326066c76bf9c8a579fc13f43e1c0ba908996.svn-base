#ifndef CDevCtlInterfaceGUI_H
#define CDevCtlInterfaceGUI_H
...
//参数描述信息
class TTCDEVCTLINTERFACESHARED_EXPORT CDevCtlInterfaceGUI;
typedef void (CDevCtlInterfaceGUI::*PFProcessMsg)(DATACONTENT&);

class TTCDEVCTLINTERFACESHARED_EXPORT CDevCtlInterfaceGUI : public ILxTsspShow
{
public:
    CDevCtlInterfaceGUI(TLxTsspObjectInfo ObjectInfo);
    int QueryParaBlock(QString ConfigPath);
    int GetAllMapValueStringByPara(int MapSum,QDomNode pInDocNode_Para);
    int proMsg(TLxTsspMessage& msg);
public:
    /**
...
protected:
    ...
    TLxTsspObjectInfo m_swMatInfo;
    QMap<QString, TLxTsspObjectInfo> m_moduleInfos;
    //日志管理
    ...
    ParaStruct m_ParaSetStruct[3][60];  //保存参数描述信息
    ...
    struct{//ss1 过程控制命令 上行载波输出 界面测控开始结束
        UCHAR ucCarrOut;
        char  cOutElectricalLevel;
        UCHAR chlSel;
    }m_upCarrOut;
    struct{
        UCHAR chlSel;
...
public:
    PlatformResource m_platformRes;
    RecvDataThread m_recvThread;
    SearchStatusThread m_searchStatuThread;
    DevCtl::ConnectStatusCollect m_connectStatu;
    void ProcessAlter(TLxTsspParamMacroInformation macroInfo,
                                             ILxTsspParameterMacro* macroObj,DATACONTENT& msg,
                                             QString baseBandName, int baseBandIndex);
    bool filter(TLxTsspMessageHeader*, TLxTsspSubMsgHeader*);
    int getSN(int);
    ...
    void ProcessGroupCMDBaseBandCommon(TLxTsspParamMacroInformation macroInfo,
                                       ILxTsspParameterMacro* macroObj,DATACONTENT& msg,
                                       QString baseBandName, int baseBandIndex);
    void ProcessAlterInternal(ILxTsspParameterMacro* macroObj, QString mdlName, UCHAR index, char conn);
    ...
    void ProcessGroupDataTrans(ILxTsspParameterMacro* macroObj, DATACONTENT msg, QString, int);
    void ProcessGroupTTC(ILxTsspParameterMacro* macroObj, DATACONTENT msg, QString, int);
    void ProcessGroupSMCU1(ILxTsspParameterMacro* macroObj, DATACONTENT msg, QString, int);
    void ProcessGroupTC(ILxTsspParameterMacro* macroObj, DATACONTENT msg, QString, int);

    void ProcessCtrlCmd(DATACONTENT& msg);  //wl 设备参数控制函数
    ...
    //wl end
    void ProcessControlCMD1(DATACONTENT &msg);
    bool getChl(DATACONTENT &msg, int bbe1_bbe2, int ddt_tc_usb_ss1_ss2);
    void ProcessGroupSMCU2(ILxTsspParameterMacro *macroObj, DATACONTENT msg, QString log, int dev);
    void ProcessRemoteServerMacro(DATACONTENT &msg);
    void ProcessGroupSafeCtrlTC(TLxTsspParamMacroInformation, ILxTsspParameterMacro *macroObj, DATACONTENT &msg, char ak, CTC &tc, QString name, int dev);
    void ProcessControlCMD2(DATACONTENT &msg);
    bool getChl(SUBITEM &subItem);
    void ProcessRecordMonitorSearchStatus(DATACONTENT &msg);
    void ProcessRecordMonitorSaveCMD(const char *pData, unsigned int &nLen,
                                     const BYTE &byteCmd, char *pChannel, bool bRM1); //处理检前存盘指令函数包括存盘开始和停止
    void ProcessRecordMonitor(DATACONTENT &msg);
    short Local2Port(short sValue);
    bool getChlUsedByMainMachine(SUBITEM &subItem);
    void ProcessControlCMD3(DATACONTENT &msg);
    void ProcessSaveControlCMD(DATACONTENT &msg);
    void getMainMachineValue(char &value);
    void ProcessSendDataControlCMD(DATACONTENT &msg);
    void setchl(SUBITEM& subItem, QString chl);
    void ProcessModeChangeControlCMD(DATACONTENT &msg);
    bool getChlUsedByMainMachineForModeChange(SUBITEM &subItem);
    void ProcessWorkWayChangeControlCMD(DATACONTENT &msg);
    void ProcessControlCMD4(DATACONTENT &msg);
    void Process_SS1_SS2_SaveControlCMD(DATACONTENT &msg);
    void ProcessDopplerInMacro(DATACONTENT &msg, UCHAR unitNo, USHORT seq);
};
