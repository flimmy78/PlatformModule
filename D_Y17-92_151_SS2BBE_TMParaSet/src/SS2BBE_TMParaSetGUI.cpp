...
namespace
{
SS2BBE_TMParaSetGUIDlg* g_form = NULL;
}
extern quint64 GetID(ushort usStationID, uchar ucSubSysID, ushort usDeviceID, ushort usType,
                     uchar ucSn);
SS2BBE_TMParaSetGUI::SS2BBE_TMParaSetGUI(TLxTsspObjectInfo ObjectInfo)
{
    //初始化
 ...
 * @return 界面对象的界面。
*/
QWidget* SS2BBE_TMParaSetGUI::ShowMe(QWidget* parent, const QString& strTitle)
{
    m_strTitle = strTitle;

    SS2BBE_TMParaSetGUIDlg * form;

    form = new SS2BBE_TMParaSetGUIDlg(parent);
 ...
 * @return szHelpID:长度。
*/
int SS2BBE_TMParaSetGUI::GetContextHelpID(char* szHelpID)
{
...
*/
int SS2BBE_TMParaSetGUI::GetModuleInfo(TLxTsspModuleInfo& moduleInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS2BBE_TMParaSetGUI::GetObjectInfo(TLxTsspObjectInfo& ObjectInfo) const
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS2BBE_TMParaSetGUI::PreInitialize()
{
    return 1;
}
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
int SS2BBE_TMParaSetGUI::Initialize()
{
    //日志信息
    TLxTsspLog tmpLog;
    QString strLog;
    ...
    stCurPath = stCurPath + "/System/ModuleConfig/SS2BBE_TMParaSetGUI.xml";
    //遍历xml文件，获取参数取值范围
    m_MapSumFlag = 0;
...
//遍历xml文件，获取参数取值范围
int SS2BBE_TMParaSetGUI::QueryParaBlock(QString ConfigPath)
{
...
int SS2BBE_TMParaSetGUI::GetAllMapValueStringByPara(int MapSum,QDomNode pInDocNode_Para)
{
 ...
 * @return 1：成功；0：未定义；-1：失败。
 */
bool SS2BBE_TMParaSetGUI::ClearUp()
{
    return 1;
}
 ...
 *           其它错误依情况增加；
 */
int SS2BBE_TMParaSetGUI::GetErrorNumber()
{
    return m_iErrCode;
}
 ...
 */
int SS2BBE_TMParaSetGUI::GetErrorMsg(int iErrorNo, QString& strErrorMsg)
{
 ...
 */
bool SS2BBE_TMParaSetGUI::IsOnline()
{
    //对象初始化
    //查找对象管理器
    LxTsspObjectManager* pObjectManager = LxTsspObjectManager::Instance();
 ...
 */
void SS2BBE_TMParaSetGUI::SendMessage(TLxTsspMessage msg)
{
    return;
}
 ...
 * @return 1：成功；-1：失败。
 */
int SS2BBE_TMParaSetGUI::SetParameter(QMap<quint64, QByteArray> parameterList)
{
 ...
 */
int SS2BBE_TMParaSetGUI::SetParameter(quint64 ui64ParamID, QByteArray parameter)
{
 ...
 */
int SS2BBE_TMParaSetGUI::GetParameterID(QList<quint64>& pParameterIDList) const
{
 ...
 * @return 大于等于0：参数块个数。-1：失败。
 */
int SS2BBE_TMParaSetGUI::GetParameter(QMap<quint64, QByteArray>& parameterList) const
{
    SMCU2::MCSSSB2TMParamStruct4 tmpMCSSSTMParam;       //码同步参数

    SMCU2::MCSSSB2TMStatusStruct4* tmpMCSSSTMStatus = NULL;      //码同步参数
    ...
    if(tmpStateMgr != NULL)
        tmpMCSSSTMStatus  =
                (SMCU2::MCSSSB2TMStatusStruct4*)tmpStateMgr->FindOneItem(m_StatusTMPID, stLen);

    if(tmpMCSSSTMStatus != NULL)
    {
        for (int i = 0; i < MAX_TARGET_COUNT; i++)
            tmpMCSSSTMParam.SSTMParam[i] = tmpMCSSSTMStatus->MCSSSB2TMStatu[i].tParams;
    }

    QByteArray  tmpCh;
    tmpCh.resize(sizeof(SMCU2::MCSSSB2TMParamStruct4));
    memcpy(tmpCh.data(), &tmpMCSSSTMParam, sizeof(SMCU2::MCSSSB2TMParamStruct4));
 ...
 * @return 1：成功；-1：失败。
 */
int SS2BBE_TMParaSetGUI::GetParameter(quint64 ui64ParamID, QByteArray& parameter) const
{
    if(m_ParamList.contains(ui64ParamID))
    {
 ...
 */
int SS2BBE_TMParaSetGUI::GetState(QMap<quint64, TLxTsspDeviceState>& stateList) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int SS2BBE_TMParaSetGUI::GetState(quint64 ui64StatusID, TLxTsspDeviceState& status) const
{
 ...
 * @return 1：成功；-1：失败。
 */
int SS2BBE_TMParaSetGUI::GetStateDirect(quint64 ui64StatusID, TLxTsspDeviceState& status)
{
    if(m_StateList.contains(ui64StatusID))
    {
        //
 ...
 */
int SS2BBE_TMParaSetGUI::WriteData(quint64 ui64DataID, uchar* aData, uint uiDataLen)
{
    return 0;
}
 ...
 */
int SS2BBE_TMParaSetGUI::ReadData(quint64 ui64DataID, uchar* aData, uint& uiDataLen,
                                 uint& uiReloadCount)
{
    return 0;
}

int SS2BBE_TMParaSetGUI::CreateDefaultMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    /*码同步参数*/
    SMCU2::MCSSSB2TMParamStruct4 aMCSSSTMParam;
    for (int nIndex = 0; nIndex < MAX_TARGET_COUNT; nIndex++)
    {
        //数据帧标志码
        aMCSSSTMParam.SSTMParam[nIndex].ucDataCode = m_ParaSetStruct[nIndex][0].InitVal.lval;
        //帧同步码组位置
...
QWidget* SS2BBE_TMParaSetGUI::ShowMeInEdit(QWidget* parent,uchar ucType,ILxTsspParameterMacro* pMacroObj)
{
    g_form = new SS2BBE_TMParaSetGUIDlg(parent);
...
int SS2BBE_TMParaSetGUI::SaveToMacroObj(ILxTsspParameterMacro* pMacroObj)
{
    if(g_form!=NULL)
    {
