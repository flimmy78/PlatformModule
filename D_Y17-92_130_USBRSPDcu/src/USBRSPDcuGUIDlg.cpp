#include "USBRSPDcuGUIDlg.h"
#include "ui_USBRSPDcuGUIDlg.h"
#include <QCloseEvent>
#include <QMessageBox>
#include <QDateTime>
...
USBRSPDcuGUIDlg::USBRSPDcuGUIDlg(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::USBRSPDcuGUIDlg)
{
    ui->setupUi(this);
    m_pStateMgr = NULL;
...
USBRSPDcuGUIDlg::~USBRSPDcuGUIDlg()
{
    killTimer(m_TimerID);
    delete ui;
}

void USBRSPDcuGUIDlg::setText(const QString& strTitle, ParaStruct xmlsetText[3][60], QString MapMean[50][30],
...
//初始化对话框
void USBRSPDcuGUIDlg::InitDLG(int Type, ILxTsspParameterMacro* pMacroObj)
{
    //设置界面的灯的默认颜色
...
void USBRSPDcuGUIDlg::initTableView()
{
...
//设置输入格式
void USBRSPDcuGUIDlg::InitInput()
{
...
void USBRSPDcuGUIDlg::ShowControls()
{
    //向下拉框控件中写入从xml中读取的内容
    //切换控制命令
...
//显示参数
void USBRSPDcuGUIDlg::ShowPara(int type)
{
    if(type == 0)       //参数设置
    {
    ...
    /********************设备是否在线************************/
    m_bOnLine = CCommFunc::getInstance()->getUSBRSPStatus();
    if (!m_bOnLine)
    {
    ...
    //目标仿真状态
    if (m_pComStatus->ucSimuOn[0] == 0xFF)
    {
        ui->lineEdit_MBFZStatus->setText(m_DlgMapMean[7][4]);
    }
    else
    {
        ui->lineEdit_MBFZStatus->setText(m_DlgMapMean[7][m_pComStatus->ucSimuOn[0]]);
    }
    //已仿真数据帧数
    ui->lineEdit_FZDataFrame->setText(QString::number(m_pComStatus->dwlFrameNum[0]));
}

void USBRSPDcuGUIDlg::SetObjInfo(TLxTsspObjectInfo ObjectInfo)
{
    ...
    {
        //第2个作为接收宏查询返回的共享内存使用
        SUBITEM sItem = pItemSearch->vSubItem.at(0);
    ...
    {
        //第2个作为接收宏查询返回的共享内存使用
        SUBITEM sItem = pItemCall->vSubItem.at(0);
...
//使能或不使能控件
void USBRSPDcuGUIDlg::EnabledControls(bool bFlag)
{
    //宏查询按钮
    ...
    //将状态界面显示为不可用
    //载波输出状态
    /*ui->comboBox_WaveInputStatus->setEnabled(false);
    //载波加调状态
    ui->comboBox_WaveLoadStatus->setEnabled(false);
    //目标仿真类型
    ui->comboBox_MBFZType->setEnabled(false);
    //目标仿真状态
    ui->comboBox_MBFZStatus->setEnabled(false);
    //已仿真数据帧数
    ui->lineEdit_FZDataFrame->setEnabled(false);*/
...
int USBRSPDcuGUIDlg::SaveToMacro(ILxTsspParameterMacro* pMacroObj)
{
    //切换控制命令
...
//定时器
void USBRSPDcuGUIDlg::timerEvent(QTimerEvent *event)
{
...
void USBRSPDcuGUIDlg::on_pushButton_Modify_clicked()
{
    if (!m_bOnLine)
    {
    ...
    if(ui->pushButton_Modify->text() == "更改")
    {
        ...
        //killTimer(m_TimerID); //状态一致刷新将其注释
...
void USBRSPDcuGUIDlg::on_pushButton_Set_clicked()
{
    //设置参数
    //取当前设置的参数
    if(m_pHAManager != NULL)
    ...
    //不使能控件
    EnabledControls(false);
    //m_TimerID = startTimer(1000); //状态一致刷新将其注释
}

void USBRSPDcuGUIDlg::closeEvent(QCloseEvent *event)
{
    event->accept();
}

QSize USBRSPDcuGUIDlg::sizeHint() const
{
    return QSize(this->width(), this->height());
}

void USBRSPDcuGUIDlg::SendMessage(char* pData, unsigned short  usLen, unsigned short usC, unsigned short usTID)
{
    static uint seq = 0;
...
bool USBRSPDcuGUIDlg::initCmdInfo(QString strModuleName)
{
    bool bRet = m_DeviceMap.loadXML();
    if (!bRet)
    {
...
DWORDLONG USBRSPDcuGUIDlg::StrToSynCode(const char * pStr)
{
    char c, C;
    const char*  p;
...
void USBRSPDcuGUIDlg::sendCMD(QByteArray ba, QString strInfo)
{
...
void USBRSPDcuGUIDlg::setLight(QLabel *pLabel, int nState)
{
    QPixmap light;
    bool bRet = false;
    switch(nState)
...
void USBRSPDcuGUIDlg::addData2TabView()
{
    ...
    {
        //参数宏号
        m_pModel->setItem(i, 0, new QStandardItem(QString::number(m_pComPara->paras[i].usMarcoNo)));
        //设置字符颜色
        //m_pModel->item(i, 0)->setForeground(QBrush(QColor(255, 0, 0)));
        //设置字符位置
        //m_pModel->item(i, 0)->setTextAlignment(Qt::AlignCenter);
...
void USBRSPDcuGUIDlg::setOfflineStatusValue()
{
...
//宏查询按钮
void USBRSPDcuGUIDlg::on_pushButton_MacroSearch_clicked()
{
    WORD wMarcoNo = 0;      //宏代号
    WORD wReserved = 0;      //保留
    //设置参数
    ...
    m_MarcoSearchTimerID = startTimer(500);
}

//宏调用按钮
void USBRSPDcuGUIDlg::on_pushButton_MacroCall_clicked()
{
...
//模式切换命令按钮
void USBRSPDcuGUIDlg::on_pushButton_ModuleCMD_clicked()
{
...
//仿真控制命令按钮
void USBRSPDcuGUIDlg::on_pushButton_FZCtrlCMD_clicked()
{
...
//存盘控制命令按钮
void USBRSPDcuGUIDlg::on_pushButton_SaveCtrlCMD_clicked()
{
...
//切换控制命令设置按钮
void USBRSPDcuGUIDlg::on_pushButton_CtrlCMD_clicked()
{
    //获取当前
