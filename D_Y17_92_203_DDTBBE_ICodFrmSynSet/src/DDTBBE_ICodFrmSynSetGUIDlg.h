#ifndef DDTBBE_CODFRMSYNSETGUIDLG_H
#define DDTBBE_CODFRMSYNSETGUIDLG_H
...
namespace Ui {
class DDTBBE_CodFrmSynSetGUIDlg;
}

class DDTBBE_CodFrmSynSetGUIDlg : public QDialog
{
    Q_OBJECT

public:
    explicit DDTBBE_CodFrmSynSetGUIDlg(QWidget *parent = 0);
    QSize sizeHint() const;
    ~DDTBBE_CodFrmSynSetGUIDlg();
...
private:
    Ui::DDTBBE_CodFrmSynSetGUIDlg *ui;
    ...
    quint64 m_ParaRecSetParamID;
    quint64 m_StatusParaRecSetParamID;

    int m_TimerID;
    int iParaSetFlag;       //参数设置标志，0表示参数设置,1表示宏编辑
    unsigned short m_usC;
    unsigned short m_usTID;
    unsigned short m_usC1;
    unsigned short m_usTID1;
    unsigned char m_ucSN;
    QTimer* m_timer1;
    ILxTsspParameterMacro* m_pMacroObj;
    int m_count;
public:
    //设置标题 传倒信息
    ...
    bool m_killTimer;
    BOOL  CheckDecodeType();
public slots:
    void updateCodeType();
private slots:
    void on_pushButton_Modify_clicked();
    void on_pushButton_Set_clicked();
    void on_comboBox_OutputType_currentIndexChanged(int index);
...
#endif // DDTBBE_CODFRMSYNSETGUIDLG_H
