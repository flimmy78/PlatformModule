...
#include "ui_DDTBBE_ICodFrmSynSetGUIDlg.h"
#include <QCloseEvent>
#include <QMessageBox>
#include <QDateTime>
#include <QDebug>
    ...
    ui(new Ui::DDTBBE_CodFrmSynSetGUIDlg)
{
    ...
    //    m_count = 0;
    //    connect(m_timer1, SIGNAL(timeout()), this, SLOT(updateCodeType()));
    m_pMacroObj = NULL;
    setAttribute(Qt::WA_DeleteOnClose);
}

//void DDTBBE_CodFrmSynSetGUIDlg::updateCodeType()
//{
//    ++m_count;
//    if(m_count == 5)
//    {
...
//        m_pMacroObj = NULL;
//    }
//    if(!m_pMacroObj)return;
//    QByteArray byteArray;
//    m_pMacroObj->GetParameterBlock(m_ParaRecSetParamID,byteArray);
//    memcpy(&m_MCSDDTRevParam, byteArray.data(), sizeof(m_MCSDDTRevParam));

//    const int size  = m_MCSDDTRevParam.ucGrayType  == 9 ? 3 : 1;
//    if(ui->comboBox_CodeType->count() == size)return;
...
void DDTBBE_CodFrmSynSetGUIDlg::InitInput()
{
    ...
    //码速率
    QRegExp regExpBitRate("([3-9][2-9]|1?[1-9][0-9]{2,3}|20000)");//32~20000 小数点后保留3位
...
void DDTBBE_CodFrmSynSetGUIDlg::ShowControls()
{
    ...
    ui->comboBox_idDirect->setCurrentIndex(m_DlgParaSetStruct[1][14].InitVal.lval);
    //帧同步容错搜索容错位数
    ...
    //"~" + QString::number(m_DlgParaSetStruct[1][0].MaxVal.lmax) + "]");
    //帧同步码组
    ...
    ui->label_SubFrmLen->setText("副帧长\n["+QString::number(m_DlgParaSetStruct[1][8].MinVal.lmin) +
            "~" + QString::number(m_DlgParaSetStruct[1][8].MaxVal.lmax) + "]");
    //副帧码组位置
    ui->lineEdit_SubFrmSynGroupPos->setText(QString::number(m_DlgParaSetStruct[1][11].InitVal.lval));
    ui->label_SubFrmSynGroupPos->setText("副帧码组位置\n["+QString::number(m_DlgParaSetStruct[1][11].MinVal.lmin) +
            "~" + QString::number(m_DlgParaSetStruct[1][11].MaxVal.lmax) + "]");
    //副帧同步码组
    ...
    ui->label_idCountPos->setText("ID计数位置\n["+QString::number(m_DlgParaSetStruct[1][15].MinVal.lmin) +
            "~" + QString::number(m_DlgParaSetStruct[1][15].MaxVal.lmax) + "]");
}
...
void DDTBBE_CodFrmSynSetGUIDlg::ShowPara(int type)
{
    ...
    if(ui->comboBox_DecodeType->currentIndex() % 2 == 1){
        ...
        //卷积码类型
        if (m_MCSDDTTMIParam.ucViterbiMd > 0)
            ui->comboBox_RecurrentCodeType->setCurrentIndex(m_MCSDDTTMIParam.ucViterbiMd);
        //维特比输出相位
        if (m_MCSDDTTMIParam.ucG2Inv > 0)
            ui->comboBox_ViterbiPhase->setCurrentIndex(m_MCSDDTTMIParam.ucG2Inv);
    }*/
...
BOOL DDTBBE_CodFrmSynSetGUIDlg::CheckDecodeType()
{
    ...
    {
        strTmp = QString("此时帧长不能为空！");
        msgBox.setText(strTmp);
        msgBox.exec();
        return FALSE;
    }
    int m_usFrameLen = ui->lineEdit_FrameLen->text().toInt();
    int nFSPLen = (ui->comboBox_FrameSyncGroupLen->currentIndex() + 2) * 8;
    ...
    {
        strTmp = QString("帧长范围为[%1~%2]字，重新输入!").arg(FrmSizeMin).arg(FrmSizeMax);
        msgBox.setText(strTmp);
        msgBox.exec();
        return FALSE;
    }
