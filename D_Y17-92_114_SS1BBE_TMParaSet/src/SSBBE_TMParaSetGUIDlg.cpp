...
extern quint64 GetID(ushort usStationID, uchar ucSubSysID, ushort usDeviceID, ushort usType,
                     uchar ucSn);
SSBBE_TMParaSetGUIDlg::SSBBE_TMParaSetGUIDlg(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SSBBE_TMParaSetGUIDlg)
{
    ui->setupUi(this);
    m_pStateMgr = NULL;
...
SSBBE_TMParaSetGUIDlg::~SSBBE_TMParaSetGUIDlg()
{
    killTimer(m_TimerID);
    m_timer1->stop();
    delete m_timer1;
...
int SSBBE_TMParaSetGUIDlg::msgBoxFunction(QMessageBox &msgBox, QString msg, BOOL ret)
{
    msgBox.setText(msg);
    msgBox.exec();
    return ret;
}
void SSBBE_TMParaSetGUIDlg::setText(const QString& strTitle, ParaStruct xmlsetText[4][60], QString MapMean[100][30], int MapNum[100], uchar Type, ILxTsspParameterMacro* pMacroObj)
{
    m_strTitle = strTitle;
...
//初始化对话框
void SSBBE_TMParaSetGUIDlg::InitDLG(int Type, ILxTsspParameterMacro* pMacroObj)
{
    iParaSetFlag = Type;
    //设置输入格式
    InitInput();
    ...
    if(Type == 0)//取当前参数
    {
        ...
        if(m_pStateMgr != NULL)
        {
            ...
            m_MCSSSTMStatus = (SMCU::MCSSSTMStatusStruct4*)m_pStateMgr->FindOneItem(m_StatusParaMCSSSTMID, stLen);
            if(m_MCSSSTMStatus == NULL || stLen != sizeof(SMCU::MCSSSTMStatusStruct4))
                return;
        }
        //启动定时器
    ...
    else if(Type == 1)//取宏里参数
    {
        ...
        if(pMacroObj != NULL)
        {
            ...
            memcpy(&m_MCSSSTMParam, byteArray.data(), sizeof(SMCU::MCSSSTMParamStruct4));
        }
...
//设置输入格式
void SSBBE_TMParaSetGUIDlg::InitInput()
{
    //正整数
    ...
    QRegExp regExp1("[0-9]{1,4}");
    QRegExp regExp2("[0-9]{1,2}");
    //卫星载波锁定波道
    ui->lineEdit_SatelliteLock->setValidator(new QRegExpValidator(regExp2, this));
    //卫星载波锁定位
    ui->lineEdit_SatelliteLockWave->setValidator(new QRegExpValidator(regExp1, this));
    //卫星长码锁定波道
    ui->lineEdit_SatelliteLongCode->setValidator(new QRegExpValidator(regExp2, this));
    //卫星载波锁定位
    ui->lineEdit_SatelliteLongCodeWave->setValidator(new QRegExpValidator(regExp1, this));
    //卫星短码锁定波道
    ui->lineEdit_SatelliteShortLock->setValidator(new QRegExpValidator(regExp2, this));
    //卫星短码锁定位
    ui->lineEdit_SatelliteShortLockWave->setValidator(new QRegExpValidator(regExp1, this));

    //字母数字
    QRegExp regExp3("[A-Fa-f0-9]{1,8}");
    //帧同步码组[HEX]
    ui->lineEdit_FrmSyncGroup->setValidator(new QRegExpValidator(regExp3, this));
    //解扰多项式[HEX]
    ui->lineEdit_RandPoly->setValidator(new QRegExpValidator(regExp3, this));
    //解扰初相[HEX]
    ui->lineEdit_RandPhase->setValidator(new QRegExpValidator(regExp3, this));
    //副帧同步码组
    ui->lineEdit_SubFrmGroup->setValidator(new QRegExpValidator(regExp3, this));
}

void SSBBE_TMParaSetGUIDlg::ShowControls()
{
    //向下拉框控件中写入从xml中读取的内容
    showTargetParam(0);
    //默认选择目标1
...
//显示参数
void SSBBE_TMParaSetGUIDlg::ShowPara(int type)
{
    if(type == 0)       //参数设置
    {
        if(m_MCSSSTMStatus != NULL)
        {
            for (int i = 0; i < MAX_TARGET_COUNT; i++)
                m_MCSSSTMParam.SSTMParam[i] = m_MCSSSTMStatus->SSTMStatu[i].tParams;
        }
        else
        {
            DWORD stLen = sizeof(SMCU::MCSSSTMStatusStruct4);
...
//设置帧长范围
void SSBBE_TMParaSetGUIDlg::SetFrameLenRange(int iWordLen)
{
    /* if(iWordLen ==8)
    {
...
void SSBBE_TMParaSetGUIDlg::SetObjInfo(TLxTsspObjectInfo ObjectInfo)
{
...
//使能或不使能控件
void SSBBE_TMParaSetGUIDlg::EnabledControls(bool bFlag)
{
    /*************************Group1*************************/
    //帧同步码组位置
...
//保存参数宏
int SSBBE_TMParaSetGUIDlg::SaveToMacro(ILxTsspParameterMacro* pMacroObj)
{
    //判断合法性
    int iRet = CheckPara();
    if(iRet == -1)
...
//从界面取参数
void SSBBE_TMParaSetGUIDlg::GetPara()
{
    getParamTarget();
}

//验证参数范围
int SSBBE_TMParaSetGUIDlg::CheckPara()
{
    return CheckParam() == TRUE ? 1 : -1;
}

////定时器
void SSBBE_TMParaSetGUIDlg::timerEvent(QTimerEvent *event)
{
    ShowPara(0);
}
...
void SSBBE_TMParaSetGUIDlg::on_pushButton_Set_clicked()
{
    //验证参数范围
    if(CheckPara() == 1)
    {
        ...
        if(m_pHAManager != NULL)
        {
            struct DATA{
                USHORT target;
                struct{USHORT tmp;
                       SMCU::MCSSSTMParamStruct SSTMParam;
...
void SSBBE_TMParaSetGUIDlg::closeEvent(QCloseEvent *event)
{
    event->accept();
}

QSize SSBBE_TMParaSetGUIDlg::sizeHint() const
{
...
/*
//副帧方式响应函数
void SSBBE_TMParaSetGUIDlg::on_comboBox_SubFrmType_currentIndexChanged(int index)
{
//    //副帧方式控件使能
...
//根据副帧类型，使能副帧控件
void SSBBE_TMParaSetGUIDlg::EnabledControlsSubFrmType(int iSubFrm)
{
//    switch(iSubFrm)
//    {
//        case 0:     //无副帧
...
void SSBBE_TMParaSetGUIDlg::SubFrmEn(bool bFlag)
{
    //副帧长度
...
//有无ID副帧
void SSBBE_TMParaSetGUIDlg::IDSubFrmEn(bool bFlag)
{
...
//有无循环副帧
void SSBBE_TMParaSetGUIDlg::CycSubFrmEn(bool bFlag)
{
    //循环副帧
...
//有无反码副帧
void SSBBE_TMParaSetGUIDlg::ReserveSubFrmEn(bool bFlag)
{

}

void SSBBE_TMParaSetGUIDlg::SendMessage(char* pData, unsigned short  usLen, unsigned short usC, unsigned short usTID)
{
    static uint seq = 0;
...
DWORDLONG SSBBE_TMParaSetGUIDlg::StrToSynCode(const char * pStr)
{
    char c,C;
    const char*  p;
...
void SSBBE_TMParaSetGUIDlg::showTargetParam(int nIndex, int nType)
{
    if (nType == 0)
    {
        int i;
        ...
        //帧同步码组位置
        ui->comboBox_FrameSyncCodePos->clear();
        ...
        ui->label_SubFrameLen->setText("格式长\n[" + QString::number(m_DlgParaSetStruct[nIndex][12].MinVal.lmin) +
                "~" + QString::number(m_DlgParaSetStruct[nIndex][12].MaxVal.lmax) + "]");
        //帧同步码组长
    ...
    {
        //数据帧格式码
        //ui->lineEdit_FrameDataFlag->setText();
        //帧同步码组位置
...
int SSBBE_TMParaSetGUIDlg::getSharedMemoryTargetNum()
{
    DWORD stLen = sizeof(SMCU::MCSSysTarget4StatusStruct);
    if(m_pStateMgr != NULL)
        m_pMCSTargetSysStatus  = (SMCU::MCSSysTarget4StatusStruct*)m_pStateMgr->
...
void SSBBE_TMParaSetGUIDlg::showRadioButton()
{
...
//    ui->radioButton_Target3->hide();
//    ui->radioButton_Target4->hide();
//    switch(m_targetNum_usedInParam)
//    {
//    case 1:
//        ui->radioButton_Target1->show();
//        break;
...
void SSBBE_TMParaSetGUIDlg::getParamTarget()
{
    bool ok;
    //帧同步码组位置
...
void SSBBE_TMParaSetGUIDlg::set_radioButton_Target()
{
    switch(m_nTargetNum)
    {
    case 0:
...
void SSBBE_TMParaSetGUIDlg::on_radioButton_Target1_clicked()
{
    if(1 != CheckPara())
    {
        set_radioButton_Target();
...
void SSBBE_TMParaSetGUIDlg::on_radioButton_Target2_clicked()
{
    if(1 != CheckPara())
    {
        set_radioButton_Target();
...
void SSBBE_TMParaSetGUIDlg::on_radioButton_Target3_clicked()
{
    if(1 != CheckPara())
    {
        set_radioButton_Target();
...
void SSBBE_TMParaSetGUIDlg::on_radioButton_Target4_clicked()
{
    if(1 != CheckPara())
    {
        set_radioButton_Target();
...
//更改参数按钮休息处理函数
void SSBBE_TMParaSetGUIDlg::on_pushButton_Modify_clicked()
{
    if(ui->pushButton_Modify->text() == "更改")
    {
...
//译码方式
void SSBBE_TMParaSetGUIDlg::on_comboBox_DecodeType_currentIndexChanged(int index)
{
    if (ui->pushButton_Modify->text() == "更改")
        return;
    if (index== 0){//关
        ui->comboBox_FrameSyncCodePos->setEnabled(true);
        ui->lineEdit_FrameDataFlag->setEnabled(true);
...
//v-译码方式
void SSBBE_TMParaSetGUIDlg::on_comboBox_ViterbType_currentIndexChanged(int index)
{
    if (ui->pushButton_Modify->text() == "更改")
        return;
    // 1/2
...
//解扰控制
void SSBBE_TMParaSetGUIDlg::on_comboBox_RandEn_currentIndexChanged(int index)
{
    if (ui->pushButton_Modify->text() == "更改")
        return;
...
//副帧同步方式
void SSBBE_TMParaSetGUIDlg::on_comboBox_SubFrmSyncType_currentIndexChanged(int index)
{
    if (ui->pushButton_Modify->text() == "更改")
        return;
...
void SSBBE_TMParaSetGUIDlg::updateRadioButton()
{
...
void SSBBE_TMParaSetGUIDlg::on_comboBox_RandPolyLen_currentIndexChanged(int index)
{
    QRegExp regExp2("[A-Fa-f0-9]{1,2}");
    QRegExp regExp4("[A-Fa-f0-9]{1,4}");
    QRegExp regExp6("[A-Fa-f0-9]{1,6}");
    QRegExp regExp8("[A-Fa-f0-9]{1,8}");
    switch (index) {
    case 0:
        //解扰多项式[HEX]
    ...
    case 1:
        //解扰多项式[HEX]
        ui->lineEdit_RandPoly->setValidator(new QRegExpValidator(regExp4, this));
        //解扰初相[HEX]
        ui->lineEdit_RandPhase->setValidator(new QRegExpValidator(regExp4, this));
        break;
    case 2:
        //解扰多项式[HEX]
        ui->lineEdit_RandPoly->setValidator(new QRegExpValidator(regExp6, this));
        //解扰初相[HEX]
        ui->lineEdit_RandPhase->setValidator(new QRegExpValidator(regExp6, this));
        break;
    case 3:
        //解扰多项式[HEX]
        ui->lineEdit_RandPoly->setValidator(new QRegExpValidator(regExp8, this));
        //解扰初相[HEX]
        ui->lineEdit_RandPhase->setValidator(new QRegExpValidator(regExp8, this));
        break;
    default:
        break;
    }
